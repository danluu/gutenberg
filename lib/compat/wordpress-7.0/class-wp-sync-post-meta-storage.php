	...
	class WP_Sync_Post_Meta_Storage implements WP_Sync_Storage {
		...
		private function get_bounded_updates_after_cursor( int $post_id, string $room, int $cursor, int $max_meta_id, int $max_response_bytes ): array {
			...
			while ( $current_cursor < $max_meta_id ) {
				$rows = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT meta_id, meta_value, LENGTH(meta_value) AS meta_value_length FROM {$wpdb->postmeta} WHERE post_id = %d AND meta_key = %s AND meta_id > %d AND meta_id <= %d ORDER BY meta_id ASC LIMIT %d",
						$post_id,
						self::SYNC_UPDATE_META_KEY,
						$current_cursor,
						$max_meta_id,
				...
				foreach ( $rows as $row ) {
					...
					if ( $bytes_returned > 0 && $bytes_returned + $row_size > $max_response_bytes ) {
