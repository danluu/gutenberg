...
const CODEX_BIN =
	process.env.RTC_FUZZ_ANALYSIS_CODEX_BIN ??
	process.env.RTC_FUZZ_CODEX_BIN ??
...
async function findRunDirsWithTriageState( root ) {
	...
	async function walk( dir, depth ) {
		...
		for ( const entry of entries ) {
			...
			if ( entry.name === 'external-imports' && ! INCLUDE_EXTERNAL_IMPORTS ) {
				continue;
			}
...
function getRepresentativeFamilyKeyForJob( job, sourceSignature, noAnalysis ) {
	...
	const isRunningUnmarkedRepresentative =
		job?.status === 'running' && isProcessAlive( job.pid );
	if (
		job?.noAnalysisProductEvidenceRepresentative !== true &&
		! isRunningUnmarkedRepresentative
	) {
		return null;
	}
	const jobFamilyKey = canonicalizeSemanticFamilyKey(
		job.noAnalysisProductEvidenceRepresentativeFamilyKey ?? ''
	);
	if ( jobFamilyKey && jobFamilyKey !== familyKey ) {
		return null;
	}
	return familyKey;
}

function isRunnableNoAnalysisProductEvidenceRepresentativeJob(
	job,
	sourceSignature,
	noAnalysis
) {
	if ( ! getRepresentativeFamilyKeyForJob( job, sourceSignature, noAnalysis ) ) {
		return false;
	}
	if ( job.status === 'running' ) {
		return isProcessAlive( job.pid );
	}
	if ( ! [ 'queued', 'retry', 'failed' ].includes( job.status ) ) {
		return false;
	}
	if (
		job.status === 'retry' &&
...
function isRetryableSuppressedNoAnalysisProductEvidenceRepresentativeJob(
	job,
	sourceSignature,
	noAnalysis
) {
	return (
		[ 'source-suppressed', 'stale-source' ].includes(
			sourceSignature?.status
		) &&
		isRunnableNoAnalysisProductEvidenceRepresentativeJob(
			job,
			sourceSignature,
			noAnalysis
		)
...
function getRunnableNoAnalysisProductEvidenceRepresentativeJobs(
	state,
	sourceState,
	noAnalysis
) {
...
function shouldDeferSuppressionForRunningRepresentative(
	job,
	sourceSignature,
	noAnalysis,
	reason
	...
	return (
		reason ===
			'producer-inactive-product-evidence-drain-not-live-admitted' ||
		reason.startsWith( 'source-status-' )
	);
}
...
function shouldAnalyzeSignature( signature, job, noAnalysis = null ) {
	...
	if (
		...
		! isRetryableSuppressedNoAnalysisProductEvidenceRepresentativeJob(
			job,
			signature,
			noAnalysis
		)
...
function hasMeaningfulSourceProductEvidence( signature ) {
	...
	if ( /reload|rejoin|save|revision|restore|persist|lifecycle/.test( lifecycleContext ) ) {
		return true;
	}
...
async function suppressJobsWithInactiveSource(
	...
	for ( const job of Object.values( state.jobs ?? {} ) ) {
		const sourceSignature = sourceState.signatures?.[ job.hash ] ?? null;
			...
			if (
				shouldDeferSuppressionForRunningRepresentative(
					job,
					sourceSignature,
					noAnalysis,
					reason
				...
				markNoAnalysisProductEvidenceRepresentativeJob(
					job,
					getRepresentativeFamilyKeyForJob(
						job,
						sourceSignature,
						noAnalysis
					),
				...
				job.sourceSuppressionDeferredReason = reason;
...
function shouldRestoreProducerFamilyCappedProductEvidenceJob(
	...
	return (
		[ 'family-capped', 'source-suppressed' ].includes(
			...
			noAnalysis?.preserveProductEvidence === true &&
			isCurrentActionableProductEvidenceSourceSignature(
				sourceSignature
			) &&
...
async function addCurrentOutputPeerNoAnalysisProductEvidenceFamilyCounts(
	...
	for ( const peerRunDir of await getCurrentOutputPeerRunDirs() ) {
		...
		const [ peerSourceState, peerAnalysisState ] = await Promise.all( [
			...
			readJson(
				path.join( peerRunDir, '.triage-watcher/analysis-tier/state.json' ),
				null
			),
		] );
...
function restoreNoAnalysisProductEvidenceFamilyCappedJobs(
	...
	if ( restored > 0 ) {
		state.lastNoAnalysisProductEvidenceFamilyCapRestored = {
			...
			restored,
			reason:
				'restored product-evidence family-capped jobs so unsafe representatives cannot hide siblings',
		};
	}
	return restored;
}
...
function shouldApplyCurrentOutputFamilyCap( signature, familyKey, job = null ) {
	...
	if (
		...
		return FAMILY_OCCUPANCY_JOB_STATUSES.has(
			job?.status ?? 'unknown'
		);
	}
...
async function markJobsStaleForRunDir(
	...
	for ( const job of Object.values( state.jobs ?? {} ) ) {
		...
		if (
			...
			job.staleRunDirDeferredReason = reason;
			continue;
		}
		if ( job.status === 'running' && isProcessAlive( job.pid ) ) {
			try {
...
async function runScanCycle() {
	...
	if ( staleRunDirReason ) {
		...
		const representativeJobs =
			getRunnableNoAnalysisProductEvidenceRepresentativeJobs(
				state,
				sourceState,
				noAnalysis
			);
		if ( representativeJobs.length > 0 ) {
			state.staleRunDirProductEvidenceDrain = {
				at: new Date().toISOString(),
				runDir: RUN_DIR,
				reason: staleRunDirReason,
				preservedJobs: representativeJobs.map( ( job ) => job.hash ),
			};
			await writeState( state );
		} else {
	...
	if (
		...
		state.lastLaunchSkippedForNoAnalysis = {
			at: new Date().toISOString(),
			reason:
				'active producer no-analysis sentinel has no live-analysis admission; suppressed stale/noise jobs and exiting instead of keeping a warm analysis loop',
			reasonKind: noAnalysis.reasonKind ?? null,
			family: noAnalysis.family ?? null,
			group: noAnalysis.group ?? null,
	...
	if (
		...
		state.lastNoAnalysisRepresentativeDrain = {
			at: new Date().toISOString(),
			reason:
				'continuing bounded analysis for selected product-evidence representative after producer no-analysis admission expired',
			jobs: representativeJobs.map( ( job ) => job.hash ),
			reasonKind: noAnalysis.reasonKind ?? null,
			family: noAnalysis.family ?? null,
			group: noAnalysis.group ?? null,
		};
