...
function shouldBypassStartupStallCooldown( group, groupState = null ) {
	...
	const bypassSeedDrain =
		group?.env?.RTC_FUZZ_SUPERVISOR_BYPASS_STARTUP_STALL_SEED_DRAIN ===
		'1';
	if (
		groupState &&
...
async function normalizeWpEnvLocalThemeMappings( group ) {
	...
	for ( const [ mappingKey, localPath ] of Object.entries(
		...
		if ( ! ( await fileExists( path.join( group.repoRoot, localPath ) ) ) ) {
			continue;
		}
...
function looksLikeWpEnvCloneDestinationConflict( output ) {
	return getWpEnvCloneDestinationConflictPath( output ) !== null;
}
...
async function cleanPartialWpEnvCheckoutsBeforeStartRetry(
	...
	if (
		conflictPath &&
		isPathInsideDirectory( installPath, conflictPath )
	) {
	...
	await log(
		`${ group.name }: removed partial generated-compose checkout(s) before wp-env start retry (${ reason }): ${ checkoutPaths.join(
			', '
		) }.`
	);
	await event( {
...
async function findAvailableWsRelayPort( requestedPort ) {
	...
	for ( let offset = 1000; offset < 1000 + WS_RELAY_PORT_SEARCH_LIMIT; offset++ ) {
...
async function main() {
	...
	while ( Date.now() < END_AT ) {
		...
		for (
			...
			// they materialize more stale run directories.
			await syncGroupConfigs();
			const groupState = state.groups[ groupIndex ];
			if ( ! groupState ) {
				continue;
			}
