...
const CODEX_BIN =
	process.env.RTC_FUZZ_LIVE_ANALYSIS_CODEX_BIN ??
	process.env.RTC_FUZZ_CODEX_BIN ??
	process.env.CODEX_BIN ??
	'codex';
const DEEP_CODEX_BIN =
	process.env.RTC_FUZZ_LIVE_DEEP_ANALYSIS_CODEX_BIN ??
...
async function getNoAnalysisDrainRunDirs( activeRunDirs ) {
	...
	for ( const runDir of runDirs ) {
		...
		const triageActionability = summarizeActionableTriageState(
			...
			const shouldKeepProductEvidenceDrain =
			...
			if (
				...
				! shouldKeepProductEvidenceDrain
			) {
				continue;
			}
		drainDirs.push( {
...
async function updateNoAnalysisLiveAdmission(
	runDir,
	launchFamilyKeys = []
) {
	...
	sentinel.liveAnalysisAdmission = {
		...
		familyKeys,
		reason:
			'live-analysis selected this no-analysis drain as the one product-evidence representative for each listed semantic family',
	};
	await writeJsonFile( sentinelPath, sentinel );
	return true;
}

async function reconcileNoAnalysisLiveAdmissions( drainRunDirs, launchRunDirs ) {
...
function isRetryableNoAnalysisProductEvidenceDeepRepresentative(
	signature,
	deepAnalysisJob,
	noAnalysis
) {
	...
	if ( ! familyKey || ! deepAnalysisJob ) {
		return false;
	}
	if ( deepAnalysisJob.noAnalysisProductEvidenceRepresentative !== true ) {
		return false;
	}
	const jobFamilyKey = canonicalizeSemanticFamilyKey(
		deepAnalysisJob.noAnalysisProductEvidenceRepresentativeFamilyKey ??
			deepAnalysisJob.semanticFamilyKey ??
			''
	);
	if ( jobFamilyKey && jobFamilyKey !== familyKey ) {
		return false;
	}
	if ( deepAnalysisJob.status === 'running' ) {
		return isProcessAlive( deepAnalysisJob.pid );
	}
	if (
		! [
			'queued',
			...
			'source-suppressed',
			'stale-source',
		].includes( deepAnalysisJob.status )
	) {
		return false;
	}
	return ( deepAnalysisJob.attempts ?? 0 ) < DEEP_ANALYSIS_MAX_ATTEMPTS;
}
...
async function getCurrentOutputDeepFamilyCounts( runDirRecords ) {
	const counts = new Map();
	for ( const { runDir } of runDirRecords ) {
		const [ noAnalysis, triageState, deepAnalysisState ] = await Promise.all( [
			readNoAnalysisSentinel( runDir ),
			readJsonFile( path.join( runDir, '.triage-watcher/state.json' ) ),
			readJsonFile(
				path.join( runDir, '.triage-watcher/deep-analysis-tier/state.json' )
...
function hasAdmittedNoAnalysisFamilyKey( noAnalysis, familyKeys ) {
	const admittedFamilyKeys = getNoAnalysisLiveAdmissionFamilyKeys( noAnalysis );
	...
	return [
		...
		`export RTC_FUZZ_ANALYSIS_INTERVAL_MS=${ shellQuote(
			ANALYSIS_INTERVAL_MS
					...
					`export RTC_FUZZ_ANALYSIS_CODEX_BIN=${ shellQuote(
						CODEX_BIN
					) }`,
...
function buildDeepAnalysisCommand( runDir ) {
	...
	return [
		...
		`export RTC_FUZZ_DEEP_ANALYSIS_INTERVAL_MS=${ shellQuote(
			...
			`export RTC_FUZZ_DEEP_ANALYSIS_CODEX_BIN=${ shellQuote(
				DEEP_CODEX_BIN
			) }`,
...
function isActionableSignature(
	...
	const retryableDeepRepresentative =
		isRetryableNoAnalysisProductEvidenceDeepRepresentative(
			signature,
			deepAnalysisJob,
			noAnalysis
		);
...
function summarizeActionableTriageState(
	...
	return {
		...
		failureBucketCounts:
			triageState?.metrics?.failureBucketCounts ?? {},
...
async function monitorOnce() {
	...
	if ( staleSupervisorStateInfo ) {
		...
		const action = {
			...
			supervisorLastUpdatedAt:
				staleSupervisorStateInfo.lastUpdatedAt,
			supervisorStateAgeMs: staleSupervisorStateInfo.ageMs,
			supervisorStateStaleTtlMs:
				staleSupervisorStateInfo.staleTtlMs,
			staleSessionsCleaned,
		};
		await writeJsonFile( STATE_PATH, {
			updatedAt: new Date().toISOString(),
