...
const CODEX_BIN =
	process.env.RTC_FUZZ_DEEP_ANALYSIS_CODEX_BIN ??
...
function isActionableSourceSignatureForDeepCandidate(
	signature,
	job,
	noAnalysis
) {
	return (
		isActionableSourceSignature( signature, noAnalysis ) ||
		isRunnableNoAnalysisProductEvidenceRepresentativeJob(
			job,
			signature,
			noAnalysis
...
function shouldDeferSuppressionForRepresentative(
	job,
	sourceSignature,
	noAnalysis,
	reason
...
async function collectCandidates(
	...
	for ( const firstJob of Object.values( analysisState.jobs ?? {} ) ) {
		...
		if (
			! isActionableSourceSignatureForDeepCandidate(
				sourceSignature,
				deepState?.jobs?.[ firstJob.hash ],
				noAnalysis
			)
		) {
			continue;
...
async function suppressJobsWithInactiveSource(
	...
	for ( const job of Object.values( state.jobs ?? {} ) ) {
		...
		const reason = getSourceSuppressionReason( sourceSignature, noAnalysis );
		if ( ! reason ) {
			continue;
		}
		const producerFamilyCapKey = getProducerNoAnalysisFamilyCapKey(
		...
		if (
			shouldDeferSuppressionForRepresentative(
				job,
				sourceSignature,
				noAnalysis,
				reason
...
async function launchCodexDeepAnalysisJob(
	...
	child.on( 'exit', async ( code, signal ) => {
		...
		const staleRepresentativeRace =
			[ 'source-suppressed', 'stale-source' ].includes(
				latestJob.status
			) &&
			latestJob.noAnalysisProductEvidenceRepresentative === true;
		if ( TERMINAL_JOB_STATUSES.has( latestJob.status ) ) {
			if ( ! staleRepresentativeRace ) {
				await writeState( latest );
				return;
			}
			latestJob.terminalStatusOverriddenAt = new Date().toISOString();
			latestJob.terminalStatusOverriddenFrom = latestJob.status;
			latestJob.terminalStatusOverrideReason =
				'own Codex child exited after a concurrent stale-source mark for a product-evidence representative';
		}
...
function buildCodexPrompt( sourceState, candidate, relatedSummaries, paths ) {
	...
	return [
		...
		'- Do not edit production/source files. Write only the durable artifacts requested under the deep analysis job directory.',
			...
			'- Do not bypass the analysis guard wrappers with absolute tool paths such as `/bin/cat`, `/usr/bin/sed`, `/usr/bin/head`, `/usr/bin/tail`, or `/usr/bin/rg`.',
			'',
			'Allowed work:',
			...
			'- Do not run `rg`, `grep`, `sed`, `cat`, `head`, or `tail` directly on `command.log`. If command-log evidence is still needed, use `node bin/rtc-browser-fuzz-command-log-probe.mjs <command.log> --pattern <regex> --max-matches 40` so only a bounded current-run tail is inspected.',
...
async function runScanCycle() {
	...
	if (
		...
		state.lastNoAnalysisRepresentativeDrain = {
			at: new Date().toISOString(),
			reason:
				'continuing bounded deep analysis for selected product-evidence representative after producer no-analysis admission expired',
