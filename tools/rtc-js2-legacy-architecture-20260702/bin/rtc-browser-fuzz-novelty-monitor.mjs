...
const EXPANSION_POLICY_VERSION = 23;
	...
	return (
		Math.max(
			state.benchmarkCanaryNoAnalysisProductEvidenceByGroup?.[ group ] ??
				0,
			state.benchmarkCanarySummaryProductFailureCountsByGroup?.[ group ] ??
				0,
		...
		rows.push( [
			...
			currentRunTransportRecords,
				...
				currentRunGreen && directCurrentOutputGreen
					? 'direct-current-run-success'
					: currentRunGreen && stickyCurrentRunGreen
					? 'sticky-deadline-current-run-green'
					: 'none',
			closureSource || 'none',
			closureRunDir,
...
async function applyPolicy(
	...
	function canRotateAwayFromGroup( group ) {
		...
		switch ( group ) {
			...
			case 'novelty-ws-same-user-lifecycle':
				return sameUserLifecycleRecords >= 150;
			case 'novelty-ws-same-user-separate-context-lifecycle':
				return sameUserSeparateContextLifecycleRecords >= 100;
			default:
				return false;
		}
	}
	...
	if (
			...
			enabled.size >= TARGET_ENABLED_GROUPS &&
			( sameUserRecords < 150 ||
				sameUserSeparateContextRecords < 100 ||
				! httpProbeEnabled ) &&
	...
	if (
		...
		sameUserSeparateContextLifecycleRecords < 100
	) {
		await enableGroup(
			'novelty-ws-same-user-separate-context-lifecycle',
			'same-user separate-context lifecycle coverage is below target; exercise two isolated browser contexts under the same account',
			{ allowRotation: true }
		);
	}
...
async function writeStartupStatus() {
	...
	await writeBenchmarkCanaryCoverageTelemetry(
		...
		[],
		activeRunDirsIncludingPausedNoAnalysis,
		await findSummaryFiles( activeRunDirs )
	);
	const lines = [
		'# RTC Novelty Monitor',
		'',
