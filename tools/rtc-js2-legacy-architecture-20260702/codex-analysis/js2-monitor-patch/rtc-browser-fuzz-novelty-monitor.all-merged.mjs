...
const BENCHMARK_CANARY_GROUP_BY_CASE = {
	...
	'rtc-reference-oracle': 'novelty-http-rtc-reference-oracle',
...
const BENCHMARK_CANARY_DEADLINE_CLOSURE_POLICY_VERSION = 3;
...
const RUN_LOCAL_NOISE_POLICY_VERSION = 41;
...
const ACTION_COVERAGE_GROUPS = {
	...
	'table-stale-snapshot-html': [ 'novelty-http-table-stale-snapshot' ],
...
const EXPANSION_POLICY_VERSION = 24;
...
const POLICY_REQUIRED_BOOTSTRAP_GROUPS = [ RTC_REFERENCE_ORACLE_GROUP ].filter(
...
function getPolicyProtectedSupervisorGroupLimit( baseLimit, groups ) {
	...
	const policyRequiredCount = getPolicyRequiredGroupCount( groups );

	return Math.min(
		groupCount,
		Math.max(
			normalizedBaseLimit,
			normalizedBaseLimit + policyRequiredCount
		)
	);
}
...
const SUPERVISOR_GAP_PUBLICATION_PRIORITY_GROUPS = [
	'novelty-ws-real-user-rich-text',
	'novelty-ws-real-user-save-reload',
	'novelty-ws-real-user-editing',
	'novelty-ws-thirty-user-lifecycle',
...
const PROFILE_GROUPS = [
	...
	},
	{
		...
		env: {
			...
			GUTENBERG_RTC_BROWSER_FORCE_SAVE_STEPS: '3,8,13,15',
	...
	},
	{
		...
		env: {
			GUTENBERG_RTC_BROWSER_ACTION_SEQUENCE:
				'assert-presence-list,append-paragraph,assert-presence-list,assert-selection-cursor-smoke,append-paragraph,assert-presence-list',
			GUTENBERG_RTC_BROWSER_ENABLE_LIFECYCLE_EVENTS: '1',
	...
	},
	{
		...
		env: {
			GUTENBERG_RTC_BROWSER_ACTION_SEQUENCE:
				'table-stale-snapshot-html,edit-table-array-attributes,ui-table-cell-edit,concurrent-paragraphs,append-paragraph,move-block',
			GUTENBERG_RTC_BROWSER_ENABLE_LIFECYCLE_EVENTS: '1',
			GUTENBERG_RTC_BROWSER_EXTRA_COLLABORATORS: '1',
			GUTENBERG_RTC_BROWSER_FINAL_PERSISTENCE_ORACLE: 'fail',
			GUTENBERG_RTC_BROWSER_FINAL_PERSISTENCE_PUBLISH: '1',
...
const state = ( await readJsonFile( STATE_PATH ) ) ?? {
	...
	benchmarkCanarySummaryProductFailureCountsByGroup: {},
	benchmarkCanarySummaryProductFailureRunDirsByGroup: {},
...
if ( state.expansionPolicyVersion !== EXPANSION_POLICY_VERSION ) {
	...
	if ( ! shouldDeferPolicyRequiredBootstrapGroupsForBenchmarkCanaryCap() ) {
		for ( const group of POLICY_REQUIRED_BOOTSTRAP_GROUPS ) {
			if (
...
state.benchmarkCanarySummaryProductFailureCountsByGroup ??= {};
state.benchmarkCanarySummaryProductFailureRunDirsByGroup ??= {};
state.startupFailureCountsByProfile ??= {};
state.startupFailureCountsByGroup ??= {};
...
const clearedObsoleteAllZeroDuplicateNoisePauses =
	clearObsoleteAllZeroDuplicateNoisePauses();
if ( clearedObsoleteAllZeroDuplicateNoisePauses > 0 ) {
	state.changes.push( {
		at: new Date().toISOString(),
		action: 'clear-obsolete-all-zero-duplicate-noise-pauses',
		reason: `cleared ${ clearedObsoleteAllZeroDuplicateNoisePauses } all-zero duplicate/noise pause(s) created by the old block top-off health warning; current duplicate/noise evidence now controls browser scheduling`,
	} );
}
...
function getBenchmarkCanaryFloorPromotionBlockedGroups() {
	...
	const promotionBlockedCount =
		Number.parseInt( floor.promotion_blocked_group_count ?? '0', 10 ) ||
		0;
...
function getAuthoritativeOpenBenchmarkCanaryFloorGroups() {
	const openStatusGroups = getCurrentOpenBenchmarkCanaryCoverageStatusGroups();
	const floorGroups = getBenchmarkCanaryFloorPromotionBlockedGroups();
	if ( openStatusGroups.length === 0 ) {
		return benchmarkCanaryCoverageStatusRows.length === 0 ? floorGroups : [];
	}
...
async function readBenchmarkCanaryFeedbackRows() {
	...
	return [ ...primaryRows, ...overrideRows ];
}
	...
	return rows
		.map( ( row ) => {
			const rowStatus = String(
				row.status ?? row.benchmark_status ?? ''
			).toLowerCase();
			if ( rowStatus !== 'promotion_blocked' ) {
				return null;
			}

			const benchmarkCase = getBenchmarkCanaryCase( row );
			const group = BENCHMARK_CANARY_GROUP_BY_CASE[ benchmarkCase ];
			if ( ! group ) {
				return null;
			}
			...
			return {
				...
				reason:
					row.notes ??
					'benchmark canary feedback row is promotion_blocked; keep equivalent fuzz coverage open until current-run green or explicit downscope',
			};
		} )
		.filter( Boolean );
}
...
function getBenchmarkCanaryFeedbackReason( group ) {
	...
	if ( matchingRows.some( isBenchmarkCanaryCoverageStatusRow ) ) {
		return 'benchmark canary coverage status requires direct current-run green or explicit downscope before this forced lane can stop consuming protected capacity';
	}
...
function getScheduledBenchmarkCanaryGroupsForBudget() {
	...
	if (
		...
		return authoritativeFloorGroups;
	}
...
function getSupervisorCoverageGapPriorityGroups( enabled ) {
	if (
		! isDeadlineBenchmarkCanaryBudgetCapActive() ||
		getBenchmarkCanarySchedulingLimit() > 0
	) {
		return [];
	}
	const unmetGoals = state.coverageGuidance?.unmetGoals ?? [];
	if ( unmetGoals.length === 0 ) {
		return [];
	}
	const gapGroups = new Set(
		unmetGoals.flatMap( ( goal ) => goal.groups ?? [] )
	);
	return uniqueStringList( [
		...SUPERVISOR_GAP_PUBLICATION_PRIORITY_GROUPS,
		...getBootstrapZeroCoveragePriorityGroups(),
		...getBootstrapSuccessDeficitGroups(),
		...( state.coverageGuidance?.recommendedGroups ?? [] ),
	] ).filter( ( group ) => enabled.has( group ) && gapGroups.has( group ) );
}

function orderEnabledGroupsForSupervisor( enabledGroups ) {
	const enabled = new Set( enabledGroups ?? [] );
	return uniqueStringList( [
		...getSupervisorCoverageGapPriorityGroups( enabled ),
		...( shouldDeferPolicyRequiredBootstrapGroupsForBenchmarkCanaryCap()
			? []
			: POLICY_REQUIRED_BOOTSTRAP_GROUPS.filter( ( group ) =>
					enabled.has( group )
			  ) ),
...
function getSummaryHistoryEvents( record ) {
	const events = [];
	for ( const coverage of Array.isArray( record?.behavioralCoverage )
		? record.behavioralCoverage
		: [] ) {
		for ( const event of Array.isArray( coverage?.historyEvents )
			? coverage.historyEvents
			: [] ) {
			events.push( event );
		}
	}
	for ( const attempt of Array.isArray( record?.attempts )
		? record.attempts
		: [] ) {
		events.push( ...getSummaryHistoryEvents( attempt ) );
	}
	for ( const event of Array.isArray( record?.historyEvents )
		? record.historyEvents
		: [] ) {
		events.push( event );
	}
	return events;
...
function getSummaryFailureText( record ) {
	...
	for ( const event of getSummaryHistoryEvents( record ) ) {
		if ( event?.error ) {
...
function isFailedSummaryRecord( record ) {
	...
	if ( record.code !== undefined && record.code !== null && record.code !== 0 ) {
		return true;
	}
	if ( [ 'failed', 'failure', 'uncertain' ].includes( record.kind ) ) {
		return true;
	}
	return ( Array.isArray( record.attempts ) ? record.attempts : [] ).some(
		isFailedSummaryRecord
	);
}

function hasRtcReferenceOracleSummaryEvidence( record ) {
	if ( record?.rtcReferenceOracle ) {
		return true;
	}
	if (
		( Array.isArray( record?.behavioralCoverage )
			...
			( coverage ) =>
				coverage?.actionProfile === 'rtc-reference-oracle' ||
				coverage?.rtcReferenceOracle
		)
	) {
		return true;
	}
	return getSummaryHistoryEvents( record ).some( ( event ) =>
		/^(?:post-new-ui-(?:reference|rtc)-oracle|rtc-reference-)/.test(
			String( event?.phase ?? '' )
		)
	);
}

function hasRtcReferenceOracleProductProgress( record ) {
	const events = getSummaryHistoryEvents( record );
	if (
		events.some(
			( event ) =>
				/^(?:post-new-ui-reference-oracle|rtc-reference-reference-apply|rtc-reference-rtc-apply|rtc-reference-initial-convergence|rtc-reference-convergence|rtc-reference-oracle)$/.test(
					String( event?.phase ?? '' )
				) && event?.status !== 'invoke'
		)
	) {
	...
	return /Post save did not settle clean|Collaborative state did not converge|RTC reference oracle mismatch|RTC fuzz-only core-data diagnostics/i.test(
		getSummaryFailureText( record )
	);
}

function isProductFailureSummaryRecord( record, filePath ) {
	if ( ! isFailedSummaryRecord( record ) ) {
		return false;
	}
	const group = getSummaryRecordGroupName( record, filePath );
	if ( group === RTC_REFERENCE_ORACLE_GROUP ) {
		return (
			hasRtcReferenceOracleProductProgress( record ) &&
			! isStrictPreActionStartupSummaryRecord( record )
		);
	}
	return hasSummaryProductEvidence( record );
}

async function summarizeSummaryProductFailureSignals( summaryFiles = [] ) {
	const recordsByGroup = {};
	const runDirsByGroup = {};

	for ( const filePath of uniquePathList( summaryFiles ) ) {
		...
		for ( const line of text.split( '\n' ) ) {
			...
			if ( ! isProductFailureSummaryRecord( record, filePath ) ) {
				continue;
			}
			const group = getSummaryRecordGroupName( record, filePath );
			if ( ! group ) {
			...
			incrementCounter( recordsByGroup, group );
			const runDir = getCurrentOutputRunDirFromPath( filePath );
			if ( runDir ) {
...
async function writeBenchmarkCanaryCoverageTelemetry(
	...
	const summaryProductFailureSignals =
		await summarizeSummaryProductFailureSignals( currentSummaryFiles );
	const rows = [
		[
			'updated_at',
			'source',
	...
	for ( const group of groups ) {
		...
		const cachedCurrentRootSuccessfulGroupRecords =
			state.benchmarkCanaryRecordCountsOutputDir === OUTPUT_DIR
				...
				: 0;
		const retainedCurrentRootSuccessfulGroupRecords =
			Math.max(
		...
		const productEvidenceRecords = Math.max(
			...
			summaryProductFailureSignals.recordsByGroup?.[ group ] ?? 0,
			Number.isFinite( supervisorProductEvidenceRecords )
				? supervisorProductEvidenceRecords
				: 0
		);
		const productEvidenceRunDir =
			summaryProductFailureSignals.runDirsByGroup?.[ group ]?.[ 0 ] ||
			noAnalysisProductEvidence.runDirsByGroup?.[ group ]?.[ 0 ] ||
			'';
		...
		const observedExplicitDownscope =
			isBenchmarkCanaryExplicitlyDownscoped( group );
		const currentProductFailure = productEvidenceRecords > 0;
		rememberBenchmarkCanaryDeadlineClosure( group, {
			currentRunGreen: directCurrentOutputGreen && ! currentProductFailure,
			explicitDownscope: observedExplicitDownscope,
			closureSource:
				directCurrentOutputGreen && ! currentProductFailure
				? 'direct-current-run-success'
		...
		const deadlineClosureSource =
			...
			? stickyDeadlineClosureSource
			: '';
		const closureSource = currentProductFailure
			? ''
			: isDeadlineBenchmarkCanaryBudgetCapActive()
		...
		const coverageReady =
			...
			( isDeadlineBenchmarkCanaryBudgetCapActive()
				...
				: !! closureSource );
		const coverageState = currentProductFailure
			? group === RTC_REFERENCE_ORACLE_GROUP
				? 'current-run-rtc-reference-oracle-product-failure'
		...
		const statusOnly = coverageReady;
		...
		rows.push( [
			...
			Math.max(
				...
				summaryProductFailureSignals.runDirsByGroup?.[ group ]?.length ??
					0,
			...
			currentProductFailure
				? group === RTC_REFERENCE_ORACLE_GROUP
					? 'current RTC reference oracle product failure blocks promotion even when other current-run attempts succeeded'
					: 'current-run product failure evidence blocks promotion even when other current-run attempts succeeded'
	...
	state.benchmarkCanaryRetainedSuccessfulRecordCountsByGroup = mergeMaxCounters(
	...
	state.benchmarkCanarySummaryProductFailureCountsByGroup =
		summaryProductFailureSignals.recordsByGroup;
	state.benchmarkCanarySummaryProductFailureRunDirsByGroup =
		summaryProductFailureSignals.runDirsByGroup;
...
function shouldHoldNoisyBlockTopOff( triageYield ) {
	const hold = getCurrentRunDuplicateNoiseHold( triageYield, {
		allowGenericDuplicate: true,
		requireNoProductEvidence: false,
	} );
	if ( ! hold || hold.kind === 'startup-noise' ) {
		return false;
	}
	return (
		( hold.count ?? 0 ) >= TRIAGE_NOISE_DOMINANCE_MIN_CANDIDATES &&
		( hold.share ?? 0 ) >= TRIAGE_DUPLICATE_SHARE_HOLD
	);
}
...
function hasSummaryProductEvidence( record ) {
	...
	if (
		hasRtcReferenceOracleSummaryEvidence( record ) &&
		hasRtcReferenceOracleProductProgress( record )
	) {
		return true;
	}
...
function mergeNoveltySummary( target, source ) {
	for ( const [ profile, summary ] of Object.entries(
		source.byProfile ?? {}
	) ) {
		target.byProfile[ profile ] ??= {
			records: 0,
			newFeatures: 0,
			failures: 0,
			cdpRecords: 0,
		};
		target.byProfile[ profile ].records += summary.records ?? 0;
		target.byProfile[ profile ].newFeatures += summary.newFeatures ?? 0;
		target.byProfile[ profile ].failures += summary.failures ?? 0;
		target.byProfile[ profile ].cdpRecords += summary.cdpRecords ?? 0;
	}
	for ( const [ transport, count ] of Object.entries(
		source.byTransport ?? {}
	) ) {
		target.byTransport[ transport ] =
			( target.byTransport[ transport ] ?? 0 ) + count;
	}
	target.newCoverageHashes += source.newCoverageHashes ?? 0;
	target.newFeatureKeys += source.newFeatureKeys ?? 0;
	target.processed += source.processed ?? 0;
}

function mergeCoverageReadStats( target, source ) {
	for ( const key of [
		'filesRead',
		'linesSeen',
		'linesProcessed',
...
async function ingestAdditionalCoverageFiles( files, novelty, stats ) {
	const incrementalRead = await readCoverageRecords( files );
	if ( incrementalRead.records.length === 0 ) {
		return 0;
	}
	mergeCoverageReadStats( stats, incrementalRead.stats );
	mergeNoveltySummary( novelty, summarizeNovelty( incrementalRead.records ) );
	return incrementalRead.records.length;
}
...
function isObsoleteAllZeroDuplicateNoisePause( value ) {
	if ( getStoredNoisePauseKind( value ) !== 'triage-duplicate-noise' ) {
		return false;
	}
	if ( hasProductEvidenceDuplicatePauseEvidence( value ) ) {
		return false;
	}
	return /actionable top family share 0, actionable signatures 0, raw signatures 0, non-actionable 0, bootstrap stalls 0, normalization noise candidates 0, visible likely-real 0/.test(
		value?.reason ?? ''
	);
}

function clearObsoleteAllZeroDuplicateNoisePauses() {
	let cleared = 0;
	for ( const [ group, value ] of Object.entries(
		state.pausedGroups ?? {}
	) ) {
		if ( ! isObsoleteAllZeroDuplicateNoisePause( value ) ) {
			continue;
		}
		delete state.pausedGroups[ group ];
		cleared += 1;
...
async function ensureNoveltyGroupRepo( group ) {
	...
	if ( exit.code !== 0 ) {
		throw new Error(
			`repo prep failed for ${ group.name } with code=${
				exit.code
...
function queueNoveltyGroupRepoPrep( groups, reason ) {
	if ( ! REPOS_BASE ) {
		return;
	}
	...
	for ( const group of groups ) {
		if (
			! group?.repoRoot ||
			group.repoRoot === REPO_ROOT ||
			queuedNoveltyRepoPrepRoots.has( group.repoRoot )
		) {
			continue;
		}
		queuedNoveltyRepoPrepRoots.add( group.repoRoot );
		pendingNoveltyRepoPrepGroups.push( { group, reason } );
	}
	...
	noveltyRepoPrepPromise = ( async () => {
		while ( pendingNoveltyRepoPrepGroups.length > 0 ) {
			...
			try {
				...
				await log(
					`ACTION-NEEDED: failed to prepare isolated novelty repo for ${ group.name }: ${
...
async function writeSupervisorGroupsForEnabledGroups( enabledGroups ) {
	...
	const deferPolicyRequiredGroups =
		shouldDeferPolicyRequiredBootstrapGroupsForBenchmarkCanaryCap();
	for ( const group of deferredBenchmarkCanaryGroups ) {
		enabled.delete( group );
	}
	for ( const group of getScheduledBenchmarkCanaryGroupsForBudget() ) {
		if (
			PROFILE_BY_GROUP[ group ] &&
	...
	if ( ! deferPolicyRequiredGroups ) {
		...
		POLICY_REQUIRED_BOOTSTRAP_GROUPS.some( ( group ) =>
			enabled.has( group )
		)
	) {
		state.changes.push( {
			...
			groups: POLICY_REQUIRED_BOOTSTRAP_GROUPS.filter( ( group ) =>
				enabled.has( group )
			),
			openBenchmarkCanaryGroups:
				getAuthoritativeOpenBenchmarkCanaryFloorGroups(),
			maxEnabledGroups: MAX_ENABLED_GROUPS,
			reason: 'deadline benchmark-canary cap has more open canary rows than browser slots; policy-required reference coverage must not displace protected canary rows until the open floor fits the cap',
		} );
	}
	for ( const group of [ ...enabled ] ) {
		...
		if ( shouldPinCurrentOpenBenchmarkCanaryStatusGroup( group ) ) {
			state.changes.push( {
				at: new Date().toISOString(),
	...
	const orderedGroups = orderEnabledGroupsForSupervisor( [ ...enabled ] );
	const benchmarkCanarySchedulingLimit =
		isDeadlineBenchmarkCanaryBudgetCapActive()
			? getBenchmarkCanarySchedulingLimit()
			: MAX_ENABLED_GROUPS;
	let baseSupervisorGroupLimit = MAX_ENABLED_GROUPS;
	if ( isDeadlineBenchmarkCanaryBudgetCapActive() ) {
		baseSupervisorGroupLimit = benchmarkCanarySchedulingLimit;
		if ( benchmarkCanarySchedulingLimit === 0 ) {
			baseSupervisorGroupLimit = MAX_ENABLED_GROUPS;
		}
	}
...
async function ensureBootstrapSupervisorGroups() {
	...
	const benchmarkCanaryBootstrapLimit = Math.min(
		MAX_ENABLED_GROUPS,
		benchmarkCanarySchedulingLimit + getPolicyRequiredGroupCount( selected )
	);
	for ( const group of getScheduledBenchmarkCanaryForcedGroups() ) {
...
async function writeStartupStatus() {
	...
	const activeSummaryFiles = await findSummaryFiles( activeRunDirs );
...
async function writeStatusHeartbeat() {
	...
	const activeCoverageFiles = await findCoverageFiles( activeRunDirs );
	const activeSummaryFiles = await findSummaryFiles( activeRunDirs );
...
async function runPass() {
	...
	if ( START_SUPERVISOR ) {
		...
		if ( missingScheduledBenchmarkCanaryGroups.length > 0 ) {
			state.changes.push( {
				at: new Date().toISOString(),
				action: 'publish-missing-scheduled-benchmark-canary-groups',
				groups: scheduledBenchmarkCanaryGroups,
				missingGroups: missingScheduledBenchmarkCanaryGroups,
				reason: 'benchmark-canary scheduling changed after status/accounting refresh; rewrite supervisor-groups.json immediately so an open PR-evidence row cannot remain unscheduled behind stale enabledGroups',
			} );
			await writeJsonFileAtomic( STATE_PATH, state );
			await writeSupervisorGroupsForEnabledGroups( state.enabledGroups );
		}
	...
	const coverageRead = await readCoverageRecords( coverageFiles );
	const { stats } = coverageRead;
	const novelty = summarizeNovelty( coverageRead.records );
	const processedCurrentRunCoverage =
		coverageRead.records.some( isCurrentRunCoverageRecord );
	await ensureCurrentRunCounters(
		currentCoverageFiles,
		currentSummaryFiles,
		processedCurrentRunCoverage
	);
	await writeBenchmarkCanaryCoverageTelemetry(
		supervisorState,
		currentRunDirs,
	...
	if ( START_SUPERVISOR ) {
		...
		if ( currentScopeChanged ) {
			...
			coverageFiles = uniquePathList( [
				...
				currentSummaryFiles = await findSummaryFiles( currentRunDirs );
				const ingestedCurrentRecords =
					await ingestAdditionalCoverageFiles(
						currentCoverageFiles,
						novelty,
						stats
					);
	...
	if ( currentCoverageInputsChanged ) {
		...
		const ingestedCurrentRecords = await ingestAdditionalCoverageFiles(
			currentCoverageFiles,
			novelty,
			stats
		);
