...
productive_analysis_resolved_by_active_artifacts() {
	...
	case "$class" in
		already_resolved_by_active_artifacts|active_artifacts_adopted|adopted_active_artifacts|controller_generator_repair)
			;;
		*)
			return 1
			;;
...
productive_analysis_exact_blocker_suppressed() {
	local blocker_id=${1:-} action_kind=${2:-} evidence_path=${3:-} next_action=${4:-}
	local status
	case "$blocker_id" in
...
write_productive_analysis_exact_blockers() {
	...
	while IFS=$'\t' read -r generated_at action_id target_loop priority action_kind family_or_pr evidence_path next_action control_path; do
		...
		if productive_analysis_exact_blocker_suppressed "$blocker_id" "$action_kind" "$evidence_path" "$next_action"; then
			continue
		fi
...
active_reload_hydration_exact_session() {
	local hit root
	hit=$(active_session_matching '^rtc-focused-shards-append-reload-hydration.*exact' || true)
	if [ -n "$hit" ]; then
		printf '%s\n' "$hit"
		return 0
	fi
	hit=$(active_session_matching '^rtc-critical-continuation-reload-hydration.*exact|^rtc-deferred-job-reload-hydration.*exact|reload-hydration.*exact' || true)
	if [ -n "$hit" ]; then
		printf '%s\n' "$hit"
		return 0
	fi
		...
		for lane in pr17-1020002 pr07c-browser-env seed-5200005-reducer seed-1060015-reducer productive-analysis-action; do
			...
			case "$lane:$class" in
				...
				seed-1060015-reducer:resolved_by_active_artifact_downscoped)
					...
					;;
				productive-analysis-action:already_resolved_by_active_artifacts|productive-analysis-action:active_artifacts_adopted)
					printf '%s\t%s\t%s\t%s\tterminal\tproductive-analysis feedback newer than terminal classification\n' "$lane" "$class" "$classification" "$(file_mtime "$classification")"
					;;
				productive-analysis-action:controller_generator_repair)
...
write_blockers_and_queue() {
	...
	reload_followup_active=$(active_session_matching 'critical-continuation-reload-hydration-followup' || true)
	reload_repair_active=$(active_session_matching 'critical-continuation-reload-hydration-repair|reload-hydration-product-repair' || true)
	reload_reacquire_active=$(active_session_matching 'critical-continuation-reload-hydration-reproducer-reacquire|reload-hydration-reproducer-reacquire' || true)
	...
	productive_active=$(active_session_matching 'critical-continuation-productive-analysis-action' || true)
	benchmark_active=$(benchmark_exact_stack_active || true)
	benchmark_coverage_status=$(latest_benchmark_coverage_status || true)
	benchmark_product_active=$(active_session_matching 'critical-continuation-benchmark-canary-product-failure|benchmark-canary-product-failure' || true)
...
6. If a concrete product fix is isolated, create only a new non-destructive local branch in this worktree. Do not rewrite active fuzzing refs and do not push to GitHub from Jetstream.
...
8. Classification rules:
   ...
   - Use repair_branch_created only if a local branch exists and validation points to that fix.
...
7. If a concrete product fix is isolated, create only a new non-destructive local branch in this worktree. Do not rewrite active fuzzing refs and do not push to GitHub from Jetstream.
...
9. Classification rules:
   - Use repair_branch_created only if a local branch exists and focused evidence points to it.
...
5. If failures are persistent product bugs in the candidate, create or advance a new non-destructive local repair branch in this worktree. Do not rewrite active fuzzing refs and do not push to GitHub from Jetstream.
...
9. Classification rules:
   ...
   - Use repair_branch_created only if a local branch exists and focused evidence points to a product fix.
...
4. Decide whether the failures are product bugs in the reload-hydration candidate, test/harness diagnostics, invalid seeds, or still unresolved. If a concrete product fix is isolated, create only a new non-destructive local repair branch in this worktree. Do not rewrite active fuzzing refs and do not push to GitHub from Jetstream.
...
4. If a concrete low-risk product fix is isolated, create a new non-destructive local branch in this worktree and validate it against the reduced reproducer. Do not rewrite active fuzzing refs and do not push to GitHub from Jetstream.
5. Produce durable artifacts:
...
6. Do not create a product-code repair branch unless a stable same-head final-ui-witness failure is reacquired and a concrete low-risk fix is isolated. Do not rewrite active fuzzing refs and do not push to GitHub from Jetstream.
7. Produce durable artifacts:
   - Write a concise report to $report.
