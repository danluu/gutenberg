...
const DEFAULT_WP_ENV_CWD = 'wp-content/plugins/gutenberg';
const DEFAULT_MAX_CONSECUTIVE_INFRA_FAILURES = 3;
const DEFAULT_SEED_TIMEOUT_MS = 300000;
...
function toContainerRepoPath( hostPath ) {
	...
	const containerRepo =
		...
		defaultContainerRepoDir();
	return path.posix.join( containerRepo, ...relative.split( path.sep ) );
}

function wpEnvCwd() {
	return (
		process.env.RTC_PROTOCOL_SERVER_FUZZ_WP_ENV_CWD ?? DEFAULT_WP_ENV_CWD
	);
}

function defaultContainerRepoDir() {
	const cwd = wpEnvCwd();
	if ( cwd.startsWith( '/' ) ) {
		return cwd;
	}
	return path.posix.join( '/var/www/html', cwd );
}
...
function appendStatus( config, record ) {
	const fields = [
		...
		`failureHash=${ record.failureHash ?? '' }`,
		`failureClass=${ statusValue( record.failureClass ) }`,
		`failureMessage=${ statusValue( record.failureMessage ) }`,
...
function statusValue( value, maxLength = 240 ) {
	return String( value ?? '' )
		.replace( /[\t\r\n]+/gu, ' ' )
		.replace( /\s+/gu, ' ' )
		.trim()
...
function caseIndexFromTrace( file, caseCount ) {
	...
	for ( let index = lines.length - 1; index >= 0; index-- ) {
		try {
			...
			if (
				Number.isInteger( record.caseIndex ) &&
				record.caseIndex >= 0 &&
				record.caseIndex < caseCount
			) {
				return record.caseIndex;
			}
		} catch {
			continue;
		}
...
function looksLikeEnvironmentFailure( text ) {
	return /wp-env|docker|container|RequestError|AggregateError|ETIMEDOUT|Cannot connect|ECONNREFUSED|Environment not initialized|vendor\/autoload|No such file|not found|Permission denied|EACCES|EROFS|protocol\/server fuzz artifact/i.test(
		text
	);
}
...
function classifyAttempt( result, summary, output ) {
	if ( summary?.ok === true ) {
		if ( result.error || result.status !== 0 || result.signal ) {
			return {
				classification: 'wrapper-exit-after-ok-summary',
				failureKind: 'wrapper-exit-after-ok-summary',
				ok: true,
				wrapperExitAfterOkSummary: true,
			};
		}
		...
		return {
			...
			failureKind: null,
			ok: true,
			wrapperExitAfterOkSummary: false,
		};
	}
	...
	if ( summary.ok === false ) {
		const summaryFailure = [
			...
			.join( '\n' );
		const looksInfra =
			/factory|wptests_|database table|bootstrap/i.test(
				summaryFailure
...
function buildCommand( seed, caseCount, caseStart, seedDir, tablePrefix ) {
	const phpunitBin =
	...
	const phpEnv = [
		...
		`RTC_PROTOCOL_SERVER_FUZZ_CASE_START=${ caseStart }`,
		`RTC_PROTOCOL_SERVER_FUZZ_TRACE_DIR=${ seedDir }`,
		`RTC_PROTOCOL_SERVER_FUZZ_SUMMARY_DIR=${ seedDir }`,
		`RTC_PROTOCOL_SERVER_FUZZ_PHPUNIT_BIN=${ phpunitBin }`,
		`RTC_PROTOCOL_SERVER_FUZZ_WP_ENV_BIN=${ wpEnvBin }`,
		`RTC_PROTOCOL_SERVER_FUZZ_WP_ENV_CONFIG=${ wpEnvConfig }`,
		`RTC_PROTOCOL_SERVER_FUZZ_WP_ENV_CWD=${ wpEnvCwd() }`,
	];
	if ( tablePrefix ) {
...
function replayEnv( config ) {
	const env = {};
	const wpEnvHome =
		process.env.RTC_PROTOCOL_SERVER_FUZZ_WP_ENV_HOME ??
	...
	if ( config.wpEnvPort ) {
		env.WP_ENV_PORT = String( config.wpEnvPort );
	}

	return env;
}

function writeReplayDescriptor( config, seed, command, replayPath ) {
	const descriptor = {
...
function runSeed( config, seed ) {
	...
	fs.rmSync( seedDir, { force: true, recursive: true } );
	fs.rmSync( phpSeedDir, { force: true, recursive: true } );
	mkdirp( seedDir );
	mkdirp( phpSeedDir );
	...
	writeReplayDescriptor( config, seed, command, replayPath );
	const traceCaseIndex = caseIndexFromTrace( tracePath, config.caseCount );
	...
	return {
		...
		wrapperExitAfterOkSummary:
			classification.wrapperExitAfterOkSummary ?? false,
	};
}
...
function attemptExecutionCount( attempt, caseCount ) {
	const caseIndex = attempt.summary?.caseIndex ?? attempt.traceCaseIndex;
	if (
		! attempt.ok &&
...
function installTerminalSignalHandlers( config, getState, writeTerminalEvent ) {
	for ( const signal of Object.keys( SIGNAL_EXIT_CODES ) ) {
		process.once( signal, () => {
			const stopReason = `interrupted by ${ signal }`;
			writeTerminalEvent( 'runner-stop', {
				interrupted: true,
				signal,
				stopReason,
			} );
...
function main() {
	...
	const caseStart = optionInt(
		options,
		'caseStart',
		'RTC_PROTOCOL_SERVER_FUZZ_CASE_START',
		0,
		0
	);
	...
	const writeTerminalEvent = ( kind, extra = {} ) => {
	...
	try {
		for ( let offset = 0; offset < seedCount; offset++ ) {
			...
			const inferredCaseIndex =
				attempt.summary?.caseIndex ?? attempt.traceCaseIndex ?? null;
			const executionCount = attemptExecutionCount( attempt, caseCount );
			cumulativeTestExecutionCount += executionCount;
			const completeEvent = baseEvent( config, 'seed-attempt-complete', {
				caseIndex: inferredCaseIndex,
				caseCount,
				...
				failureClass: attempt.summary?.failure?.class ?? null,
				...
				wrapperExitAfterOkSummary: attempt.wrapperExitAfterOkSummary,
			} );
		...
		writeTerminalEvent( 'runner-error', {
