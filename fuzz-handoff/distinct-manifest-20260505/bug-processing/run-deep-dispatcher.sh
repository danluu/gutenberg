...
repeat_candidates="$root/confirmed-real-candidates.tsv"
...
generate_repeat_candidates() {
	...
	while IFS=$'\t' read -r sig transport bug_type spec_path result_file; do
		[ -n "$sig" ] || continue
		previous_summary="$(latest_summary_for "$pass" "$sig")"
		[ -n "$previous_summary" ] || previous_summary="$state_dir/$sig.summary.md"
		printf '%s\t%s\t%s\t%s\t%s\t%s\n' "$sig" "$transport" "$bug_type" "$spec_path" "$result_file" "$previous_summary" >> "$candidates"
	done < "$repeat_candidates"
	touch "$dir/initialized"
	log "pass=$pass seeded $(wc -l < "$candidates" | tr -d ' ') confirmed-real candidates for another deep pass"
}
...
while :; do
	...
	if [ ! -e "$dir/initialized" ]; then
		...
		if [ "$REAL_WORLD_LIKELIHOOD_MODE" = "1" ] && [ -f "$repeat_candidates" ]; then
			log "pass=$current_pass real-user-likelihood mode active; seeding confirmed-real set"
	...
	if [ -f "$dir/candidates.tsv" ] && [ ! -s "$dir/candidates.tsv" ]; then
		if [ -f "$repeat_candidates" ]; then
			log "pass=$current_pass has no likely/unresolved candidates; seeding pass=$(( current_pass + 1 )) from confirmed-real set"
