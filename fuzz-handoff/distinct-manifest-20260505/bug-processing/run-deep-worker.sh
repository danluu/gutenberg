...
This is deep-analysis pass $pass for a likely or unresolved RTC fuzz bug.
...
Run a deeper independent analysis than the previous pass. Because this bug has already had many deep-analysis passes, prioritize practical impact: analyze and report the likelihood that a real user would hit this issue in normal Gutenberg use.
...
Re-read the previous summary, source result, logs, screenshots/traces, and relevant code. Try hard to disprove the bug first: distinguish product defects from readiness waits, action locator errors, malformed generated specs, environment failures, inverted assertions, and expected behavior. If it is not a real product bug, write \`$summary_file\` with the evidence and stop.
...
For pass $pass specifically, do not merely restate the previous pass. Add one of:
