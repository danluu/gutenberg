...
`pr-split-20260517T225642Z-synthesis.md`, moves the active recommendation from
the stale Cycle310 iteration-27 microhead view and the stale i28/i29 grouped
...
Review-shape changes since the prior report:

- Add `PR07B2` after `PR07B1`.
- Split the previously grouped i29 `PR06`, `PR11`, `PR12`, and `PR15` back
  into finer i30 filing units.
- Keep `PR09+` forked from `PR06D`, not serialized behind PR07.
- Clean `PR05D` is only `27c6e7924217...`; reject fallback/PR15-tail
  `PR05D`, including `fix/rtc-fallback-group-delete-stale-local` and
  `d06e3528cbd`.
- Reject raw `PR07D`, `PR17`, `PR18`, `PR18x`, `ready/*`,
  `try/rtc-fix-stack-validation`, raw deferred refs, Cycle293/Cycle306 rows,
  and stale i28/i29/finalization publication artifacts as active filing
  sources.
...
- i30 lacks a fresh non-Docker branch audit and manifest. The required next
  bounded audit is `rtc-cycle312-i30-audit-manifest-and-stale-finalization-guard`
  or an equivalent current i30 artifact set.
- Seed `1020002` still blocks final-stack fuzz, filing, and rebuilt full-stack
  validation. It must not block independent i30 audit/manifest work, PR07
  readiness/root-cause work, deferred owner comparison, or loop repair.
...
- Stale, zero-byte, `report.tmp`, wrong-base, setup-only, runtime-preflight-only,
  or disk-preflight-only artifacts are not filing evidence.
- Many active i30 rows still lack verified GitHub branch links. Rows below say
...
diff; it does not remove the i30 audit, topology, PR07 runtime, seed-1020002,
...
| PR 5A | Entity/reference normalization equivalence | No verified branch link yet | TBD | TBD | active i30 row; old aggregate PR5 is prior art |
| PR 5B | Parser/rich-text HTML equivalence | No verified branch link yet | TBD | TBD | active i30 row |
| PR 5C | Preserve-whitespace linebreak equivalence | No verified branch link yet | TBD | TBD | active i30 row |
...
| PR 6A | First save request payload guard microhead | No verified branch link yet | TBD | TBD | active i30 row; old aggregate PR6 is prior art |
| PR 6B | Second save request payload guard microhead | No verified branch link yet | TBD | TBD | active i30 row |
| PR 6C | Third save request payload guard microhead | No verified branch link yet | TBD | TBD | active i30 row |
| PR 6D | Terminal save request payload guard microhead | No verified branch link yet | TBD | TBD | active i30 row; PR06D must be ancestor of PR06E and PR09 |
...
| PR | Scope | Audit branch link | Files | Diff | Current status |
| --- | --- | --- | ---: | ---: | --- |
| PR 7A1 | Save response entity-state guard microhead 1 | No verified branch link yet | TBD | TBD | active i30 row; runtime ownership still setup-only |
| PR 7A2 | Save response entity-state guard microhead 2 | No verified branch link yet | TBD | TBD | active i30 row; runtime ownership still setup-only |
| PR 7A3 | Save response entity-state guard microhead 3 | No verified branch link yet | TBD | TBD | active i30 row; runtime ownership still setup-only |
| PR 7B0 | Save response manager/base-record guard microhead 0 | No verified branch link yet | TBD | TBD | active i30 row; rerun owner matrix only after readiness is true |
| PR 7B1 | Save response manager/base-record guard microhead 1 | No verified branch link yet | TBD | TBD | active i30 row |
| PR 7B2 | Save response manager/base-record guard microhead 2 | No verified branch link yet | TBD | TBD | new i30 row after PR07B1 |
| PR 7C | Reload record snapshots sidecar after PR7B2 | No verified branch link yet | TBD | TBD | held; no raw PR07D without first-divergence evidence |

### Independent CRDT/Data-Loss Lane

| PR | Scope | Audit branch link | Files | Diff | Current status |
| --- | --- | --- | ---: | ---: | --- |
| PR 9 | Core-data lock fairness from PR06D | [`review/rtc-pr09-store-lock-fairness`](https://github.com/danluu/gutenberg/tree/review/rtc-pr09-store-lock-fairness) | 2 | +185 / -2 | verified content; i30 audit must prove PR06D ancestry and PR07 non-ancestry |
| PR 10 | CRDT block reconciliation foundation after PR9 | [`review/rtc-pr10-crdt-block-rebase-foundation`](https://github.com/danluu/gutenberg/tree/review/rtc-pr10-crdt-block-rebase-foundation) | 2 | +145 / -4 | verified content |
| PR 11A | Explicit-base top-level operation microhead A | No verified branch link yet | TBD | TBD | active i30 row; old aggregate PR11 is prior art |
| PR 11B | Explicit-base top-level operation microhead B | No verified branch link yet | TBD | TBD | active i30 row |
| PR 11C | Explicit-base top-level operation microhead C | No verified branch link yet | TBD | TBD | active i30 row |
| PR 11D | Explicit-base top-level operation microhead D | No verified branch link yet | TBD | TBD | active i30 row |
| PR 11E | Explicit-base top-level operation microhead E | No verified branch link yet | TBD | TBD | active i30 row |
| PR 12A | Previous-local-cache operation microhead A | No verified branch link yet | TBD | TBD | active i30 row; old aggregate PR12 is prior art |
| PR 12B | Previous-local-cache operation microhead B | No verified branch link yet | TBD | TBD | active i30 row |
| PR 12C | Previous-local-cache operation microhead C | No verified branch link yet | TBD | TBD | active i30 row |
| PR 13A | Observed-delete top-level provenance | [`review/rtc-pr13a-observed-delete-provenance-repaired`](https://github.com/danluu/gutenberg/tree/review/rtc-pr13a-observed-delete-provenance-repaired) | 2 | +1151 / -25 | repaired verified content |
| PR 13B0 | Identity/provenance guard microhead | No verified branch link yet | TBD | TBD | active i30 row; repaired PR13C is fallback/supporting evidence only |
| PR 13B1 | Direct cross-parent source retirement microhead | No verified branch link yet | TBD | TBD | active i30 row; repaired PR13B is aggregate fallback/supporting evidence only |
| PR 13B2 | Current-only cross-parent source retirement microhead | No verified branch link yet | TBD | TBD | active i30 row |
| PR 13B3 | Explicit-base cross-parent source retirement microhead | No verified branch link yet | TBD | TBD | active i30 row |
...
| PR 15A | Fallback group move stale reorder after PR14B | [`review/rtc-pr15a-fallback-group-move-stale-reorder`](https://github.com/danluu/gutenberg/tree/review/rtc-pr15a-fallback-group-move-stale-reorder) | 2 | +123 / -4 | verified content; i30 audit must confirm current PR14B ancestry |
| PR 15B | Fallback group insert anchor after PR15A | [`review/rtc-pr15b-fallback-group-insert-anchor`](https://github.com/danluu/gutenberg/tree/review/rtc-pr15b-fallback-group-insert-anchor) | 2 | +197 / -4 | verified content; i30 audit must confirm current placement |
| PR 15C | Fallback group delete after PR15B | [`review/rtc-pr15c-fallback-group-delete`](https://github.com/danluu/gutenberg/tree/review/rtc-pr15c-fallback-group-delete) | 2 | +161 / -4 | verified content; not a clean PR05D substitute |
...
| PR 6A prior art | Persisted empty-content CRDT body guard | [`review/rtc-pr06a-persisted-empty-content-guard`](https://github.com/danluu/gutenberg/tree/review/rtc-pr06a-persisted-empty-content-guard) | verified prior art; do not substitute for active PR06A-D without i30 audit mapping |
...
suppressed strict startup records in active current-run scope: 0
unmet goals: 5
quality issues: 0
health: ok
enabled groups:
...
no-bugs claim. Active current-run and drain triage are clean at this pass. The
new run root was reset at `2026-05-17T23:01:12Z`; the earlier startup-noise
hold and paused/no-analysis drain state are scheduling/control-plane evidence,
not product filing evidence.

Coverage guidance still has five unmet auto-ratchet goals:

```text
reload-post-action: 1079/2000
title-save-reload: 535/1000
body-save-reload: 594/1000
real-user-editing success: 600/1000
ui-format-paragraph: 1698/2000
```

The latest trend packet was generated at `2026-05-17T23:00:23Z` from monitor
data through `2026-05-17T22:56:29Z`:

```text
monitor passes: 2139
coverage files: 272 -> 48118
coverage files delta: 47846
unmet goals: 5
likely_real_max: 4
...
total fuzz-level test executions: 5381655
browser-e2e likely-real findings: 622 over 1963.3 runner-hours
...
`pr-split-20260517T225642Z-synthesis.md`. It confirms:

- The current filing hypothesis is `fresh-prset/iteration-30/*`.
- Add `PR07B2` after `PR07B1`.
- Keep `PR09+` forked from `PR06D`, not serialized behind PR07.
- Reject grouped i28/i29 as filing shape unless a later fresh audit/review
  explicitly accepts that larger granularity.
- Reject raw `PR07D`, `PR17`, `PR18`, `PR18x`, `ready/*`,
  `try/rtc-fix-stack-validation`, raw deferred refs, fallback-tail `PR05D`,
  and stale publication/finalization artifacts.
...
- Filing, broad final-stack fuzzing, and stack-wide validation remain blocked
  by missing i30 audit/manifest evidence, PR07 runtime ownership, and the
  seed-1020002 final gate.
...
while preserving product evidence. The remaining consensus root cause was a
consumer-scope leak where paused/no-analysis drain dirs could still be treated
by live-analysis as partly live.

The matching duplicate/noise feedback action,
`duplicate-noise-20260517T224618Z-feedback-action.md`, completed that narrow
fix in the remote fuzz repo:
...
- One patched live-monitor pass showed `no-analysis-drain` as
  `skipped-analysis-gate-only-drain` even with product-evidence actionable
  work.
- Only the affected control-plane sessions were restarted:
  `rtc-coverage-guided-supervisor` and `rtc-coverage-guided-live-analysis`.
- The active root is now
  `/media/volume/danluu-fuzz-data/rtc-coverage-guided-20260515/run-20260517T230103Z`.
...
PR06B" recommendations are superseded by the repaired PR13 audit links, the i30
...
| --- | --- | --- | --- |
| i30 split audit | `fresh-prset/iteration-30/*` | active working target, but not audited as a filing manifest | Run bounded fresh audit/manifest newer than `latest-fresh-pr-set.md`, stale finalization artifacts, and the deferred queue |
| Required i30 artifacts | `report.md`, `push-manifest.tsv`, `manifest-age.tsv`, `base-allowlist.tsv`, `head-bundle-manifest-check.tsv`, `branch-graph.txt`, adjacent diffstat/numstat, patch-id or range-diff vs i27/i29/Cycle306, deferred-output audit, artifact verification | missing for the new i30 recommendation | Produce nonempty allowlisted artifacts; reject zero-byte or stale reports |
| Missing verified product refs | PR02A, PR05A-D, PR06A-E, PR07A1-A3, PR07B0-B2, PR07C, PR11A-E, PR12A-C, PR13B0-B3, PR14B | rows correctly say `No verified branch link yet` | Publish/fetch/audit explicit GitHub refs before filing |
...
| PR14B / PR15A-C placement | table query-array suffix and fallback-group rows after PR14B | PR15A-C have verified links, but active topology still lacks verified current PR14B and i30 placement proof | Publish/fetch/audit explicit PR14B-based refs and confirm PR15A-C ancestry |
...
Cycle308, Cycle310, i28/i29 grouped aliases, validation-stack, dirty evidence,
fallback-tail branches, raw deferred refs, or zero-byte/stale finalization
artifacts as-is.

Before filing any maintainer-facing PR:

1. Use the i30 topology as the current working target.
2. Produce a fresh i30 audit/manifest with nonempty durable artifacts newer
...
4. Prove `PR06D -> PR06E`, `PR07 !-> PR06E`, `PR06D -> PR09`,
   `PR07 !-> PR09/PR15C`, `PR07B2` based on `PR07B1`, clean `PR05D` only, and
   no fallback-tail `PR05D`.
5. Until PR13B0/B1/B2/B3 have verified branch links, use only the repaired
   ...
   evidence.
6. Keep old aggregate PR5, aggregate PR6, aggregate PR7A/7B, aggregate PR11,
   aggregate PR12, broad PR8, dirty evidence branches, stale/misordered PR13
   refs, fallback-tail PR15/PR05D confusion, and the untracked reload-hydration
...
The next useful work is a bounded i30 audit/deferred refresh, then targeted
