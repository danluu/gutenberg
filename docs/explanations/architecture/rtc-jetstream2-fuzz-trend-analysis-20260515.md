...
Snapshot generated: `2026-05-18T02:47:25Z`
...
The coverage-guided loop is still expanding coverage. Across `2179` monitor
passes from `2026-05-15T01:21:42Z` through `2026-05-18T02:42:10Z`, coverage
files grew from `272` to `49176`, a delta of `48904`. The monitor's visible
...
`0`. The last twelve current-output duplicate-share samples are nine `0.0000`
samples, then `1.0000`, then two `0.0000` samples, so the graph shows a recent
clean state after a live duplicate/noise spike rather than durable recovery. The
latest pass has `0` quality issues, `0` warnings, `420.0G` free memory, and a
true monitor headroom flag. This report uses current-output duplicate/noise and
summary startup failures for live health. Historical aggregate duplicate/noise
is context only; its latest duplicate share is `0.3448` and is not the plotted
live health signal.

Persona-loop evidence is treated as evidence to evaluate, and it rejects reading
the clean latest graph point as durable recovery. The latest duplicate/noise
...
`20260518T022917Z`, says the grouped i40 split should be replaced by the
Cycle324 ungrouped `finalized/cycle324-i40/*` shape. The matching
feedback-action updated the current split and completed the non-Docker ungrouped
manifest/audit refresh with 43 manifest rows and bundle `PASS`, but filing and
final-stack validation remain blocked by PR07 owner evidence, root disk pressure
(`/` at `712 MB` free versus the `2048 MB` replay threshold), stale publish
manifest evidence, and final-stack-only seed `1020002`. The latest synthesis
also rejects the zero-byte `20260518T023218Z/finalization.report.md` as
evidence.
...
`420.0G`, and headroom true. The last twelve current-output duplicate-share
samples are nine `0.0000` samples, then `1.0000`, then two `0.0000` samples.
The latest graph therefore shows clean current-output status after a recent live
duplicate/noise regression, not sustained recovery. The health graph does not
...
and `novelty-ws-parser-transform` as enabled groups. Current-output duplicate
share is `0.0000` and current summary startup failures are `0`. The latest
duplicate/noise synthesis rejects treating the clean sample as a durable
all-clear: strict startup records are no longer the live current-root hotspot,
but producer/scheduler startup handling and product-evidence duplicate-family
caps still need to move earlier.

![CPU utilization over time](rtc-jetstream2-fuzz-trends-20260515/plots/cpu-utilization-over-time.png)

![Load average over time](rtc-jetstream2-fuzz-trends-20260515/plots/load-average-over-time.png)

Recent sysstat samples through `2026-05-18T02:40:00Z` show bursty CPU. The
latest 25 CPU samples range from `58.1%` to `84.1%` utilization, with the latest
sample at `67.1%`. Over those same 25 samples, one-minute, five-minute, and
...
is `61.37`, `57.97`, and `62.19`; all three windows are below the logical CPU
count.
...
`novelty-ws-multi-reload-lifecycle` and `novelty-ws-parser-transform`.
...
contradicts it. The latest non-empty level-mix synthesis, `20260518T022320Z`,
says the trusted live mix should be treated as `browser-e2e=24-25 fragile`,
`coverage-guided-lower-level=3`, `unit-property=1`, and `protocol-server=0`,
`fuzz-assertion=0`, `backend-api=0`. It rejects counting protocol action
validation roots as active protocol fuzzing and says protocol must fail closed
until an exact tmux session, fresh current-run root, `status.tsv`, and
`events.ndjson` all reconcile. The latest level-mix feedback-action reports
live browser PID samples of `25`, `25`, and `25` with no final telemetry
invariant failure, but it still says the protocol sentinel stopped after three
infra failures from `Call to undefined function gutenberg_override_style()`.
...
latest non-empty native action, `20260518T021000Z`, adopted the rich-text CRDT
lower-level harness and emitted `fuzzLevel: "coverage-guided-lower-level"`
events for `2` smoke inputs. The latest protocol-server synthesis,
`20260518T023647Z`, still selects the HTTP polling REST endpoint. The latest
non-empty protocol action, `20260518T021124Z`, validated the protocol/server
harness with `2` seeds, `13` cases each, and isolated storage PHPUnit passing
...
The latest collected execution data has about `5,485,962` completed test
executions: `140,765` browser/e2e, `3,006` transport/integration, `4,894,368`
unit-property, and `447,823` coverage-guided-lower-level. The latest partial
15-minute bucket reports about `68` browser/e2e test executions/hour, `1,536`
...
On that triage-output metric, browser/e2e currently dominates: `659` unique
likely-real findings over about `2,023.5` runner-hours, or `32.57` per 100
...
Current unique bug-output candidate rates are: browser/e2e `5,560` candidates
over `2,023.5` runner-hours (`274.77` per 100 runner-hours),
transport/integration `106` over `59.5` runner-hours (`178.14` per 100
runner-hours), coverage-guided lower-level `2` over `20.9` runner-hours
(`9.55` per 100 runner-hours), and unit/property `5` over `29.6` runner-hours
(`16.87` per 100 runner-hours). `backend-api`, `protocol-server`, standalone
...
The broader unique-output candidate view is led by three-user late join (`757`),
session lifecycle (`692`), real-user editing (`660`), revision persistence
...
are about `5787.3` for browser/e2e, `4537.5` for transport/integration,
`759.3` for coverage-guided lower-level, and `2422.9` for unit/property.
...
| --- | ---: | ---: | ---: | ---: |
| `full` | 840 | 18 | 0 | 2.1% |
| `multi-reload-lifecycle` | 3446 | 135 | 0 | 3.9% |
| `revision-persistence` | 4827 | 191 | 0 | 4.0% |
| `parser-serialization` | 3534 | 212 | 0 | 6.0% |
| `real-user-editing` | 7628 | 602 | 0 | 7.9% |
| `parser-transform` | 4433 | 480 | 0 | 10.8% |
| `common-blocks` | 4305 | 466 | 0 | 10.8% |
| `structure` | 536 | 95 | 0 | 17.7% |
| `long-session-large-doc` | 3110 | 578 | 0 | 18.6% |
| `block-gauntlet` | 6441 | 1212 | 0 | 18.8% |
| `persistence-no-title` | 3509 | 1008 | 0 | 28.7% |
| `session-lifecycle` | 8527 | 2578 | 0 | 30.2% |
...
review cycle, `20260518T022917Z`, took `11.8` minutes from
`2026-05-18T02:29:17Z` to `2026-05-18T02:41:03Z`. The latest synthesis rejects a
...
and `420.0G` free memory. The latest 1/5/15-minute load windows are `61.37`,
`57.97`, and `62.19`; all three are below the `64` logical CPU count.
...
a durable all-clear. The latest synthesis says strict no-product startup rows
are mostly blocked at analysis consumers, but producer/scheduler lag can still
...
duplicate/noise readout; it is current-root triage after producer/scheduler
sentinels and representative caps land.

The PR-split persona loop rejects filing or broad final-stack fuzzing. The
latest synthesis says the grouped i40 topology should be replaced by the
Cycle324 ungrouped shape from `finalized/cycle324-i40/*`. Filing remains blocked
by PR07 owner evidence, root disk below the replay threshold, stale
publish-manifest evidence, final-stack-only seed `1020002`, and rebuilt
validation. The latest synthesis also rejects the zero-byte finalization report
as evidence.
...
executions. The latest level-mix synthesis rejects counting protocol validation
roots as active fuzzing, and the latest feedback-action says the protocol
sentinel stopped on the `gutenberg_override_style()` infra bootstrap failure.
The report therefore treats protocol/backend/assertion work as harness/action
evidence until
