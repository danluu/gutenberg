...
Snapshot generated: `2026-05-19T13:15:16Z`
...
last completed monitor pass is rooted at `run-20260519T131133Z`, and the
repaired passes continue through `2026-05-19T13:12:51Z`. Across `2427` monitor
passes from `2026-05-15T01:21:42Z` through `2026-05-19T13:12:51Z`, cumulative
coverage record observations rose from `782` to `252974`. Coverage-file counts
are current-scan counts, not cumulative coverage; the latest current scan is
`1498` files after the latest root rollover. The monitor's visible likely-real
maximum remains `4`.
...
`no_progress=1`, and `411.4G` free memory. The latest monitor pass has
`headroom=true`. The newest load sample is below the `64` logical CPU count, but
recent load still exceeded it in `9` of the latest `25` windows, so the clean
current duplicate/noise sample should not be read as a progress or capacity
claim.
Historical aggregate duplicate/noise is context only; its latest duplicate
share is `0.25` and is not the plotted live health signal.

The copied novelty state has advanced to `run-20260519T131133Z` with no enabled
groups, `8` paused groups, no active current run dirs, one current dir only in
the paused/no-analysis-inclusive view, and empty current summary startup-failure
maps. The current-run triage view sees `0` roots and `0` signatures. The
external-live view sees one root, `10` files, `94` signatures, and `55` merged
likely-real signatures. Useful live materialization is not visible in the copied
current state.

Persona-loop evidence rejects several graph-only interpretations. The latest
duplicate/noise synthesis, `20260519T125720Z`, still finds a control-plane
problem in novelty producer admission and startup cooldown scoping, plus
structured product-evidence metadata loss in sticky/no-analysis sentinels. The
latest duplicate/noise feedback action, `20260519T122914Z`, tightened
product-evidence duplicate holds, made productless fleet startup-noise holds
block producer selection, and restarted the control plane; the earlier
`20260519T113555Z` action had already tightened family occupancy,
no-analysis seed drains, and live-analysis launch scope. The refreshed graph
shows a clean current duplicate/startup sample, but persona evidence rejects
treating that as durable healthy materialization while useful producers are
still absent from the copied current state.
...
`36` browser/e2e lanes across `24` groups, plus `1` `unit-property` lane and
`1` `coverage-guided-lower-level` lane. The latest level-mix synthesis,
`20260519T124623Z`, explicitly says to repair browser/e2e capacity first, not
to add lower-level lanes. It reports browser/e2e below the `24` useful-lane
floor, with live counts ranging from `6` to `15` and context/accounting around
`16-18`. Its matching feedback file edited nothing and recommends validating
useful browser lanes twice, then running admission-gated `focused-shards`
backfill only if the count remains below `24`.

Native/protocol persona evidence shows lower-level work exists outside some
committed counters. The latest native-harness synthesis, `20260519T130051Z`,
selects the isolated rich-text CRDT multiblock harness as the first ready
coverage-guided lower-level target; its newest action file is empty, so the
latest implemented native action remains `20260519T123618Z`, which validated a
V8/Node smoke with `fuzzLevel: "coverage-guided-lower-level"` and `2`
completed inputs while leaving the existing block-parser lower-level lane
running. The latest protocol-server synthesis, `20260519T130422Z`, selects the
HTTP polling REST harness; its newest action file is empty, so the latest
implemented protocol action remains `20260519T123640Z`, which validated
`POST /wp-sync/v1/updates` with one passing seed, `20` individual protocol
cases, and `23` nonzero oracle counters. The committed `backend-api`,
...
`252974`; coverage-file counts are tracked separately and reset when the active
output root rolls. The latest current-output sample is `1498` files from
`run-20260519T131133Z`. Dense monitor-pass points are intentionally small and
...
`0`, warnings `0`, `no_progress=1`, `411.4G` free memory, and
`headroom=true`. Historical aggregate duplicate/noise is not the plotted live
health signal.

The copied current novelty state has `enabledGroups=[]`, no active current run
dirs, one paused/no-analysis-inclusive current dir, empty current summary
startup-failure maps, and a current-run triage view with `0` roots and `0`
signatures. The external-live triage view sees one root, `10` files, `94`
signatures, and `55` merged likely-real signatures. Useful live materialization
is not visible in the copied current state.

![CPU utilization over time](rtc-jetstream2-fuzz-trends-20260515/plots/cpu-utilization-over-time.png)

![Load average over time](rtc-jetstream2-fuzz-trends-20260515/plots/load-average-over-time.png)

Recent sysstat samples through `2026-05-19T13:10:00Z` show bursty CPU and load.
The latest 25 CPU samples range from `43.60%` to `81.01%` utilization, with the
latest sample at `61.83%`. Over those same 25 samples, one-minute load exceeded
...
newest 1/5/15-minute load sample is `33.70`, `43.01`, and `49.96`; all three
load windows are below the `64` logical CPU count. The newest sample has `5`
blocked tasks.
...
state has no enabled groups, `8` paused groups, empty current summary
...
shows `36` browser/e2e lanes across `24` groups, plus `1` `unit-property` lane
and `1` `coverage-guided-lower-level` lane.

Live graph-visible fuzzing is concentrated in browser/e2e lanes: `36` of the
latest `38` graph-visible lanes are browser/e2e. Lower-level work is active but
...
`20260519T124623Z`, says the effective mix should change by restoring
browser/e2e capacity first, not by adding lower-level lanes. It reports browser
live counts from `6` to `15`, with context/accounting around `16-18`, all below
the `24` useful-lane floor. It says to keep `unit-property`,
`coverage-guided-lower-level`, backend/API, and protocol at one lane, and to
count stale or held `fuzz-assertion` as `0` useful. Its matching feedback file
edited nothing; it recommends two live-lane validations and an admission-gated
`focused-shards` backfill only if useful browser lanes stay below `24`. This
rejects both a graph-only "add lower-level capacity now" interpretation and a
graph-only "`36` browser/e2e lanes means healthy useful capacity"
interpretation.

Native/protocol actions show useful work that is not yet reflected in the
committed backend/protocol/assertion counters. The newest native and protocol
action files, `20260519T130051Z` and `20260519T130422Z`, are empty. The latest
implemented native action, `20260519T123618Z`, validated the isolated rich-text
CRDT coverage-guided-lower-level smoke with `2` completed inputs and left the
parser lower-level lane running. The latest implemented protocol action,
`20260519T123640Z`, validated the HTTP polling REST harness with `20`
individual protocol cases and `23` nonzero oracle counters. The latest native
synthesis, `20260519T130051Z`, keeps the rich-text CRDT multiblock harness as
the first ready coverage-guided lower-level target; the latest protocol
synthesis, `20260519T130422Z`, keeps the HTTP polling REST harness as the first
ready protocol-server target. The latest fuzz-only assertion action,
`20260519T034502Z`, added two gated browser/e2e assertions and restarted
affected loops, while the level-mix feedback action says stale/held
...
The latest collected execution data has about `6,237,307` completed test
executions: `381,838` browser/e2e, `3,006` transport/integration,
`5,404,640` unit-property, and `447,823` coverage-guided-lower-level. The
latest partial 15-minute bucket reports about `4,216` browser/e2e executions/hour
and `5,376` unit-property executions/hour, with `0` current counted rate for
...
On that triage-output metric, browser/e2e currently dominates: `819` unique
likely-real findings over about `2804.9` runner-hours, or `29.20` per 100
...
Current unique bug-output candidate rates are: browser/e2e `6,395` candidates
over `2804.9` runner-hours (`227.99` per 100 runner-hours),
transport/integration `106` over `59.5` runner-hours (`178.14` per 100
runner-hours), coverage-guided lower-level `2` over `20.9` runner-hours
(`9.55` per 100 runner-hours), and unit/property `6` over `62.6` runner-hours
(`9.58` per 100 runner-hours). `backend-api`, `protocol-server`, standalone
...
are about `12647.4` for browser/e2e, `4537.5` for transport/integration,
`759.3` for coverage-guided lower-level, and `2206.1` for unit/property.
...
| --- | ---: | ---: | ---: | ---: |
| `full` | 1047 | 34 | 0 | 3.2% |
| `multi-reload-lifecycle` | 4506 | 163 | 0 | 3.6% |
| `revision-persistence` | 7738 | 289 | 0 | 3.7% |
| `parser-serialization` | 4648 | 283 | 0 | 6.1% |
| `real-user-editing` | 10212 | 873 | 0 | 8.5% |
| `common-blocks` | 5345 | 512 | 0 | 9.6% |
| `parser-transform` | 6023 | 657 | 0 | 10.9% |
| `long-session-large-doc` | 5253 | 703 | 0 | 13.4% |
| `block-gauntlet` | 7750 | 1366 | 0 | 17.6% |
| `structure` | 541 | 96 | 0 | 17.7% |
| `session-lifecycle` | 10020 | 2757 | 0 | 27.5% |
| `media-cross-entity` | 566 | 171 | 0 | 30.2% |
| `three-user-late-join` | 12552 | 3961 | 0 | 31.6% |
| `permissions-auth-locks` | 10280 | 4337 | 0 | 42.2% |
| `persistence-no-title` | 4837 | 2103 | 0 | 43.5% |
| `async-server-blocks` | 11393 | 4966 | 0 | 43.6% |
...
It also shows quality issues `0`, warnings `0`, `no_progress=1`, and `411.4G`
free memory. The same latest pass has `headroom=true`, and the newest load
sample is below the `64` logical CPU count, but recent load still exceeded it in
`9` of the latest `25` windows. Clean startup/noise status is not the same as
healthy useful capacity, especially because the copied novelty state has no
active current run dirs. Historical aggregate duplicate/noise is `0.25`,
...
claim. The latest synthesis, `20260519T125720Z`, says the remaining problem is
mis-scoped novelty producer admission/cooldown logic plus sentinel metadata
loss for product-evidence no-analysis cases. The latest feedback action at
`20260519T122914Z` tightened product-evidence holds and productless startup
blocking, while `20260519T113555Z` repaired family occupancy, no-analysis
drains, and live-analysis launch scope. The graph should still be read as
current duplicate/startup cleanliness plus absent copied current
materialization, not as proof that producer selection is stable under load.
...
The committed fuzzing graph is browser/e2e-heavy: `36` current browser/e2e
...
useful browser/e2e capacity is below the `24`-lane floor and rejects lower-level
expansion now. Its matching feedback file edited nothing and recommends
admission-gated `focused-shards` backfill only after two live-lane checks still
show the browser floor is missed. Lower-level work is real but narrow:
...
and gated assertion work exist. The newest native/protocol action files are
empty, but the latest implemented protocol action validated `20` REST protocol
cases with nonzero oracle counters, and the latest implemented native action
validated a `2`-input coverage-guided lower-level smoke. The graph-only "none
