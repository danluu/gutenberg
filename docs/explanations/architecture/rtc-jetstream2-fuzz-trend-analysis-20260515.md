...
Snapshot generated: `2026-05-22T05:08:44Z`
...
current-run accounting row was sampled at `2026-05-22T05:07:27Z` for
`run-20260522T045104Z`. That row is trusted:
`current_run_metrics_trusted` is `TRUE`, `pending_until_first_pass` is `FALSE`,
and `full_pass_pending` is `FALSE`. The monitor has `3,919` passes from
...
trusted row has `duplicateShareCurrent` `0.5`, summary startup failures `0`,
and current-run signature/actionable-signature/product-evidence denominators
`2`/`2`/`2`. That means one of two current actionable/product-evidence
signatures is the top duplicate, not a broad duplicate storm. Historical
duplicate share is `0.2143` for context only; it is not the plotted live
health signal.
...
trusted row supports the narrow control-plane reading rather than a broad
...
`/` and `377.7GiB` free on `/media/volume/danluu-fuzz-data`. Latest CPU
...
The execution counter has `16,621,373` estimated individual executions. These
...
at `2026-05-22T05:00:00Z` is partial and has `46` browser/e2e executions,
`320` unit-property executions, and `25` protocol-server executions, about
`184`/hour, `1,280`/hour, and `100`/hour. Coverage-guided lower-level,
backend-api, transport-integration, and fuzz-assertion are zero in that bucket.
The preceding `2026-05-22T04:45:00Z` bucket had `59` browser/e2e executions,
`704` unit-property executions, and no protocol-server executions.
...
blockers: `2` runnable blockers, `1` active blocker, `2` queued blockers, and
...
`run-20260522T045104Z` was taken at `2026-05-22T05:07:27Z` with
`current_run_metrics_trusted` `TRUE`, `pending_until_first_pass` `FALSE`, and
`full_pass_pending` `FALSE`. The latest row carries current-output-dir
...
That makes the live reading one duplicate top signature out of two current
actionable/product-evidence signatures, not a broad duplicate storm.
...
newest active row is trusted and small-denominator, so it does not confirm a
...
around `89.3%` used and root is around `40.8%` used. Root pressure remains
...
enabled-group events refreshed HTTP existing-post CRDT metadata,
persistence-probe, large-post lifecycle, and large-post lifecycle completion
at `2026-05-22T04:48:39Z`.
...
permissions, and long-session coverage. The graph-counted lower-level targets
are table-query-array CRDT and the old rich-text CRDT smoke row; HTTP polling
...
protocol-server HTTP polling sentinel. It has `25` executions in the latest
`2026-05-22T05:00:00Z` bucket after a zero `04:45` bucket and `25` executions
in the `04:30` bucket, so long-lived protocol residency remains
pressure-sensitive even though the latest smoke is represented in graph
telemetry.
...
legacy batch-count fields. The latest totals are approximately `75,850`
browser/e2e, `5,632,448` unit-property, `458,097` coverage-guided lower-level,
...
The latest 15-minute bucket at `2026-05-22T05:00:00Z` is partial and has `46`
browser/e2e executions (`184`/hour), `320` unit-property executions
(`1,280`/hour), and `25` protocol-server executions (`100`/hour).
Coverage-guided lower-level, backend-api, transport-integration, and
fuzz-assertion are zero in that bucket. The preceding `2026-05-22T04:45:00Z`
bucket had `59` browser/e2e executions (`236`/hour), `704` unit-property
executions (`2,816`/hour), and no protocol-server executions. The latest
...
triaged likely-real output is still all browser/e2e: `215` likely-real findings
over about `869.2` runner-hours, or `24.73` per 100 runner-hours.
...
lower-level assertion failures. Current unique candidate output is `1,950`
...
(`productive-analysis-action` and `seed-5200005-reducer`), one active blocker
...
persona-loop outputs. The active coverage root is `run-20260522T045104Z`, and
its latest current-run duplicate/noise accounting is trusted. The live row was
sampled at `2026-05-22T05:07:27Z` with current-output-dir
`duplicateShareCurrent` `0.5`, summary startup failures `0`, and current-run
signature/actionable-signature/product-evidence denominators `2`/`2`/`2`.
Read that as a small current denominator, not a historical aggregate
...
protocol-server HTTP polling sentinel, with `25` executions in the latest
`2026-05-22T05:00:00Z` bucket. Protocol telemetry is present, but long-lived
...
`2026-05-22T05:00:00Z` bucket has `46` browser/e2e executions, `320`
unit-property executions, `25` protocol-server executions, and coverage-guided
lower-level at zero. The preceding `2026-05-22T04:45:00Z` bucket had about
`236` browser/e2e executions/hour and `2,816` unit-property executions/hour.
