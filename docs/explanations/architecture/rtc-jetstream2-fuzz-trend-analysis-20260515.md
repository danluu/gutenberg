...
Snapshot generated: `2026-05-18T01:12:31Z`
...
- latest copied novelty state:
  current coverage output-dir state through `2026-05-18T01:08:57Z`
...
The coverage-guided loop is still expanding coverage. Across `2166` monitor
passes from `2026-05-15T01:21:42Z` through `2026-05-18T01:08:57Z`, coverage
files grew from `272` to `48678`, a delta of `48406`. The monitor's visible
...
`415.3G` free memory, and a true monitor headroom flag. This report uses current-output-dir
...
`415.3G`, and headroom true. The last eight current-output duplicate-share
samples are `0.5000`, `0.5000`, `0.6000`, `0.5000`, `0.5000`, `0.0000`,
...
Recent sysstat samples through `2026-05-18T01:10:00Z` show bursty CPU. The
latest 25 CPU samples range from `55.4%` to `84.1%` utilization, with the latest
sample at `65.6%`. Over those same 25 samples, one-minute, five-minute, and
...
is `75.66`, `62.07`, and `60.90`; the one-minute window is above the logical
CPU count, while the five- and 15-minute windows are below it.
...
contradicts it. The latest level-mix synthesis, `20260518T005407Z`, rejects
treating requested browser/e2e lane metadata as stable live health by itself:
its context saw the graph/status report mark browser/e2e as okay while live
browser PID checks found only `4/24` current browser lanes. It recommends
repairing focused/strict/gap browser materialization first, keeping
`unit-property` capped, keeping `protocol-server` at one audited lane, and not
adding lower-level capacity. The latest non-empty feedback-action is older and
said a later context had reached `25` live browser PIDs, started one audited
protocol/server lane, and replaced a duplicate rich-text lower-level lane with a
multiblock oracle. The refreshed graph still has `0` protocol-server executions
and one visible `coverage-guided-lower-level` lane, still named
`coverage-guided-lower-level-rich-text-crdt`, so this report treats the lane mix
as supervisor metadata until live PID and collector-visible throughput checks
agree.
...
events for that harness. The latest protocol-server synthesis file is empty;
the latest non-empty protocol synthesis and action validate a bounded HTTP
polling REST smoke with `fuzzLevel: "protocol-server"` events but report no
long-lived protocol/server tmux session from that action. The latest fuzz-only
assertion apply file added diagnostic browser collaboration assertions and
restarted browser coverage-guided and strict-expansion fuzzing. These are
...
The latest collected execution data has about `5,448,572` completed test
executions: `133,391` browser/e2e, `3,006` transport/integration, `4,864,352`
unit-property, and `447,823` coverage-guided-lower-level. The latest partial
15-minute bucket reports about `1,240` browser/e2e test executions/hour,
...
likely-real findings over about `1,991.7` runner-hours, or `32.39` per 100
...
Current unique bug-output candidate rates are: browser/e2e `5,502` candidates
over `1,991.7` runner-hours (`276.25` per 100 runner-hours),
transport/integration `106` over `59.5` runner-hours (`178.14` per 100
runner-hours), coverage-guided lower-level `2` over `20.9` runner-hours
(`9.55` per 100 runner-hours), and unit/property `5` over `28.1` runner-hours
(`17.82` per 100 runner-hours). `backend-api`, `protocol-server`, standalone
...
(`654`), revision persistence (`449`), and permissions/auth/locks (`438`), with
...
are about `5520.3` for browser/e2e, `4537.5` for transport/integration,
`759.3` for coverage-guided lower-level, and `2558.2` for unit/property.
...
| --- | ---: | ---: | ---: | ---: |
| `full` | 840 | 18 | 0 | 2.1% |
| `multi-reload-lifecycle` | 3386 | 126 | 0 | 3.7% |
| `revision-persistence` | 4753 | 184 | 0 | 3.9% |
| `parser-serialization` | 3521 | 212 | 0 | 6.0% |
| `real-user-editing` | 7527 | 602 | 0 | 8.0% |
| `parser-transform` | 4346 | 456 | 0 | 10.5% |
| `common-blocks` | 4252 | 458 | 0 | 10.8% |
| `structure` | 536 | 95 | 0 | 17.7% |
| `block-gauntlet` | 6374 | 1190 | 0 | 18.7% |
| `long-session-large-doc` | 3052 | 578 | 0 | 18.9% |
| `persistence-no-title` | 3457 | 960 | 0 | 27.8% |
| `session-lifecycle` | 8427 | 2541 | 0 | 30.2% |
...
| action ui-format-paragraph next 2000 tier | 1753 | 2000 |
...
issues `1`, warnings `1`, and `415.3G` free memory. The latest 1/5/15-minute
load windows are `75.66`, `62.07`, and `60.90`, with only the one-minute window
...
deferred and the synthesis still rejects filing-ready status. Filing remains
blocked by fresh i40 audit evidence, PR07 owner replay, final-stack fuzz, and
rebuilt validation.
...
requested lane metadata alone because live browser PID accounting contradicted
the status report. The committed graph still shows `0` protocol-server
executions and no standalone `fuzz-assertion` executions, so the report treats
those as harness/action evidence until collector-visible counts appear. The next
narrow checks are producer-side duplicate/noise stability, live browser
materialization, collector-visible lower-level/protocol/assertion counts, fresh
i40 audit evidence, and PR07 replay evidence before any filing or final-stack
claim.
