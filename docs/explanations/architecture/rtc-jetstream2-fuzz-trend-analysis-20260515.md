...
Snapshot generated: `2026-05-16T16:45:47Z`
...
`1598` monitor passes from `2026-05-15T01:21:42Z` through
`2026-05-16T16:43:46Z`, coverage files grew from `272` to `34193`, a delta of
`33921`. The monitor's visible likely-real count stayed at `0`.
...
duplicate/noise: `duplicateShareCurrent` is `0.5`, summary startup failures are
`0`, warnings are `0`, free memory is `429.6G`, the monitor headroom flag is
true, and quality issues are `0`. The copied novelty state currently lists no
enabled coverage-guided groups after the latest output-dir reset. This report
...
The latest PR-split synthesis, `20260516T163625Z`, says the explicit 28-head
...
The latest PR-split feedback-action, `20260516T163625Z`, is empty. The latest
synthesis says the active repair report was still missing and its tmux session
was active; it launches no new jobs and keeps filing blocked.
...
`0` quality issues and `0` warnings, with `429.6G` free memory and the monitor
...
absent but current duplicate share present in the latest output directory, so
the persona-loop read keeps the system on probation.

![CPU utilization over time](rtc-jetstream2-fuzz-trends-20260515/plots/cpu-utilization-over-time.png)

![Load average over time](rtc-jetstream2-fuzz-trends-20260515/plots/load-average-over-time.png)

Recent sysstat samples through `2026-05-16T16:40:00Z` show sustained CPU
pressure: the 12:40-16:40 samples range from `46.2%` to `86.2%` utilization,
with the latest sample at `67.3%`. Load exceeded the logical CPU count in
sampled windows from 12:40 through 13:10, at 14:20, from 15:10 through 15:50,
and again at 16:10; the latest sampled 1/5/15-minute load is `59.39`, `57.46`,
and `59.37` against `64` logical CPUs.
...
`0` enabled coverage-guided groups after the latest output-dir reset. The
...
`duplicateShareCurrent=0.5`, so browser admission remains under probation until
...
latest coverage-guided supervisor snapshot and copied novelty state currently
show no enabled coverage-guided group after the latest reset.
...
coverage-guided output directory itself has no enabled groups after the latest
reset.
...
The latest collected execution data has about `813,528` completed test
executions: `52,600` browser/e2e, `3,006` transport/integration, `739,740`
unit-property, and `18,182` coverage-guided-lower-level. The latest 15-minute
bucket reports about `36` browser/e2e test executions/hour, `4,816`
unit-property test executions/hour, `64` coverage-guided-lower-level test
...
| --- | ---: | ---: | ---: | ---: |
| `full` | 840 | 18 | 0 | 2.1% |
| `revision-persistence` | 3481 | 89 | 0 | 2.6% |
| `multi-reload-lifecycle` | 2660 | 76 | 0 | 2.9% |
| `parser-serialization` | 2148 | 71 | 0 | 3.3% |
| `real-user-editing` | 5225 | 287 | 0 | 5.5% |
| `common-blocks` | 3181 | 288 | 0 | 9.1% |
| `parser-transform` | 3351 | 310 | 0 | 9.3% |
| `long-session-large-doc` | 2181 | 313 | 0 | 14.4% |
| `block-gauntlet` | 4044 | 597 | 0 | 14.8% |
| `structure` | 536 | 95 | 0 | 17.7% |
| `persistence-no-title` | 2099 | 424 | 2 | 20.2% |
...
| gauntlet block core/html next coverage tier | 383 | 500 |
...
the latest completed review, `20260516T163625Z`, took `7.5` minutes. The latest
PR-split feedback-action file, `20260516T163625Z`, is empty. The latest
non-empty feedback-action, `20260516T162217Z`, applied the Cycle 164 consensus
that the 28-head allow-list is only a known-fix prefix, recorded the active
repair job's partial state, and launched no new jobs.

The latest PR-split synthesis, `20260516T163625Z`, says the split design is not
...
split history. The latest parsed snapshot, `2026-05-16T16:38:50Z`, has `26`
...
and deep-analysis follow-ups remain open. The graph should be read as startup
...
as a new post-PR15C PR unless source ownership proves otherwise, and says the
active repair report is still missing while its tmux session remains active. The
next gate is one bounded repair and focused seed validation, not filing or broad
final-stack fuzz.
...
bucket has about `36` browser/e2e test executions/hour, `4,816`
unit-property test executions/hour, `64` coverage-guided-lower-level test
