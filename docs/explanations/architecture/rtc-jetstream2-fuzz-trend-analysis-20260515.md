...
Snapshot generated: `2026-05-18T02:27:16Z`
...
The coverage-guided loop is still expanding coverage. Across `2177` monitor
passes from `2026-05-15T01:21:42Z` through `2026-05-18T02:24:59Z`, coverage
files grew from `272` to `49088`, a delta of `48816`. The monitor's visible
...
metric: the newest sample has `duplicateShareCurrent=1.0000` and current
summary startup failures `0`. The eleven preceding current-output samples were
`0.0000`, so the newest point is a fresh live regression rather than historical
aggregate noise. The latest pass has `0` quality issues, `0` warnings, `416.1G`
free memory, and a true monitor headroom flag. This report uses current-output
...
`0.3449` and is not the plotted live health signal.

Persona-loop evidence is treated as evidence to evaluate, and it agrees that
the graph should not be read as recovery. The latest duplicate/noise synthesis,
`20260518T021322Z`, says the consensus root cause is still a producer-side
no-product `pre_action_bootstrap_stall` leak: below-threshold seed drains can
fall out of current-run accounting without durable `no-analysis.json` sentinels
or group cooldowns, then be regenerated. The latest duplicate/noise
feedback-action file, `20260518T012323Z`, says a bounded cross-root cooldown was
implemented in the novelty monitor, but the later synthesis says the smallest
safe fix still belongs in the supervisor seed-drain path. The graph's latest
`1.0000` current duplicate share supports treating that action as insufficient
...
ungrouped split aliases before filing. Filing and final-stack validation remain
blocked by PR07 owner evidence, root disk below the replay threshold, and
final-stack-only seed `1020002`. It still rejects stale Cycle293/Cycle306/local
publish rows, fallback-tail `PR05D`, raw `PR07D`, `PR17`, `PR18`, and `PR18x`
as filing evidence.
...
samples are eleven `0.0000` samples followed by `1.0000`. The latest graph
therefore shows live current-output duplicate/noise regression, not sustained
recovery. The health graph does not use
...
duplicate/noise synthesis rejects treating earlier clean samples as a durable
all-clear because no-product startup seed drains can disappear from current-run
accounting without durable sentinels or cooldowns. The latest feedback-action
says a novelty-monitor cooldown was implemented and the active monitor moved to
`run-20260518T021144Z`, but the later synthesis says the supervisor seed-drain
path still needs the product-preserving sentinel/cooldown fix.

![CPU utilization over time](rtc-jetstream2-fuzz-trends-20260515/plots/cpu-utilization-over-time.png)

![Load average over time](rtc-jetstream2-fuzz-trends-20260515/plots/load-average-over-time.png)

Recent sysstat samples through `2026-05-18T02:20:00Z` show bursty CPU. The
latest 25 CPU samples range from `58.1%` to `84.1%` utilization, with the latest
sample at `79.8%`. Over those same 25 samples, one-minute, five-minute, and
15-minute load all exceeded the `64` logical CPU count in `6` windows, and at
least one load window exceeded it in `16`. The newest 1/5/15-minute load sample
is `66.31`, `68.38`, and `67.70`; all three windows are above the logical CPU
count.
...
The latest native-harness synthesis, `20260518T021000Z`, keeps rich-text CRDT
...
The latest protocol-server synthesis, `20260518T021124Z`, still selects the
HTTP polling REST endpoint. The latest protocol action file is empty; the
latest non-empty protocol action, `20260518T014952Z`, implemented and validated
the protocol/server harness with `1` seed, `13` fixed cases, and isolated
storage PHPUnit passing `18` tests with `86` assertions. The later level-mix
feedback-action says the protocol sentinel stopped after three infra failures
from `Call to undefined function gutenberg_override_style()`. The latest
fuzz-only assertion apply file is empty. These are harness and assertion
...
The latest collected execution data has about `5,478,367` completed test
executions: `139,570` browser/e2e, `3,006` transport/integration, `4,887,968`
unit-property, and `447,823` coverage-guided-lower-level. The latest partial
15-minute bucket reports about `4,608` browser/e2e test executions/hour,
`13,184` unit-property executions/hour, and `0` coverage-guided-lower-level
...
likely-real findings over about `2,017.1` runner-hours, or `32.57` per 100
...
Current unique bug-output candidate rates are: browser/e2e `5,545` candidates
over `2,017.1` runner-hours (`274.89` per 100 runner-hours),
transport/integration `106` over `59.5` runner-hours (`178.14` per 100
runner-hours), coverage-guided lower-level `2` over `20.9` runner-hours
(`9.55` per 100 runner-hours), and unit/property `5` over `29.3` runner-hours
...
are about `5747.8` for browser/e2e, `4537.5` for transport/integration,
`759.3` for coverage-guided lower-level, and `2450.5` for unit/property.
...
| --- | ---: | ---: | ---: | ---: |
| `full` | 840 | 18 | 0 | 2.1% |
| `multi-reload-lifecycle` | 3428 | 132 | 0 | 3.9% |
| `revision-persistence` | 4808 | 188 | 0 | 3.9% |
| `parser-serialization` | 3534 | 212 | 0 | 6.0% |
| `real-user-editing` | 7625 | 602 | 0 | 7.9% |
| `parser-transform` | 4424 | 478 | 0 | 10.8% |
| `common-blocks` | 4294 | 465 | 0 | 10.8% |
| `structure` | 536 | 95 | 0 | 17.7% |
| `long-session-large-doc` | 3098 | 578 | 0 | 18.7% |
| `block-gauntlet` | 6432 | 1211 | 0 | 18.8% |
| `persistence-no-title` | 3497 | 999 | 0 | 28.6% |
| `session-lifecycle` | 8510 | 2571 | 0 | 30.2% |
...
filing/final-stack validation. It keeps rejecting stale
Cycle293/Cycle306/local-publish rows, fallback-tail `PR05D`, raw `PR07D`,
`PR17`, `PR18`, and `PR18x` as filing sources.

The suggested-PR size charts are parsed from the status report's proposed PR
split history. The latest parsed snapshot, `2026-05-18T02:16:51Z`, has `8`
...
plotted live duplicate/noise sample is bad on the current-output-dir metric:
...
and `416.1G` free memory. The latest 1/5/15-minute load windows are `66.31`,
`68.38`, and `67.70`; all three are above the `64` logical CPU count.
Historical aggregate duplicate/noise is not the live health signal.

The duplicate/noise persona loop rejects converting the preceding clean samples
into a durable all-clear. The latest synthesis says below-threshold no-product
startup seed drains can lose current-run visibility without durable
...
root disk below the replay threshold, final-stack-only seed `1020002`, and fresh
manifest/audit evidence. The earlier feedback-action launched a fixed PR07 i40
owner replay including `HOLD-07B2`, refreshed i40 manifest/deferred artifacts,
and ran a PR05 owner comparison that remained blocked by a missing
`framer-motion` dependency, but the later synthesis still treats PR07 evidence
as setup/preflight-only.
