...
Snapshot generated: `2026-07-10T11:43:28Z`
...
at `2026-07-10T11:41:37Z`. There are `3,834` monitor
passes from `2026-06-23T12:08:03Z` onward. Cumulative coverage record
observations rose from `515` to
`132,869`, current-scan coverage files rose from
`890` to `93`, and
unmet goals are `112` of `138`.
...
`2`, and free memory is
`190.9GiB`. Because this snapshot has no
...
The latest CPU/load telemetry sample has CPU utilization `36.2`, load1/load5/load15 `51.69`/`56.94`/`52.48`, and `64` logical cores.

The latest disk sample is `2026-07-10T11:42:21Z`: root has `48.4GiB` free and is `65.9%` used; the data volume has `202.4GiB` free and is `94.3%` used.
...
`13,121,862` individual test or case executions.
The latest cumulative/rate summary is:
`browser-e2e=10646 cumulative/1144 per-hour; transport-integration=0 cumulative/0 per-hour; unit-property=156 cumulative/0 per-hour; coverage-guided-lower-level=0 cumulative/0 per-hour; backend-api=0 cumulative/0 per-hour; protocol-server=13111060 cumulative/0 per-hour; fuzz-assertion=0 cumulative/0 per-hour; other=0 cumulative/0 per-hour`. Some lower-level rows are approximate
...
`102` triaged bug-finding rows,
`35` unique likely-real findings,
`690` bug-output rows, and
`627` unique output candidates.

PR-controller graph inputs are present. The controller snapshot has
`26` items:
`deferred-family/downscoped/medium=2; ready-product-pr/held-by-controller/high=3; ready-product-pr/published/high=14; ready-product-pr/superseded/low=1; ready-product-pr/superseded-by-repair/low=1; repair-branch-adoption/already-in-release-candidate/high=2; repair-branch-adoption/stale-candidate-base/high=2; runtime-gated-pr/runtime-held-consumed/high=1`. The artifact index contributes
...
Current enabled groups: `novelty-http-table-stale-snapshot,novelty-ws-structure,novelty-ws-revision-persistence,novelty-ws-revision-recovery,novelty-http-table-stale-snapshot-completion`.
