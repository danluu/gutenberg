...
Snapshot generated: `2026-05-24T09:04:04Z`
...
The graph-refresh pipeline is current through `2026-05-24T09:00:54Z` for
completed monitor passes and through `2026-05-24T09:02:37Z` for current-run
...
current-run full-pass timestamp is `2026-05-24T09:00:54Z`.

The monitor has `4,403` passes from `2026-05-15T01:21:42Z` onward. Cumulative
coverage record observations are `294,195`, and the current-scan coverage-file
count is `1,114`. Current coverage/profile intake is populated:
`coverage_goals.csv` has `137` goals with `29` unmet, and `profile_counts.csv`
has `19` profiles.
...
That one-of-two top-duplicate share is a narrow current-run signal, not a broad
duplicate storm.
...
refreshed trusted graph is higher at `0.5`, but only over two actionable
...
CPU is elevated but load remains just below the `64`-core line in the newest
samples. At `2026-05-24T09:00:00Z`, CPU utilization is `68.90%`, iowait is
`1.54%`, and load averages are `61.58`, `59.05`, and `51.46` on `64` logical
CPUs. Disk is still the main resource constraint: at `2026-05-24T09:03:20Z`,
root has `88.0GiB` free and the data volume has `206.9GiB` free while `94.2%`
used.
...
`run-20260524T082929Z`, with fresh rows at `2026-05-24T08:59:27Z` for WS
...
protocol-server-heavy, but the latest partial bucket has only browser/e2e
counts; transport-integration, backend-api, and standalone fuzz-only assertion
work remain graph-inactive.

The execution counter has `17,232,325` estimated individual executions. The
latest plotted bucket, `2026-05-24T09:00:00Z`, is a partial bucket with `54`
browser/e2e executions, `0` coverage-guided lower-level executions, and `0`
...
`2026-05-24T09:02:01Z`. The state-count table has `35`
...
`2026-05-24T09:02:37Z`. It has `status_available=TRUE`,
`pending_until_first_pass=FALSE`, `current_run_metrics_trusted=TRUE`, and a
completed current-run full-pass timestamp of `2026-05-24T09:00:54Z`. The
...
signatures are `2`/`2`/`2`, so the top duplicate share is one signature out of
a two-signature denominator. Historical aggregate
...
below threshold. The refreshed graph is now trusted and shows `0.5` over only
two actionable signatures. That is a live control-plane watch item, but still
rejects interpreting the available evidence as a product bug or broad product
duplicate storm.
...
has `206.9GiB` free while still `94.2%` used. Latest CPU iowait is `1.54%`,
...
coverage-guided rows from `run-20260524T082929Z` at `2026-05-24T08:59:27Z`.
...
partial execution bucket has only browser/e2e counts, while the preceding
complete-ish buckets remain protocol-server-heavy.
...
interpretation: graph-counted browser/e2e residency is `36` lanes across `36`
...
Latest cumulative totals are approximately `4,726,697` browser/e2e,
`1,132,740` unit-property, `458,609` coverage-guided lower-level, and
`10,914,279` protocol-server executions. Transport-integration, backend-api,
fuzz-assertion, and other buckets are `0` in the reconstructed table.

The latest `2026-05-24T09:00:00Z` bucket is partial: it has `54` browser/e2e
executions (`216`/hour), `0` coverage-guided-lower-level executions (`0`/hour),
...
triaged likely-real output has `23` unique likely-real findings, all
browser/e2e over about `114.0` runner-hours. Protocol-server has `0` likely-real
...
lower-level assertion failures. Current unique candidate output has `156`
raw candidates: `149` browser/e2e candidates, `5` unit-property candidates,
...
persistence-no-title has `46` and `16`, list-move-refresh HTTP has `21` and
`0`, revision persistence has `18` and `7`, parser serialization has `5` and
`0`, and multi-reload lifecycle has `4` and `0`.
The large-post three-user HTTP
lifecycle profile has `369` records, `27` successes, and `0` profile-row
...
The refreshed `coverage_goals.csv` has `137` goals, with `108` met and `29`
...
signals, multi-reload lifecycle, and parser serialization. Low-progress misses
include media cross-entity, autosave/local-autosave, same-user collaborator
mode, twelve-user late join, revision eligibility, real-user editing, and weak
block/file/details/social/gallery edges. The strict combined-ingredient and
...
marked enabled, and the adjacent large-post three-user HTTP profile has `369`
records seen, `27` successful records, and a `0.0732` success rate. The
generated goal table also shows the related successful-profile goal at
`27`/`10`, HTTP lifecycle scale at `369`/`10`, and successful three-user HTTP
records at `23`/`10`, but the strict
...
largest feature categories by total count are history (`38,927`), invariant
(`24,784`), operation-ledger (`21,718`), block-depth (`8,684`), block
(`8,064`), other (`7,810`), action-pair (`7,266`), action (`7,224`), and
transport (`5,282`). Successful actions remain concentrated in a few paths:
`edit-title` (`730`), `append-paragraph` (`465`), `concurrent-paragraphs`
(`298`), block-gauntlet insert/edit actions (`100`/`100`),
...
`2026-05-24T09:02:01Z`. The state-count table has `35` counted items:
...
queue updated at `2026-05-24T09:01:35Z` and reports the exact-stack repair as
...
`2026-05-24T09:00:54Z`, `duplicateShareCurrent=0.5`, and summary startup
...
keeps duplicate/noise as a live control-plane watch item, but still rejects a
broad product duplicate-storm interpretation while preserving
producer/control-plane follow-up.

The resource picture is disk-constrained with elevated CPU/load in the latest
sample. CPU utilization is `68.90%`, iowait is `1.54%`, one-minute load is just
below the `64`-core line, and the data volume is `94.2%` used with `206.9GiB`
free.
...
mix rows include `36` graph-counted browser/e2e lanes across `36` groups, plus
...
residency and execution evidence through the `08:45` bucket, unit-property has
a bounded canary row, and coverage-guided lower-level has fresh plotted
rich-text CRDT residency. The
