...
Snapshot generated: `2026-07-02T17:08:49Z`
...
The latest CPU/load telemetry sample has CPU utilization `31`, load1/load5/load15 `16.66`/`15.98`/`19.15`, and `64` logical cores.

The latest disk sample is `2026-07-02T17:07:39Z`: root has `69.4GiB` free and is `51.0%` used; the data volume has `828.4GiB` free and is `76.6%` used.
...
`13,132,338` individual test or case executions.
The latest cumulative/rate summary is:
`browser-e2e=21278 cumulative/72 per-hour; transport-integration=0 cumulative/0 per-hour; unit-property=0 cumulative/0 per-hour; coverage-guided-lower-level=0 cumulative/0 per-hour; backend-api=0 cumulative/0 per-hour; protocol-server=13111060 cumulative/0 per-hour; fuzz-assertion=0 cumulative/0 per-hour; other=0 cumulative/0 per-hour`. Some lower-level rows are approximate
...
`1,786` bug-output rows, and
`1,765` unique output candidates.
