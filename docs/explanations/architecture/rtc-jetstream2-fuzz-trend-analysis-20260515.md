...
Snapshot generated: `2026-05-24T13:33:56Z`
...
The graph-refresh pipeline is current through `2026-05-24T13:31:02Z` for
completed monitor passes, with current-run accounting sampled at
`2026-05-24T13:32:47Z`. The active accounting row is
`run-20260524T112329Z`; `current_run_metrics_trusted_last` is `TRUE`, so the
active output dir has completed a full pass.

The monitor has `4,500` passes from `2026-05-15T01:21:42Z` onward. Cumulative
coverage record observations are `296,016`, and the current-scan coverage-file
count is `2,394`. Current coverage/profile intake is populated:
...
At `2026-05-24T13:30:00Z`, CPU utilization is `24.54%`, iowait is `0.80%`, and
load averages are `16.58`, `20.85`, and `21.30` on `64` logical CPUs. At
`2026-05-24T13:33:06Z`, root has `87.8GiB` free and the data volume has
`202.6GiB` free while `94.3%` used.
...
The execution counter has `17,257,261` estimated individual executions. The
latest plotted bucket, `2026-05-24T13:30:00Z`, has `5` browser/e2e executions
and `1,260` protocol-server executions. The `13:15` bucket had `7`
...
`2026-05-24T13:32:02Z`. The state-count table has `35` counted items:
...
`2026-05-24T13:32:47Z`. It has `status_available=TRUE`,
...
volume has `202.6GiB` free while still `94.3%` used. Latest CPU iowait is
`0.80%`; output-size and disk headroom remain the dominant resource constraints.
...
The latest coverage-guided browser rows are at `2026-05-24T13:33:04Z`:
`novelty-http-large-post-lifecycle-completion` and
...
refreshed graph now shows the synthesis-requested second forced HTTP browser
row and the protocol-server restart, but it still does not show a backend-api
lane. Treat the scheduled browser rows as residency, not proven useful
capacity, until successful records or explicit blockers appear.
...
in the `13:15` bucket, and `1,260` in the `13:30` bucket.
...
Latest cumulative totals are approximately `4,727,233` browser/e2e,
`1,132,740` unit-property, `458,611` coverage-guided lower-level, and
`10,938,677` protocol-server executions. Transport-integration, backend-api,
fuzz-assertion, and other buckets are `0` in the reconstructed table.

The latest `2026-05-24T13:30:00Z` bucket has `5` browser/e2e executions
(`20`/hour) and `1,260` protocol-server executions (`5,040`/hour), with zero
unit-property, coverage-guided-lower-level, backend-api,
transport-integration, and fuzz-assertion executions. The `13:15` bucket has
`7` browser/e2e executions and `6,505` protocol-server executions, the `13:00`
...
browser/e2e findings over about `122.0` runner-hours and `1`
...
lower-level assertion failures. Current unique candidate output has `269` raw
candidates: `223` browser/e2e candidates, `39` transport-integration
...
three-user HTTP lifecycle profile has `566` records, `55` successes, and `0`
...
completion row. The adjacent large-post three-user HTTP profile has `566`
records seen, `55` successful records, and a `0.09717` success rate. The
generated goal table also shows HTTP lifecycle scale at `566`/`10`, successful
...
largest feature categories by total count are history (`79,229`), invariant
(`48,397`), operation-ledger (`46,949`), block-depth (`19,066`), block
(`17,531`), action-pair (`17,246`), action (`15,279`), other (`14,247`),
transport (`8,846`), and collaborator (`8,708`). Successful actions remain
...
accounting: the latest trusted row is clean. The `13:25`, `13:17`, `13:09`,
`13:00`, and `12:52` rows were also clean over zero signatures. The `12:45` and
...
The resource picture is still disk-constrained. CPU utilization is `24.54%`,
iowait is `0.80%`, five-minute load is below the `64`-core line at `20.85`, and
the data volume is `94.3%` used with `202.6GiB` free. Level-mix persona output
