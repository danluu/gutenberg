...
Snapshot generated: `2026-05-21T17:42:35Z`
...
The graph data is current through `2026-05-21T17:41:08Z`. The monitor has
`3,790` passes from `2026-05-15T01:21:42Z` onward. Cumulative coverage record
...
`390.1G` free memory, `31.5GiB` free on `/`, and `582.1GiB` free on
...
fresh events, and fresh summaries agree. That feedback was based on a context
where browser/e2e was below its 24-lane floor; the refreshed graph-counted mix
now shows `25` browser/e2e lanes, but the feedback still rejects counting them
as useful capacity without the fail-closed live-lane checks. It recommends
recovering operator HTTP correctness first and says backend/protocol capacity
should count as zero when exact sessions are missing.
...
around `84%` used and root is around `80%` used, so cleanup and output-size
...
The latest current enabled set includes WS many-user lifecycle,
thirty-user lifecycle, parser serialization, revision persistence, and
permissions/auth/locks, plus HTTP large-post lifecycle completion and HTTP
persistence probes. Historical enabled events cover additional real-user
editing, same-user lifecycle, HTTP table stale snapshots, HTTP title reload
convergence, existing-post CRDT metadata, block-gauntlet, rich-text UI,
revision/autosave/recovery, async/server-backed, parser transform,
...
instead of scanning unbounded history. The latest mix is concentrated in
browser/e2e lanes at `25` graph-counted lanes. Lower-level work is visible:
...
rich-text CRDT. Protocol-server evidence calls the HTTP polling protocol/server
harness the first ready server-side target, but the level-mix feedback still
says protocol/server capacity should count as zero when exact tmux sessions are
...
The latest totals are approximately `389,956` browser/e2e, `161`
...
lower-level assertion failures. Current unique candidate output is `4,279`
...
coverage-promotion blocker, queued PR07C owner-matrix and reload-hydration
blockers, and three terminal blockers. Repeated no-progress artifacts are
...
The current fuzzing mix is browser/e2e-heavy and now graph-counted at `25`
...
queued reload-hydration and PR07C blockers into validated publishable branches
