...
Snapshot generated: `2026-05-19T13:55:15Z`
...
repaired passes continue through `2026-05-19T13:53:34Z`. Across `2449` monitor
passes from `2026-05-15T01:21:42Z` through `2026-05-19T13:53:34Z`, cumulative
coverage record observations rose from `782` to `253502`. Coverage-file counts
are current-scan counts, not cumulative coverage; the latest current scan is
`1754` files after the latest root rollover. The monitor's visible likely-real
maximum remains `4`.
...
`no_progress=0`, and `406.5G` free memory. The latest monitor pass has
`headroom=false`. The newest one-minute load sample is just below the `64`
logical CPU count, but recent load exceeded it in `10` of the latest `25`
...
share is `0.2222` and is not the plotted live health signal.
...
paused/no-analysis-inclusive triage views each see one root, one file, `7`
...
matching feedback-action reports a control-plane patch: no-product
...
`20260519T133640Z`, rejects reading those graph-visible lanes as healthy useful
capacity: it saw browser/e2e still under the `24`-lane floor, with context at
`9/24` and live checks around `14-19/24`. It says to repair the three gap
REST/core-install materialization failures first and keep lower-level capacity
capped. The latest level-mix feedback-action file is empty; the previous
substantive feedback, `20260519T124623Z`, also rejected adding lower-level
...
committed counters. The latest native-harness synthesis and action,
`20260519T132653Z`, select and validate
`coverage-guided-lower-level-block-parser-serialization` as the first ready
isolated lower-level harness, with rich-text CRDT next. The latest
protocol-server synthesis, `20260519T134156Z`, keeps the REST protocol harness
ahead of WebSocket fuzzing, and the latest action, `20260519T132815Z`,
implemented and validated the HTTP polling REST harness for
...
`253502`; coverage-file counts are tracked separately and reset when the active
output root rolls. The latest current-output sample is `1754` files from
...
`0`, warnings `0`, `no_progress=0`, `406.5G` free memory, and
...
root, one file, `7` summary product-evidence records, and `0` signatures. The
...
Recent sysstat samples through `2026-05-19T13:50:00Z` show bursty CPU and load.
The latest 25 CPU samples range from `43.60%` to `81.01%` utilization, with the
latest sample at `74.05%`. Over those same 25 samples, one-minute load exceeded
...
newest 1/5/15-minute load sample is `61.19`, `62.52`, and `57.55`; all are
below the `64` logical CPU count. The newest sample has `1` blocked task.
...
`20260519T133640Z`, says the intended mix should remain browser-heavy, but
useful browser/e2e capacity is still under the `24`-lane floor: context showed
`9/24` lanes and live checks were around `14-19/24`. It recommends targeted
repair for three gap REST/core-install failures and keeping lower-level lanes
capped. The latest level-mix feedback-action file is empty; the previous
substantive feedback-action, `20260519T124623Z`, also says to restore
browser/e2e capacity first and not add lower-level lanes. This rejects both a
graph-only "add lower-level capacity now" interpretation and a graph-only "`37`
...
as the next lane; the matching action implemented and validated that parser
lane, including a one-attempt coverage-guided smoke and continuous parser tmux
lane evidence. The latest protocol-server synthesis, `20260519T134156Z`, keeps
the REST protocol harness ahead of WebSocket fuzzing; the latest action,
`20260519T132815Z`, implemented and validated the HTTP polling REST harness with
`20` individual protocol cases and `23` nonzero oracle counters. The latest
...
The latest collected execution data has about `6,242,716` completed test
executions: `383,343` browser/e2e, `3,006` transport/integration,
`5,408,544` unit-property, and `447,823` coverage-guided-lower-level. The
latest partial 15-minute bucket reports about `1012` browser/e2e executions/hour
and `3584` unit-property executions/hour, with `0` current counted rate for
...
On that triage-output metric, browser/e2e currently dominates: `829` unique
likely-real findings over about `2813.1` runner-hours, or `29.47` per 100
...
Current unique bug-output candidate rates are: browser/e2e `6,420` candidates
over `2813.1` runner-hours (`228.22` per 100 runner-hours),
transport/integration `106` over `59.5` runner-hours (`178.14` per 100
runner-hours), coverage-guided lower-level `2` over `20.9` runner-hours
(`9.55` per 100 runner-hours), and unit/property `6` over `63.1` runner-hours
...
are about `12655.2` for browser/e2e, `4537.5` for transport/integration,
`759.3` for coverage-guided lower-level, and `2360.5` for unit/property.
...
| --- | ---: | ---: | ---: | ---: |
| `full` | 1047 | 34 | 0 | 3.2% |
| `multi-reload-lifecycle` | 4513 | 163 | 0 | 3.6% |
| `revision-persistence` | 7746 | 289 | 0 | 3.7% |
| `parser-serialization` | 4657 | 283 | 0 | 6.1% |
| `real-user-editing` | 10218 | 873 | 0 | 8.5% |
| `common-blocks` | 5352 | 512 | 0 | 9.6% |
| `parser-transform` | 6034 | 658 | 0 | 10.9% |
| `long-session-large-doc` | 5329 | 741 | 0 | 13.9% |
| `block-gauntlet` | 7758 | 1366 | 0 | 17.6% |
| `structure` | 541 | 96 | 0 | 17.7% |
| `session-lifecycle` | 10041 | 2768 | 0 | 27.6% |
| `media-cross-entity` | 566 | 171 | 0 | 30.2% |
| `three-user-late-join` | 12557 | 3961 | 0 | 31.5% |
| `permissions-auth-locks` | 10402 | 4415 | 0 | 42.4% |
| `persistence-no-title` | 4846 | 2110 | 0 | 43.5% |
| `async-server-blocks` | 11617 | 5068 | 0 | 43.6% |
...
It also shows quality issues `0`, warnings `0`, `no_progress=0`, and `406.5G`
free memory. The same latest pass has `headroom=false`. The newest
one-minute load sample is just below the `64` logical CPU count, but recent load
...
Historical aggregate duplicate/noise is `0.2222`,
...
startup pauses. The latest feedback action patched the control plane so
no-product `pre_action_bootstrap_stall` cooldowns hard-block producers, stale
startup-noise canary/bypass state is cleared unless current product evidence is
present, and product-evidence mixed runs do not pause on absolute startup count
alone. The graph should still be read as current
...
useful browser/e2e capacity is still below the `24`-lane floor despite the
graph-visible lane count, and rejects lower-level expansion now. Lower-level
work is real but narrow: unit-property
