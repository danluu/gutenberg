...
Snapshot generated: `2026-06-04T07:28:53Z`
...
share is `0.4667`. Current summary startup
failures are `0`, quality issues are
`0`, and free memory is
`227.3GiB`. Because this snapshot has no
...
The latest CPU/load telemetry sample has CPU utilization `24.1`, load1/load5/load15 `17.63`/`20.25`/`18.97`, and `64` logical cores.

The latest disk sample is `2026-06-04T07:26:41Z`: root has `115.9GiB` free and is `18.3%` used; the data volume has `277.4GiB` free and is `92.2%` used.
...
`18,642,199` individual test or case executions.
The latest cumulative/rate summary is:
`browser-e2e=700291 cumulative/880 per-hour; transport-integration=0 cumulative/0 per-hour; unit-property=5838464 cumulative/0 per-hour; coverage-guided-lower-level=623766 cumulative/0 per-hour; backend-api=0 cumulative/0 per-hour; protocol-server=11479678 cumulative/0 per-hour; fuzz-assertion=0 cumulative/0 per-hour; other=0 cumulative/0 per-hour`. Some lower-level rows are approximate
...
`182` triaged bug-finding rows,
`96` unique likely-real findings,
`1,423` bug-output rows, and
