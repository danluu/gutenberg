...
Snapshot generated: `2026-06-19T19:06:43Z`
...
at `2026-06-19T19:05:05Z`. There are `485` monitor
passes from `2026-06-17T17:45:17Z` onward. Cumulative coverage record
observations rose from `544` to
`25,642`, current-scan coverage files rose from
`296` to `1,311`, and
unmet goals are `14` of `136`.
...
`202.7GiB`. Because this snapshot has no
...
The latest CPU/load telemetry sample has CPU utilization `19.9`, load1/load5/load15 `25.32`/`22.97`/`22.5`, and `64` logical cores.

The latest disk sample is `2026-06-19T19:05:54Z`: root has `93.7GiB` free and is `33.9%` used; the data volume has `147.8GiB` free and is `95.8%` used.

The latest graph-counted fuzzing mix is: `browser-e2e=11 lanes/11 groups; protocol-server=1 lanes/1 groups`.
...
`11,480,433` individual test or case executions.
The latest cumulative/rate summary is:
`browser-e2e=755 cumulative/116 per-hour; transport-integration=0 cumulative/0 per-hour; unit-property=0 cumulative/0 per-hour; coverage-guided-lower-level=0 cumulative/0 per-hour; backend-api=0 cumulative/0 per-hour; protocol-server=11479678 cumulative/0 per-hour; fuzz-assertion=0 cumulative/0 per-hour; other=0 cumulative/0 per-hour`. Some lower-level rows are approximate
...
`128` bug-output rows, and
`128` unique output candidates.
