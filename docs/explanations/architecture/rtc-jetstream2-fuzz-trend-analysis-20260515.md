...
Snapshot generated: `2026-06-04T03:26:20Z`
...
`218.8GiB`. Because this snapshot has no
...
The latest CPU/load telemetry sample has CPU utilization `29.1`, load1/load5/load15 `15.75`/`18.33`/`18.65`, and `64` logical cores.

The latest disk sample is `2026-06-04T03:24:21Z`: root has `113.6GiB` free and is `19.9%` used; the data volume has `279.1GiB` free and is `92.1%` used.
...
`18,639,378` individual test or case executions.
The latest cumulative/rate summary is:
`browser-e2e=697470 cumulative/1036 per-hour; transport-integration=0 cumulative/0 per-hour; unit-property=5838464 cumulative/0 per-hour; coverage-guided-lower-level=623766 cumulative/0 per-hour; backend-api=0 cumulative/0 per-hour; protocol-server=11479678 cumulative/0 per-hour; fuzz-assertion=0 cumulative/0 per-hour; other=0 cumulative/0 per-hour`. Some lower-level rows are approximate
...
`165` triaged bug-finding rows,
`92` unique likely-real findings,
`1,151` bug-output rows, and
`858` unique output candidates.
