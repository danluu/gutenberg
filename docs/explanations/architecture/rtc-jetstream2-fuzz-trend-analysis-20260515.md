...
Snapshot generated: `2026-05-23T11:00:37Z`
...
The graph-refresh pipeline is current through `2026-05-23T10:57:49Z` for
monitor passes, with the latest current-run accounting sample at
`2026-05-23T10:59:04Z`. The latest completed full duplicate/noise accounting
pass recorded in the active row is `2026-05-23T10:57:49Z`, about `1.25`
minutes behind the sample. The monitor has
`4,213` passes from `2026-05-15T01:21:42Z` onward, cumulative coverage record
observations are `285,329`, current-scan coverage files are `4,296`, and the
...
disk sample has `89.1GiB` free on `/` and `244.4GiB`
free on `/media/volume/danluu-fuzz-data`; the data volume is `93.1%` used.
...
The execution counter has `17,082,319` estimated individual executions. The
trailing `2026-05-23T10:45:00Z` bucket is partial and has `1,302`
browser/e2e, `3` unit-property, and `4,500` protocol-server executions, with zero
...
at `2026-05-23T10:59:04Z`; the latest completed full duplicate/noise
accounting pass recorded in that row is `2026-05-23T10:57:49Z`, so the
sampled full-pass lag is about `1.25` minutes. The row reports
...
completed full pass about `1.25` minutes behind the latest sample. Startup
...
stable at `89.1GiB` free and `42.1%` used; the data volume has `244.4GiB`
free and is `93.1%` used, so output-size budgeting still matters.
...
`2026-05-23T10:37:28Z` and `4,500` protocol-server executions in the trailing
...
Latest cumulative totals are approximately `116,967` browser/e2e, `5,837,123`
unit-property, `458,097` coverage-guided lower-level, and `10,670,132`
...
The latest `2026-05-23T10:45:00Z` bucket is partial and has `1,302`
browser/e2e executions (`5,208`/hour), `3` unit-property executions
(`12`/hour), and `4,500` protocol-server executions (`18,000`/hour), with zero
coverage-guided-lower-level, backend-api, transport-integration, and
...
findings over about `638.4` runner-hours, or `24.90` per 100 runner-hours.
...
collaboration UI signals (`686` records) and list-move refresh HTTP (`149`
records, with `2` startup failures). Table stale snapshot HTTP now has `233`
successful records from `406` records and has cleared its `10`-record goal.
...
lifecycle has `291` from `499`, and the adjacent large-post three-user HTTP
profile has `154` successful records from `1,383` records.
...
adjacent large-post three-user HTTP profile has `1,383` records seen with `154`
...
| Goal                                                        | Current | Target |
| ----------------------------------------------------------- | ------: | -----: |
| CDP coverage records next coverage tier                     |   4,089 |  5,000 |
| action ui-type-title next coverage tier                     |   1,330 |  2,000 |
| successful two-user documents next coverage tier            |   1,361 |  2,000 |
| action ui-undo-redo-paragraph next coverage tier            |     565 |  1,000 |
| action ui-type-paragraph next coverage tier                 |   1,695 |  2,000 |
...
the `2026-05-23T10:59:04Z` sample, with the latest completed full pass
recorded at `2026-05-23T10:57:49Z`, about `1.25` minutes behind the sample.
...
the data volume is `93.1%` used. Optional browser admission should still
...
buckets, including `4,500` executions in the trailing partial `10:45` bucket,
...
and has `1,302` browser/e2e, `3` unit-property, and `4,500` protocol-server
...
though the adjacent large-post three-user HTTP profile has `1,383` records and
