...
Snapshot generated: `2026-07-02T13:07:04Z`
...
`196.4GiB`. Because this snapshot has no
...
The latest CPU/load telemetry sample has CPU utilization `30.2`, load1/load5/load15 `33.05`/`30.24`/`28.55`, and `64` logical cores.

The latest disk sample is `2026-07-02T13:05:47Z`: root has `70.5GiB` free and is `50.3%` used; the data volume has `854.2GiB` free and is `75.9%` used.
...
`13,130,172` individual test or case executions.
The latest cumulative/rate summary is:
`browser-e2e=19112 cumulative/248 per-hour; transport-integration=0 cumulative/0 per-hour; unit-property=0 cumulative/0 per-hour; coverage-guided-lower-level=0 cumulative/0 per-hour; backend-api=0 cumulative/0 per-hour; protocol-server=13111060 cumulative/0 per-hour; fuzz-assertion=0 cumulative/0 per-hour; other=0 cumulative/0 per-hour`. Some lower-level rows are approximate
...
`1,694` bug-output rows, and
`1,673` unique output candidates.
...
`10` blockers:
`active=4; held=1; runnable=1; terminal=4`.
