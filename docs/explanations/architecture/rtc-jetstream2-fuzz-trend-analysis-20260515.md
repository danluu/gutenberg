...
Snapshot generated: `2026-05-23T05:10:40Z`
...
The graph-refresh pipeline is current through `2026-05-23T05:05:52Z` for
monitor passes, with the latest current-run accounting sample at
`2026-05-23T05:09:17Z`. The latest completed full duplicate/noise accounting
pass is `2026-05-23T05:05:52Z`. The monitor has `4,164` passes from
`2026-05-15T01:21:42Z` onward, cumulative coverage record observations are
`283,091`, current-scan coverage files are `2,844`, and the parsed
...
sample has `89.5GiB` free on `/` and `347.2GiB`
...
persona-loop floor: `30` browser/e2e lanes across `29` groups.
...
browser/e2e read at `30`/`24` lanes. It captures a fresh focused
...
The execution counter has `16,993,096` estimated individual executions. The
trailing `2026-05-23T05:00:00Z` bucket has `654` browser/e2e executions,
`540` unit-property executions, and `25` protocol-server executions, with zero
...
`2026-05-23T05:09:17Z`; the latest completed full duplicate/noise accounting
pass was `2026-05-23T05:05:52Z`, about `3.4` minutes behind the sample. The
...
stable at `89.5GiB` free and `41.9%` used; the data volume has `347.2GiB` free
...
dominant and now above the persona-loop floor: `30` browser/e2e lanes across
`29` groups. Current browser/e2e rows include coverage-guided WebSocket lanes
...
large-post readiness/lifecycle/completion; one focused
...
blocks, permissions/auth locks, and long-session large-doc; and fourteen
strict-expansion rows spanning WebSocket real-user, same-user, late-join,
...
HTTP polling canary row, and `protocol-server` has a current HTTP polling
...
executions. The refreshed graph is later and now above floor at `30`/`24`
...
Latest cumulative totals are approximately `104,727` browser/e2e, `5,822,556`
...
`654` browser/e2e executions (`2,616`/hour), `540` unit-property executions
(`2,160`/hour), and `25` protocol-server executions (`100`/hour), with zero
coverage-guided-lower-level, backend-api, transport-integration, and
...
findings over about `598.6` runner-hours, or `26.23` per 100 runner-hours.
...
lower-level assertion failures. Current unique candidate output has `1,567`
raw candidates. The by-level rate table attributes `1,559` browser/e2e
...
list-move refresh HTTP (`73` records) and collaboration UI signals (`229`
records). Table stale snapshot HTTP now has `42` successful records from `168`
...
lifecycle has `150` from `280`, and the adjacent large-post three-user HTTP
profile has `100` successful records from `1,105` records. This still argues
...
adjacent large-post three-user HTTP profile has `1,105` records seen with `100`
...
`duplicateShareCurrent` `0.25`, summary startup failures `0`, four current
signatures, four actionable signatures, four product-evidence signatures, and
top duplicate share `0.25` at `2026-05-23T05:09:17Z`, with the latest
completed full pass at `2026-05-23T05:05:52Z`. This is measured current-run
...
loop's `24`-lane browser/e2e floor with `30` lanes across `29` groups. Current
...
levels were represented; the refreshed graph is later, reports `30`/`24`
...
and has `654` browser/e2e, `540` unit-property, and `25` protocol-server
...
though the adjacent large-post three-user HTTP profile has `1,105` records and
