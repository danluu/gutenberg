...
Snapshot generated: `2026-05-23T11:29:06Z`
...
The graph-refresh pipeline is current through `2026-05-23T11:23:24Z` for
monitor passes, with the latest current-run accounting sample at
`2026-05-23T11:28:04Z`. The latest completed full duplicate/noise accounting
pass recorded in the active row is `2026-05-23T11:23:24Z`, about `4.67`
minutes behind the sample. The monitor has
`4,217` passes from `2026-05-15T01:21:42Z` onward, cumulative coverage record
observations are `285,439`, current-scan coverage files are `4,405`, and the
...
is `56.76%`, with `2.01%` iowait. Latest load averages are `36.92`,
`43.56`, and `49.94` on `64` logical CPUs, with `1` blocked task. The latest
disk sample has `89.1GiB` free on `/` and `238.9GiB`
...
The latest non-empty level-mix feedback-action says it started one exact focused
...
The execution counter has `17,086,878` estimated individual executions. The
trailing `2026-05-23T11:15:00Z` bucket is partial and has `298` browser/e2e,
`12` unit-property, and zero protocol-server executions, with zero
coverage-guided-lower-level, backend-api, transport-integration, and
fuzz-assertion executions. The prior `11:00` bucket had `423` browser/e2e
and `3,625` protocol-server executions; `10:45` had `1,323` browser/e2e,
`3` unit-property, and `4,680` protocol-server executions; `10:30` had `146`
browser/e2e and `2,545` protocol-server executions; and `10:15` had `122`
browser/e2e and `50` protocol-server executions. The largest recent protocol
...
at `2026-05-23T11:28:04Z`; the latest completed full duplicate/noise
accounting pass recorded in that row is `2026-05-23T11:23:24Z`, so the
sampled full-pass lag is about `4.67` minutes. The row reports
...
completed full pass about `4.67` minutes behind the latest sample. Startup
...
stable at `89.1GiB` free and `42.1%` used; the data volume has `238.9GiB`
...
floor by its own telemetry. The latest non-empty feedback-action protected browser/e2e,
...
The latest non-empty level-mix feedback-action reports a bounded lower-level block-parser
...
`2026-05-23T11:03:50Z`. Protocol-server executions are `0` in the trailing
partial `11:15` bucket, while that same bucket has `298` browser/e2e and `12`
unit-property executions. Protocol-server has `3,625` in `11:00`, `4,680` in
...
Latest cumulative totals are approximately `117,709` browser/e2e, `5,837,135`
...
The latest `2026-05-23T11:15:00Z` bucket is partial and has `298`
browser/e2e executions (`1,192`/hour), `12` unit-property executions
(`48`/hour), and zero protocol-server executions, with zero
coverage-guided-lower-level, backend-api,
transport-integration, and fuzz-assertion executions. The prior `11:00`
bucket had `423` browser/e2e (`1,692`/hour) and `3,625` protocol-server
executions (`14,500`/hour); `10:45` had `1,323` browser/e2e (`5,292`/hour),
`3` unit-property (`12`/hour), and `4,680` protocol-server executions
(`18,720`/hour); `10:30` had `146` browser/e2e (`584`/hour) and `2,545`
protocol-server executions (`10,180`/hour); and `10:15` had `122` browser/e2e
(`488`/hour) and `50` protocol-server executions (`200`/hour).
...
findings over about `643.3` runner-hours, or `24.72` per 100 runner-hours.
...
lower-level assertion failures. Current unique candidate output has `1,625`
raw candidates. The by-level rate table attributes `1,617` browser/e2e
...
collaboration UI signals (`686` records) and list-move refresh HTTP (`156`
records, with `2` startup failures). Table stale snapshot HTTP now has `252`
successful records from `429` records and has cleared its `10`-record goal.
...
lifecycle has `303` from `518`, and the adjacent large-post three-user HTTP
profile has `156` successful records from `1,405` records.
...
adjacent large-post three-user HTTP profile has `1,405` records seen with `156`
...
| Goal                                                        | Current | Target |
| ----------------------------------------------------------- | ------: | -----: |
| CDP coverage records next coverage tier                     |   4,153 |  5,000 |
| action ui-type-title next coverage tier                     |   1,341 |  2,000 |
| successful two-user documents next coverage tier            |   1,381 |  2,000 |
| action ui-undo-redo-paragraph next coverage tier            |     565 |  1,000 |
| action ui-type-paragraph next coverage tier                 |   1,706 |  2,000 |
...
the `2026-05-23T11:28:04Z` sample, with the latest completed full pass
recorded at `2026-05-23T11:23:24Z`, about `4.67` minutes behind the sample.
...
still tight: latest CPU utilization is `56.76%`, iowait is `2.01%`,
one-minute load is `36.92` on `64` logical CPUs with `1` blocked task, and
...
bucket evidence, including `0` executions in the trailing partial `11:15`
bucket, `3,625` in `11:00`, `4,680` in `10:45`, `2,545` in `10:30`, `50` in
...
browser/e2e is still underfilled by its own accounting. The latest non-empty
...
and has `298` browser/e2e, `12` unit-property, and zero protocol-server
executions; the prior `11:00` bucket has `423` browser/e2e and `3,625`
protocol-server executions, `10:45` has `1,323` browser/e2e, `3`
unit-property, and `4,680`
protocol-server executions, `10:30` has `146` browser/e2e and `2,545`
protocol-server executions, and `10:15` has `122` browser/e2e and `50`
...
though the adjacent large-post three-user HTTP profile has `1,405` records and
`156` successful records. Many-user active editing is also still `0` at the
