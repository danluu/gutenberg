...
Snapshot generated: `2026-06-21T03:48:07Z`
...
at `2026-06-21T03:46:35Z`. There are `749` monitor
passes from `2026-06-17T17:45:17Z` onward. Cumulative coverage record
observations rose from `544` to
`40,927`, current-scan coverage files rose from
`296` to `1,123`, and
unmet goals are `40` of `136`.
...
`207.4GiB`. Because this snapshot has no
...
The latest CPU/load telemetry sample has CPU utilization `33.7`, load1/load5/load15 `18.09`/`17.07`/`17.67`, and `64` logical cores.

The latest disk sample is `2026-06-21T03:47:25Z`: root has `92.3GiB` free and is `35.0%` used; the data volume has `76.3GiB` free and is `97.8%` used.
...
`11,480,272` individual test or case executions.
The latest cumulative/rate summary is:
`browser-e2e=594 cumulative/4 per-hour; transport-integration=0 cumulative/0 per-hour; unit-property=0 cumulative/0 per-hour; coverage-guided-lower-level=0 cumulative/0 per-hour; backend-api=0 cumulative/0 per-hour; protocol-server=11479678 cumulative/0 per-hour; fuzz-assertion=0 cumulative/0 per-hour; other=0 cumulative/0 per-hour`. Some lower-level rows are approximate
...
`102` bug-output rows, and
`102` unique output candidates.
...
`8` blockers:
`active=1; held=1; runnable=3; terminal=3`.
