...
Snapshot generated: `2026-05-19T12:02:53Z`
...
copied coverage-guided state is rooted at `run-20260519T115237Z`, and the
repaired passes continue through `2026-05-19T12:00:48Z`. Across `2390` monitor
passes from `2026-05-15T01:21:42Z` through `2026-05-19T12:00:48Z`, cumulative
coverage record observations rose from `782` to `251916`. Coverage-file counts
are current-scan counts, not cumulative coverage; the latest current scan is
`2683` files. The monitor's visible likely-real maximum remains `4`.
...
issues, `0` warnings, `416.2G` free memory, `no_progress=0`, and
`headroom=true`. The copied novelty state points at `run-20260519T115237Z`
with two enabled groups, `novelty-ws-real-user-title-body-save-reload` and
`novelty-ws-real-user-editing`, no current run dirs, and empty current summary
startup-failure maps. The live startup/noise sample is clean, but useful
materialization remains narrow and resource-sensitive: current-run triage is
zeroed, while the external-live view sees one root, `10` files, `94`
...
duplicate share is `0.125` and is not the plotted live health signal.
...
`251916`; coverage-file counts are tracked separately and reset when the active
output root rolls. The latest current-output sample is `2683` files from
...
`0`, free memory `416.2G`, `no_progress=0`, and `headroom=true`. The health
...
"novelty-ws-real-user-editing"]`, no current run dirs, empty current summary
startup-failure maps, and current-run duplicate/noise `0` in the monitor
sample. Its current-run triage-yield view is zeroed, while the external-live
triage view sees one root, `10` files, `94` signatures, and `55` merged
likely-real signatures. Useful live materialization is still narrower than a
graph-only "healthy capacity" read.
...
Recent sysstat samples through `2026-05-19T12:00:01Z` show bursty CPU and load.
The latest 25 CPU samples range from `43.60%` to `79.83%` utilization, with the
latest sample at `74.70%`. Over those same 25 samples, one-minute load exceeded
...
newest 1/5/15-minute load sample is `47.62`, `60.58`, and `64.52`; only the
15-minute value is above the `64` logical CPU count. The newest sample has `1`
blocked task.
...
`enabledGroups=["novelty-ws-real-user-title-body-save-reload",
"novelty-ws-real-user-editing"]`, no current run dirs, empty current summary
startup-failure maps, and `10` paused groups, mostly startup/noise plus one
browser-budget rotation. The monitor-level current duplicate share is now `0`,
with plotted current summary startup failures still `0`; the current-run
triage view is zeroed. That keeps this a clean but narrow live-materialization
...
completed inputs. The latest level-mix feedback action, `20260519T113109Z`,
repaired the missing focused
browser controller first, but the autoscaler later shed it under severe
pressure with admission blocked at `load1=102.09/64`. It explicitly says not to
add lower-level capacity. This contradicts both a graph-only
"lower-level work is idle" read and a graph-only "`40` browser/e2e lanes means
...
protocol-server synthesis, `20260519T114904Z`,
...
latest action file is empty; the latest non-empty protocol action,
`20260519T112141Z`, says that harness was implemented and validated with one
passing `seed-attempt-complete` record,
...
The latest collected execution data has about `6,228,544` completed test
executions: `379,795` browser/e2e, `3,006` transport/integration, `5,397,920`
unit-property, and `447,823` coverage-guided-lower-level. The latest partial
15-minute bucket reports about `108` browser/e2e executions/hour and `768`
...
likely-real findings over about `2,788.5` runner-hours, or `28.80` per 100
...
Current unique bug-output candidate rates are: browser/e2e `6,354` candidates
over `2,788.5` runner-hours (`227.87` per 100 runner-hours),
transport/integration `106` over `59.5` runner-hours (`178.14` per 100
runner-hours), coverage-guided lower-level `2` over `20.9` runner-hours
(`9.55` per 100 runner-hours), and unit/property `6` over `61.7` runner-hours
...
`759.3` for coverage-guided lower-level, and `1909.6` for unit/property.
...
| --- | ---: | ---: | ---: | ---: |
| `full` | 1047 | 34 | 0 | 3.2% |
| `multi-reload-lifecycle` | 4499 | 163 | 0 | 3.6% |
| `revision-persistence` | 7698 | 289 | 0 | 3.8% |
| `parser-serialization` | 4636 | 283 | 0 | 6.1% |
| `real-user-editing` | 10040 | 769 | 0 | 7.7% |
| `common-blocks` | 5335 | 512 | 0 | 9.6% |
| `parser-transform` | 6011 | 657 | 0 | 10.9% |
| `long-session-large-doc` | 5114 | 630 | 0 | 12.3% |
...
| `permissions-auth-locks` | 10019 | 4178 | 0 | 41.7% |
| `persistence-no-title` | 4815 | 2088 | 0 | 43.4% |
| `async-server-blocks` | 11079 | 4807 | 0 | 43.4% |
...
| Goal | Current | Target |
| --- | ---: | ---: |
| action reload-post-action next 2000 tier | 1307 | 2000 |
| real-user title save/reload next 1000 tier | 760 | 1000 |
| successful real-user-editing records next 1000 tier | 769 | 1000 |
| real-user body save/reload next 1000 tier | 819 | 1000 |
...
`416.2G` free memory. The copied current novelty state points at
`run-20260519T115237Z` with enabled groups
`novelty-ws-real-user-title-body-save-reload` and
`novelty-ws-real-user-editing`, no current run dirs, empty current summary
startup-failure maps, and a zeroed current-run triage-yield view. The
external-live triage view still sees one root, `10` files, `94` signatures, and
`55` merged likely-real signatures, so the live graph is clean but narrow.
Historical aggregate duplicate/noise is `0.125`, but it is context
...
values are `47.62`, `60.58`, and `64.52`; only the 15-minute value is above the
logical CPU count. The newest sample has `1` blocked task.
...
strict/browser capacity instead. Its latest feedback action repaired the
missing focused controller first, but the autoscaler later shed it under severe
pressure and admission was blocked at `load1=102.09/64`. The refreshed graph
shows browser/e2e concentration, but raw browser/e2e lane count still should
not be treated as useful capacity without materialization and pressure checks.
...
exist, including a latest native-harness action that implemented and
smoke-validated rich-text CRDT multiblock coverage-guided events, an active
