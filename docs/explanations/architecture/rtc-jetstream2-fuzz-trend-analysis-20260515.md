...
Snapshot generated: `2026-06-03T11:14:54Z`
...
The latest CPU/load telemetry sample has CPU utilization `20.5`, load1/load5/load15 `14.42`/`15.69`/`15.36`, and `64` logical cores.

The latest disk sample is `2026-06-03T11:12:16Z`: root has `116.2GiB` free and is `18.1%` used; the data volume has `219.7GiB` free and is `93.8%` used.

The latest graph-counted fuzzing mix is: `browser-e2e=31 lanes/31 groups; unit-property=1 lanes/1 groups; coverage-guided-lower-level=1 lanes/1 groups; protocol-server=1 lanes/1 groups`.
...
`18,635,897` individual test or case executions.
The latest cumulative/rate summary is:
`browser-e2e=693989 cumulative/280 per-hour; transport-integration=0 cumulative/0 per-hour; unit-property=5838464 cumulative/0 per-hour; coverage-guided-lower-level=623766 cumulative/0 per-hour; backend-api=0 cumulative/0 per-hour; protocol-server=11479678 cumulative/0 per-hour; fuzz-assertion=0 cumulative/0 per-hour; other=0 cumulative/0 per-hour`. Some lower-level rows are approximate
...
`345` triaged bug-finding rows,
`203` unique likely-real findings,
`1,788` bug-output rows, and
`1,448` unique output candidates.

PR-controller graph inputs are present. The controller snapshot has
`22` items:
`deferred-family/needs-product-decision/high=1; deferred-family/diagnostic/medium=2; ready-product-pr/held-by-controller/high=4; ready-product-pr/published/high=13; ready-product-pr/superseded-by-repair/low=1; runtime-gated-pr/runtime-held-consumed/high=1`. The artifact index contributes
...
Current enabled groups: `novelty-http-title-reload-convergence,novelty-http-same-user-stale-draft,novelty-http-existing-post-crdt-metadata,novelty-ws-collaboration-ui-signals,novelty-http-large-post-lifecycle-completion,novelty-ws-parser-serialization,novelty-ws-many-user-lifecycle-completion,novelty-ws-permissions-auth-locks,novelty-ws-media-cross-entity,novelty-ws-long-session-large-doc`.
