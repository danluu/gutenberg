...
Snapshot generated: `2026-05-23T14:24:21Z`
...
The graph-refresh pipeline is current through `2026-05-23T14:17:59Z` for
monitor passes, with the latest current-run accounting sample at
`2026-05-23T14:23:05Z`. The latest completed full duplicate/noise accounting
pass recorded in the active row is `2026-05-23T14:17:59Z`, so the sampled
full-pass lag is `5.1` minutes. The monitor has
`4,242` passes from `2026-05-15T01:21:42Z` onward, cumulative coverage record
observations are `286,210`, current-scan coverage files are `5,176`, and the
...
is `69.71%`, with `3.28%` iowait. Latest load averages are `55.22`,
`57.19`, and `57.65` on `64` logical CPUs, with `0` blocked tasks. The latest
disk sample has `88.9GiB` free on `/` and `186.8GiB`
...
polling row from `2026-05-23T14:03:44Z`, and one stale
...
HTTP/WS row set sampled at `2026-05-23T14:17:44Z`, a fresh
...
at `2026-05-23T12:47:07Z`, and the protocol-server row sampled at
`2026-05-23T14:03:44Z`. It still does not show
graph-counted backend/API, transport-integration, fuzz-assertion, or fresh
sustained parser lower-level capacity.

The execution counter has `17,124,398` estimated individual executions. The
latest plotted `2026-05-23T14:15:00Z` bucket is a partial bucket with
`299` browser/e2e executions and `3,240` protocol-server executions, with
zero unit-property, coverage-guided-lower-level, backend-api,
transport-integration, and fuzz-assertion executions. The prior `14:00` bucket
had `724` browser/e2e and `5,223` protocol-server executions; `13:45` had
`1,026` browser/e2e and `5,425` protocol-server executions; `13:30` had
`1,259` browser/e2e and `4,140` protocol-server executions; and `13:15` had
...
at `2026-05-23T14:23:05Z`; the latest completed full duplicate/noise
accounting pass recorded in that row is `2026-05-23T14:17:59Z`, so the
...
stable at `88.9GiB` free and `42.2%` used; the data volume has `186.8GiB`
...
sampled at `2026-05-23T14:17:44Z`;
...
`2026-05-23T14:03:44Z`. The lone `coverage-guided-lower-level` row is stale
...
`2026-05-23T14:03:44Z`. Protocol-server executions are `3,240` in the latest
partial `14:15` bucket, while that same bucket has `299` browser/e2e executions.
The prior `14:00` bucket has `5,223` protocol-server executions and `724`
browser/e2e executions. Protocol-server has `5,425` in `13:45`, `4,140` in
`13:30`, `25` in `13:15`, `0` in `13:00`, `0` in `12:45`, `25` in `12:30`,
...
Latest cumulative totals are approximately `121,796` browser/e2e, `5,837,141`
unit-property, `458,097` coverage-guided lower-level, and `10,707,364`
...
The latest `2026-05-23T14:15:00Z` bucket is a partial bucket with `299`
browser/e2e executions (`1,196`/hour) and `3,240` protocol-server executions
...
`14:00` bucket had `724` browser/e2e (`2,896`/hour) and `5,223`
protocol-server executions (`20,892`/hour); `13:45` had `1,026` browser/e2e
(`4,104`/hour) and `5,425` protocol-server executions (`21,700`/hour);
`13:30` had `1,259` browser/e2e (`5,036`/hour) and `4,140` protocol-server
executions (`16,560`/hour); and `13:15` had `1,232` browser/e2e
...
findings over about `661.2` runner-hours, or `24.35` per 100 runner-hours.
...
lower-level assertion failures. Current unique candidate output has `1,672`
raw candidates. The by-level rate table attributes `1,664` browser/e2e
...
list-move refresh HTTP (`199` records, with `3` startup failures) and
collaboration UI signals (`686` records). Table stale snapshot HTTP now has
`351` successful records from `557` records, with `9` startup failures, and
...
revision persistence has `168` from `315`, multi-reload lifecycle has `421` from
`690`, and the adjacent large-post three-user HTTP profile has `175`
successful records from `1,537` records with `12` startup failures.
...
adjacent large-post three-user HTTP profile has `1,537` records seen with `175`
...
| Goal                                                        | Current | Target |
| ----------------------------------------------------------- | ------: | -----: |
| CDP coverage records next coverage tier                     |   4,561 |  5,000 |
| action ui-type-title next coverage tier                     |   1,405 |  2,000 |
| successful two-user documents next coverage tier            |   1,556 |  2,000 |
| action ui-undo-redo-paragraph next coverage tier            |     565 |  1,000 |
| action ui-type-paragraph next coverage tier                 |   1,771 |  2,000 |
...
the `2026-05-23T14:23:05Z` sample, with the latest completed full pass
recorded at `2026-05-23T14:17:59Z` and `5.1` minutes of sampled
completed-pass lag.
...
still tight: latest CPU utilization is `69.71%`, iowait is `3.28%`,
one-minute load is `55.22` on `64` logical CPUs with `0` blocked tasks, and
...
bucket evidence, including `3,240` executions in the latest partial `14:15`
bucket, `5,223` in the prior `14:00` bucket, `5,425` in `13:45`, `4,140` in
`13:30`, and `25` in `13:15`.
...
`299` browser/e2e and `3,240` protocol-server executions. The prior `14:00`
bucket has `724` browser/e2e and `5,223` protocol-server executions; `13:45`
has `1,026` browser/e2e and `5,425` protocol-server executions; `13:30` has
`1,259` browser/e2e and `4,140` protocol-server executions; and `13:15` has
`1,232` browser/e2e, `3` unit-property, and `25` protocol-server executions.
...
though the adjacent large-post three-user HTTP profile has `1,537` records and
`175` successful records. Many-user active editing is also still `0` at the
