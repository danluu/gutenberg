...
Snapshot generated: `2026-05-23T15:56:00Z`
...
The graph-refresh pipeline is current through `2026-05-23T15:50:18Z` for
monitor passes, with the latest current-run accounting sample at
`2026-05-23T15:54:05Z`. The latest completed full duplicate/noise accounting
pass recorded in the active row is `2026-05-23T15:50:18Z`, so the sampled
full-pass lag is `3.8` minutes. The monitor has
`4,255` passes from `2026-05-15T01:21:42Z` onward, cumulative coverage record
observations are `286,599`, current-scan coverage files are `5,568`, and the
...
graph is later and the trusted active row is now at duplicate share `0.2`
...
is `56.56%`, with `2.95%` iowait. Latest load averages are `39.69`,
`41.80`, and `44.98` on `64` logical CPUs, with `16` blocked tasks. The latest
disk sample has `88.8GiB` free on `/` and `149.6GiB`
free on `/media/volume/danluu-fuzz-data`; the data volume is `95.8%` used.
...
`2026-05-23T15:50:02Z`, the `gap-booster-20260523T124703Z` row set sampled at
...
The execution counter has `17,155,440` estimated individual executions. The
latest plotted `2026-05-23T15:45:00Z` bucket is a partial bucket with
`85` browser/e2e executions and `3,780` protocol-server executions, with zero
...
at `2026-05-23T15:54:05Z`; the latest completed full duplicate/noise
accounting pass recorded in that row is `2026-05-23T15:50:18Z`, so the
sampled full-pass lag is `3.8` minutes. The row reports
...
product-evidence signatures, with trusted current-run accounting and `3.8`
...
stable at `88.8GiB` free and `42.3%` used; the data volume has `149.6GiB`
free and is `95.8%` used, so output-size budgeting still matters.
...
`2026-05-23T15:50:02Z`; `1` focused-shard lane from
...
`2026-05-23T15:34:21Z`. Protocol-server executions are `3,780` in the latest
partial `15:45` bucket, while that same bucket has `85` browser/e2e
...
Latest cumulative totals are approximately `123,027` browser/e2e, `5,837,147`
unit-property, `458,097` coverage-guided lower-level, and `10,737,169`
...
The latest `2026-05-23T15:45:00Z` bucket is a partial bucket with `85`
browser/e2e executions (`340`/hour) and `3,780` protocol-server executions
...
findings over about `672.4` runner-hours, or `24.39` per 100 runner-hours.
...
lower-level assertion failures. Current unique candidate output has `1,715`
raw candidates. The by-level rate table attributes `1,707` browser/e2e
...
list-move refresh HTTP (`221` records, with `3` startup failures) and
collaboration UI signals (`686` records). Table stale snapshot HTTP now has
`402` successful records from `624` records, with `10` startup failures, and
...
revision persistence has `168` from `315`, multi-reload lifecycle has `478`
from `775`, and the adjacent large-post three-user HTTP profile has `183`
successful records from `1,606` records with `13` startup failures.
...
adjacent large-post three-user HTTP profile has `1,606` records seen with `183`
...
| Goal                                                        | Current | Target |
| ----------------------------------------------------------- | ------: | -----: |
| action ui-type-title next coverage tier                     |   1,440 |  2,000 |
| action ui-undo-redo-paragraph next coverage tier            |     565 |  1,000 |
| successful two-user documents next coverage tier            |   1,644 |  2,000 |
| CDP coverage records next coverage tier                     |   4,770 |  5,000 |
| action ui-type-paragraph next coverage tier                 |   1,806 |  2,000 |
...
`duplicateShareCurrent` `0.2`, summary startup failures `0`, `5` current,
actionable, and product-evidence signatures, and `0.2` top-duplicate share
in the `2026-05-23T15:54:05Z` sample, with the latest completed full pass
recorded at `2026-05-23T15:50:18Z` and `3.8` minutes of sampled
completed-pass lag.
...
still tight: latest CPU utilization is `56.56%`, iowait is `2.95%`,
one-minute load is `39.69` on `64` logical CPUs with `16` blocked tasks, and
the data volume is `95.8%` used. Optional browser admission should still
...
bucket evidence, including `3,780` executions in the latest partial `15:45`
...
`85` browser/e2e and `3,780` protocol-server executions.
...
though the adjacent large-post three-user HTTP profile has `1,606` records and
`183` successful records. Many-user active editing is also still `0` at the
