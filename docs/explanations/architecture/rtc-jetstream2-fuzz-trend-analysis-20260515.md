...
Snapshot generated: `2026-07-05T18:49:36Z`
...
at `2026-07-05T18:43:53Z`. There are `2,319` monitor
passes from `2026-06-23T12:08:03Z` onward. Cumulative coverage record
observations rose from `515` to
`88,648`, current-scan coverage files rose from
`890` to `5,636`, and
unmet goals are `15` of `162`.
...
`203.4GiB`. Because this snapshot has no
...
The latest CPU/load telemetry sample has CPU utilization `43.4`, load1/load5/load15 `29.89`/`29.53`/`28.33`, and `64` logical cores.

The latest disk sample is `2026-07-05T18:48:08Z`: root has `66.1GiB` free and is `53.4%` used; the data volume has `280.8GiB` free and is `92.1%` used.
...
`13,133,714` individual test or case executions.
The latest cumulative/rate summary is:
`browser-e2e=22654 cumulative/112 per-hour; transport-integration=0 cumulative/0 per-hour; unit-property=0 cumulative/0 per-hour; coverage-guided-lower-level=0 cumulative/0 per-hour; backend-api=0 cumulative/0 per-hour; protocol-server=13111060 cumulative/0 per-hour; fuzz-assertion=0 cumulative/0 per-hour; other=0 cumulative/0 per-hour`. Some lower-level rows are approximate
...
`246` triaged bug-finding rows,
`109` unique likely-real findings,
`701` bug-output rows, and
`589` unique output candidates.
...
`12` blockers:
`active=2; runnable=4; terminal=6`.
...
Current enabled groups: `novelty-ws-same-user-lifecycle,novelty-http-plain-editor-product-smoke,novelty-http-real-world-editor-usability,novelty-ws-same-user-separate-context-lifecycle,novelty-http-rtc-reference-oracle,novelty-ws-collaboration-ui-signals,novelty-http-existing-post-crdt-metadata,novelty-ws-multi-reload-lifecycle,novelty-ws-common-blocks,novelty-ws-block-gauntlet,novelty-ws-long-session-large-doc,novelty-http-persistence-probe`.
