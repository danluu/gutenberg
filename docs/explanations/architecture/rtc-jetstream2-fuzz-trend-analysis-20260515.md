...
Snapshot generated: `2026-05-22T17:29:50Z`
...
The monitor data is current through `2026-05-22T17:27:34Z`, and the latest
current-run accounting row was sampled at `2026-05-22T17:28:22Z` for
...
The completed full pass is about `0.80` minutes old in the sampled row. The
monitor has `4,064` passes from `2026-05-15T01:21:42Z` onward. Cumulative
coverage record observations are `279,336`; current-scan coverage files are
`2,446`. The parsed coverage-goal table has `17` unmet target rows out of
`137`.

The live duplicate/noise signal is current-output-dir accounting. The latest
trusted active-run row has completed-pass `duplicateShareCurrent` `0.3333`,
summary startup failures `0`, `3` current-run signatures, `3` actionable
signatures, `3` product-evidence signatures, and top duplicate share
`0.3333`. That is a complete current-run denominator, but it is still a small
...
three-signature duplicate denominator, so the right live status is
...
`391.7G` free memory; the latest disk sample has `90.6GiB` free on `/` and
`485.9GiB` free on `/media/volume/danluu-fuzz-data`. Latest CPU utilization
is `66.16%`, with `6.12%` iowait. Latest load averages are `61.32`, `59.96`,
and `56.26` on `64` logical CPUs, with `0` blocked tasks in the same sample.
...
that remains the latest non-empty action evidence. The native-harness action
validates the HTTP polling lower-level harness. The newest native-harness
synthesis disagrees on target priority and selects block parser/serialization
as the next isolated lower-level harness. The refreshed execution bucket still
has zero current coverage-guided-lower-level executions. `transport-integration`,
`backend-api`, and standalone fuzz-only assertion work have no current
graph-counted lane.

The execution counter has `16,833,604` estimated individual executions. These
...
`2026-05-22T17:15:00Z` has `105` browser/e2e executions, `2,720`
unit-property executions, and `1,920` protocol-server executions, about
`420`/hour, `10,880`/hour, and `7,680`/hour. The latest nonzero
...
`2026-05-22T17:28:22Z`; it reports `current_run_metrics_trusted` `TRUE`,
...
`2026-05-22T17:27:34Z`. This row carries completed
`duplicateShareCurrent` `0.3333` and summary startup failures `0`, with `3`
current-run signatures, `3` actionable signatures, `3` product-evidence
signatures, and top duplicate share `0.3333`. That is complete current-run
accounting, but it is still a small current/actionable signature denominator,
...
has only three current/actionable signatures, the refreshed graph rejects a
...
The latest enabled-group event count is `1,764`. Current enabled groups are
...
graph-counted browser/e2e rows include five coverage-guided rows, one
...
large-post readiness/completion, existing-post CRDT metadata, HTTP persistence
...
backfill toward the `24`-lane floor. The refreshed graph sees `15`, still
below that floor.
...
confirmed. That conflicts with the latest native-harness action, which says
the HTTP polling lower-level harness was implemented and validated with
...
`2026-05-22T17:15:00Z`. The protocol action says the HTTP polling REST harness
...
legacy batch-count fields. The latest totals are approximately `79,270`
browser/e2e, `5,754,368` unit-property, `458,097`
coverage-guided lower-level, and `10,541,869` protocol-server executions.
Transport-integration, backend-api, fuzz-assertion, and other buckets are `0`
in the current reconstructed table.

The latest 15-minute bucket at `2026-05-22T17:15:00Z` is partial and has
`105` browser/e2e executions (`420`/hour), `2,720` unit-property executions
(`10,880`/hour), and `1,920` protocol-server executions (`7,680`/hour);
...
findings over about `556.6` runner-hours, or `23.36` per 100 runner-hours.
...
lower-level assertion failures. Current unique candidate output has `1,150`
total candidates: `1,142` browser/e2e candidates, `6` unit-property
...
`57`, code-editor smoke has `64`, parser transform has `68`, three-user late
...
three-user HTTP profile has `658` records seen with `57` successful records.
...
sampled at `2026-05-22T17:28:22Z`; the latest completed full pass and
completed monitor pass were both at `2026-05-22T17:27:34Z`, with
`full_pass_pending` `FALSE`, `pending_until_first_pass` `FALSE`, and
`current_run_metrics_trusted` `TRUE`. The row carries completed
`duplicateShareCurrent` `0.3333` and summary startup failures `0`, with a
current-run signature/actionable-signature denominator of `3`/`3`. That is
complete accounting, but it is still a small-denominator watch rather than a
...
accounting has only six current/actionable signatures, the persona-loop
...
below the core count: latest CPU utilization is `66.16%`, with `6.12%` iowait;
load is `61.32`, `59.96`, and `56.26` on `64` logical CPUs, with `0` blocked
tasks.
...
lower-level target, while the latest action validated HTTP polling with
`productYield=true` and root/lane events. The latest non-empty level-mix feedback-action
reports a bounded lower-level HTTP polling readiness run with `48` executions
and product yield, which conflicts with the graph's lack of current lower-level
execution buckets; keep that as action evidence until the next graph-visible
row appears.

Browser/e2e remains the only level producing triaged likely-real findings in
the committed triage-output metric. The latest execution bucket is partial:
the `2026-05-22T17:15:00Z` bucket has `105` browser/e2e executions, `2,720`
unit-property executions, and `1,920` protocol-server executions. Lower-level
...
though the adjacent large-post three-user HTTP profile has `658` records and
`57` successful records. Many-user active editing is also still `0` at the
