...
Snapshot generated: `2026-07-10T15:44:55Z`
...
The latest CPU/load telemetry sample has CPU utilization `51.1`, load1/load5/load15 `42.93`/`38.23`/`40.6`, and `64` logical cores.

The latest disk sample is `2026-07-10T15:43:56Z`: root has `44.6GiB` free and is `68.5%` used; the data volume has `292.1GiB` free and is `91.8%` used.

The latest graph-counted fuzzing mix is: `browser-e2e=20 lanes/17 groups; unit-property=1 lanes/1 groups; protocol-server=1 lanes/1 groups`.
...
`13,122,534` individual test or case executions.
The latest cumulative/rate summary is:
`browser-e2e=11294 cumulative/1492 per-hour; transport-integration=0 cumulative/0 per-hour; unit-property=180 cumulative/0 per-hour; coverage-guided-lower-level=0 cumulative/0 per-hour; backend-api=0 cumulative/0 per-hour; protocol-server=13111060 cumulative/0 per-hour; fuzz-assertion=0 cumulative/0 per-hour; other=0 cumulative/0 per-hour`. Some lower-level rows are approximate
...
`188` triaged bug-finding rows,
`36` unique likely-real findings,
`1,641` bug-output rows, and
`1,548` unique output candidates.

PR-controller graph inputs are present. The controller snapshot has
`27` items:
`deferred-family/downscoped/medium=2; ready-product-pr/held-by-controller/high=3; ready-product-pr/published/high=14; ready-product-pr/superseded/low=1; ready-product-pr/superseded-by-repair/low=1; repair-branch-adoption/already-in-release-candidate/high=2; repair-branch-adoption/needs-validation/high=1; repair-branch-adoption/published-standalone/high=1; repair-branch-adoption/stale-candidate-base/high=1; runtime-gated-pr/runtime-held-consumed/high=1`. The artifact index contributes
`5,042` rows. The critical-path snapshot has
`11` blockers:
`active=4; held=1; runnable=2; terminal=4`.
...
Current enabled groups: `novelty-http-large-post-lifecycle,novelty-http-title-reload-convergence,novelty-http-same-user-stale-draft,novelty-http-table-stale-snapshot,novelty-http-existing-post-crdt-metadata`.
...
The current controller has `4`
publishable branches and `1,323` net LOC
...
- Because `current_run_accounting.csv` is absent, verify the current output root
  directly before making broad claims from `duplicateShareCurrent=0.0278`.
