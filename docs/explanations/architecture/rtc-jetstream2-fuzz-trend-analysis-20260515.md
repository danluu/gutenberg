...
Snapshot generated: `2026-05-17T04:50:10Z`
...
- latest copied novelty state:
  ...
  `lastUpdatedAt=2026-05-17T04:43:26.334Z`)
...
`1839` monitor passes from `2026-05-15T01:21:42Z` through
`2026-05-17T04:43:26Z`, coverage files grew from `272` to `40176`, a delta of
`39904`. The monitor's visible likely-real count reached `2`.
...
dedupe migration. Its "current-run clean" premise is older than this graph
refresh: the latest current-output triage now shows `2` visible likely-real
product-evidence signatures. That contradiction narrows the readout rather than
closing it: product-evidence failures should stay visible, while stale
control-plane startup-noise recycling still needs the scheduler fix.
...
`4` signatures, `2` visible likely-real failures, and top duplicate family
...
synthesis rejects closure and its earlier clean-current-run statement is no
longer true for this snapshot. Downstream no-product suppression is not enough
if the scheduler can recycle noisy producer lanes, and product-evidence failures
must remain visible rather than being suppressed by family name alone.

![CPU utilization over time](rtc-jetstream2-fuzz-trends-20260515/plots/cpu-utilization-over-time.png)

![Load average over time](rtc-jetstream2-fuzz-trends-20260515/plots/load-average-over-time.png)

Recent sysstat samples through `2026-05-17T04:40:01Z` show heavy CPU and load
pressure. The latest 25 CPU samples range from `55.0%` to `96.5%` utilization,
with the latest sample at `81.7%`. One-minute load exceeded the logical CPU
count in `19` of those `25` sampled windows, while the latest sampled
1/5/15-minute load is `79.00`, `86.89`, and `104.32` against `64` logical
...
context, not the plotted live signal. The current-output state has `4`
signatures across `2` current roots/files and `2` visible likely-real failures.
...
strict no-product noise; the refreshed current-output product-evidence failures
also reject any blanket suppression by family name.
...
The latest collected execution data has about `3,475,615` completed test
executions: `97,622` browser/e2e, `3,006` transport/integration, `2,998,444`
unit-property, and `376,543` coverage-guided-lower-level. The latest
15-minute bucket reports about `196` browser/e2e test executions/hour,
`43,344` unit-property test executions/hour, `8,576`
...
`292` unique likely-real outputs over about `1,685.7` runner-hours, or `17.32`
...
Current unique bug-output candidate rates are: browser/e2e `4,401` candidates
over `1,685.7` runner-hours (`261.08` per 100 runner-hours),
transport/integration `106` over `59.5` runner-hours (`178.14` per 100
runner-hours), coverage-guided lower-level `2` over `17.6` runner-hours
(`11.36` per 100 runner-hours), and unit/property `1` over `12.9` runner-hours
(`7.72` per 100 runner-hours). `backend-api`, `protocol-server`, standalone
...
are about `4,710.6` for browser/e2e,
`4,537.5` for transport/integration, `902.9` for coverage-guided lower-level,
and `826.3` for unit/property.
...
| --- | ---: | ---: | ---: | ---: |
| `full` | 840 | 18 | 0 | 2.1% |
| `multi-reload-lifecycle` | 3060 | 97 | 0 | 3.2% |
| `revision-persistence` | 4102 | 141 | 0 | 3.4% |
| `parser-serialization` | 2951 | 137 | 0 | 4.6% |
| `real-user-editing` | 5945 | 435 | 0 | 7.3% |
| `parser-transform` | 3904 | 377 | 0 | 9.7% |
| `common-blocks` | 3713 | 374 | 0 | 10.1% |
| `long-session-large-doc` | 2361 | 384 | 0 | 16.3% |
| `structure` | 536 | 95 | 0 | 17.7% |
| `block-gauntlet` | 5827 | 1048 | 0 | 18.0% |
| `persistence-no-title` | 2847 | 514 | 0 | 18.1% |
...
| Goal | Current | Target |
| --- | ---: | ---: |
| real-user title save/reload next coverage tier | 297 | 500 |
| real-user body save/reload next coverage tier | 356 | 500 |
| action ui-heading-shortcut next coverage tier | 742 | 1000 |
| action reload-post-action next coverage tier | 750 | 1000 |
...
232 ran from `2026-05-17T04:36:36Z` to `2026-05-17T04:44:58Z`, and the next
review started at `2026-05-17T04:45:03Z`.
...
`5200005`, `1060015`, and `7510029` should run now. The matching
feedback-action, `20260517T042625Z`, updated the split document, completed
finalization and zero-report audits, and launched PR07C validation plus owner
reducers for `5200005`, `1060015`, and `7510029`.
...
current roots/files, `4` current signatures, and `2` visible likely-real
...
leak is reproduced. Its clean-current-run statement conflicts with the refreshed
current-output triage, which now has `2` visible likely-real product-evidence
signatures, so this report keeps those failures visible while preserving the
control-plane-noise warning.
...
latest execution bucket has about `196` browser/e2e, `43,344` unit-property,
and `8,576` coverage-guided-lower-level test executions/hour, with `0`
