...
Snapshot generated: `2026-06-04T23:38:53Z`
...
The latest CPU/load telemetry sample has CPU utilization `18.1`, load1/load5/load15 `12.33`/`14.94`/`17.5`, and `64` logical cores.

The latest disk sample is `2026-06-04T23:36:56Z`: root has `115.3GiB` free and is `18.7%` used; the data volume has `275.6GiB` free and is `92.2%` used.

The latest graph-counted fuzzing mix is: `browser-e2e=29 lanes/29 groups; unit-property=1 lanes/1 groups; coverage-guided-lower-level=1 lanes/1 groups; protocol-server=1 lanes/1 groups`.
...
`18,649,767` individual test or case executions.
The latest cumulative/rate summary is:
`browser-e2e=707859 cumulative/316 per-hour; transport-integration=0 cumulative/0 per-hour; unit-property=5838464 cumulative/0 per-hour; coverage-guided-lower-level=623766 cumulative/0 per-hour; backend-api=0 cumulative/0 per-hour; protocol-server=11479678 cumulative/0 per-hour; fuzz-assertion=0 cumulative/0 per-hour; other=0 cumulative/0 per-hour`. Some lower-level rows are approximate
...
`202` triaged bug-finding rows,
`72` unique likely-real findings,
`1,404` bug-output rows, and
`1,069` unique output candidates.

PR-controller graph inputs are present. The controller snapshot has
`23` items:
`deferred-family/needs-product-decision/high=1; deferred-family/diagnostic/medium=2; ready-product-pr/held-by-controller/high=4; ready-product-pr/publishable/high=1; ready-product-pr/published/high=13; ready-product-pr/superseded-by-repair/low=1; runtime-gated-pr/runtime-held-consumed/high=1`. The artifact index contributes
...
Current enabled groups: `novelty-http-rtc-reference-oracle,novelty-http-large-post-lifecycle,novelty-http-title-reload-convergence,novelty-http-same-user-stale-draft,novelty-ws-parser-serialization,novelty-http-large-post-lifecycle-completion`.
