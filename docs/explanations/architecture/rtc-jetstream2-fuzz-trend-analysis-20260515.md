...
Snapshot generated: `2026-05-22T15:24:31Z`
...
The monitor data is current through `2026-05-22T15:23:33Z`, and the latest
current-run accounting row was sampled at `2026-05-22T15:23:33Z` for
...
monitor has `4,016` passes from `2026-05-15T01:21:42Z` onward. Cumulative
coverage record observations are `278,800`; current-scan coverage files are
`2,111`. The parsed coverage-goal
...
`403.9G` free memory; the latest disk sample has `90.7GiB` free on `/` and
`490.8GiB` free on `/media/volume/danluu-fuzz-data`. Latest CPU utilization is
`50.39%`, with `3.25%` iowait. Latest load averages are `38.45`, `37.89`,
and `41.84` on `64` logical CPUs, with `2` blocked tasks in the same sample.
...
The execution counter has `16,794,616` estimated individual executions. These
...
`2026-05-22T15:15:00Z` has `34` browser/e2e executions, `1,760` unit-property
executions, and `1,260` protocol-server executions, about `136`/hour,
`7,040`/hour, and `5,040`/hour. The preceding `2026-05-22T15:00:00Z` bucket had
...
`8,260`/hour. The prior `2026-05-22T14:45:00Z` bucket had `76` browser/e2e
executions, `2,368` unit-property executions, and `565` protocol-server
executions. The latest nonzero protocol-server bucket is
`2026-05-22T15:15:00Z`. The latest nonzero
...
critical-path executor has `7` blockers: `3` runnable, `1` queued, `1` held,
and `2` terminal/downscoped. The latest PR-split feedback
...
pending. Benchmark-canary exact-stack repair is runnable, the seed `5200005`
reducer is runnable but queued for later, productive-analysis feedback is
...
`run-20260522T151953Z` was taken at `2026-05-22T15:23:33Z`; it reports
...
`2026-05-22T15:23:33Z`, and the latest completed current-run accounting pass
was at `2026-05-22T15:23:33Z`. This row carries completed
...
graph is now on `run-20260522T151953Z` with a trusted pass and a live
duplicate/noise denominator of `0` current/actionable signatures, `0`
product-evidence signatures, and top duplicate share `0`. The right reading is
no broad measured product duplicate/noise storm, plus a current-run accounting
watch until the active run accumulates a larger actionable-signature
denominator.
...
around `86.1%` used and root is around `41.1%` used. Root pressure remains
...
The latest enabled-group event count is `1,733`. Current enabled groups are
...
coverage-guided media cross-entity lane, five coverage-guided HTTP lanes, one
focused-shards many-user-scale row, six gap-booster rows, and three strict HTTP
expansion rows for HTTP persistence, same-user stale draft, and large HTTP
lifecycle. Those rows cover media cross-entity, large-post three-user HTTP
lifecycle, title reload, existing-post persistence, many-user lifecycle,
...
agrees with the capacity concern: it has only `16` current browser/e2e lanes,
still below the `24`-lane floor, and protocol-server telemetry is present.
Current supervisor rows show several HTTP browser rows, but still no
backend/API as live residency, though the latest execution buckets still have
recent browser and protocol events. The graph still rejects treating
...
that narrow V8/Jest target before rich-text CRDT, table query-array CRDT, or
HTTP polling manager expansion. Parser serialization has `90` records and `20`
successful records in the graph, and the latest native-harness action says the
block-parser harness was implemented and smoke-validated with V8 coverage and
required `coverage-guided-lower-level` event accounting. It is still not
current graph-counted lower-level residency. The only graph-counted
...
validation protocol-server row and `1,260` executions in the latest nonzero
`2026-05-22T15:15:00Z` bucket, after `2,065` executions in the
`2026-05-22T15:00:00Z` bucket and `565` in the `2026-05-22T14:45:00Z` bucket.
The newest protocol action says the HTTP polling REST harness was implemented
and validated with root/lane `seed-attempt-complete` events and
`fuzzLevel:"protocol-server"` metadata, so the graph and action agree that
protocol-server telemetry is present. The refreshed graph
accepts focused-browser, coverage-guided browser, strict HTTP, and protocol-server
...
legacy batch-count fields. The latest totals are approximately `78,972`
browser/e2e, `5,730,848` unit-property, `458,097` coverage-guided lower-level,
and `10,526,699` protocol-server executions. Transport-integration,
backend-api, fuzz-assertion, and other buckets are `0` in the current
reconstructed table.

The latest 15-minute bucket at `2026-05-22T15:15:00Z` is partial and has
`34` browser/e2e executions (`136`/hour), `1,760` unit-property executions
(`7,040`/hour), and `1,260` protocol-server executions (`5,040`/hour);
...
`2,944` unit-property executions (`11,776`/hour), and `2,065` protocol-server
executions (`8,260`/hour). The prior `2026-05-22T14:45:00Z` bucket had `76`
browser/e2e executions (`304`/hour), `2,368` unit-property executions
(`9,472`/hour), and `565` protocol-server executions (`2,260`/hour). The
latest nonzero protocol-server bucket is `2026-05-22T15:15:00Z`. The
latest nonzero
coverage-guided lower-level bucket remains
...
findings over about `541.2` runner-hours, or `22.54` per 100 runner-hours.
...
lower-level assertion failures. Current unique candidate output has `1,117`
total candidates: `1,109` browser/e2e candidates, `6` unit-property
...
three-user HTTP profile has `530` records seen with `44` successful records.
...
(`benchmark-canary-fuzzer-gap` exact-stack repair,
`productive-analysis-action`, and `seed-5200005-reducer`), one queued
owner-evidence blocker (`pr07c-owner-matrix`), one held deferred-family blocker
...
benchmark-canary exact-stack repair runnable, seed `5200005` queued for later,
PR07C owner-matrix queued, productive-analysis runnable, and reload-hydration
gated by a single-flight manifest. The repeated no-progress table currently
has three `benchmark-canary-fuzzer-gap` rows: `exact_stack_promotion_blocked_without_active_repair`,
...
`2026-05-22T15:23:33Z`; the latest completed monitor pass was at
`2026-05-22T15:23:33Z`, and the latest completed current-run accounting pass
was at `2026-05-22T15:23:33Z`, with `full_pass_pending` `FALSE`,
...
`run-20260522T151953Z` with top duplicate share `0`. The live health issue is
...
`50.39%`, with `3.25%` iowait; load is `38.45`, `37.89`, and `41.84` on
`64` logical CPUs, with `2` blocked tasks. The graph-counted fuzzing mix is
browser/e2e-heavy: `16` browser/e2e lanes, plus one lane each for
...
The refreshed graph confirms current coverage-guided browser HTTP and
media rows, focused many-user-scale, gap-booster rows, strict HTTP rows, and
...
`16` current browser/e2e lanes, and the synthesis still rejects trusting raw
...
isolated lower-level harness and says to land that bounded V8/Jest target
before broader lower-level expansion; the action says that harness was
smoke-validated, but it is not graph-counted live lower-level residency.
Protocol-server HTTP polling has current graph telemetry and `1,260`
...
`2026-05-22T15:15:00Z` bucket has `34` browser/e2e executions, `1,760`
unit-property executions, and `1,260` protocol-server executions. The preceding
`2026-05-22T15:00:00Z` bucket shows `83` browser/e2e executions, `2,944`
unit-property executions, and `2,065` protocol-server executions; the latest
nonzero protocol-server bucket is `2026-05-22T15:15:00Z`. Lower-level
...
ready-product rows, runnable benchmark-canary exact-stack repair, queued seed
...
runnable, seed `5200005` is runnable but queued for later, and the no-progress
table now rejects three benchmark-canary artifacts.
