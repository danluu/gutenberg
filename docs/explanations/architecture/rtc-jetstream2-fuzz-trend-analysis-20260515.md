...
Snapshot generated: `2026-07-05T02:41:33Z`
...
share is `0.1786`. Current summary startup
failures are `0`, quality issues are
`0`, and free memory is
`204.3GiB`. Because this snapshot has no
...
The latest CPU/load telemetry sample has CPU utilization `27.9`, load1/load5/load15 `24.74`/`23.21`/`22.83`, and `64` logical cores.

The latest disk sample is `2026-07-05T02:39:48Z`: root has `66.1GiB` free and is `53.4%` used; the data volume has `280.0GiB` free and is `92.1%` used.
...
`13,142,798` individual test or case executions.
The latest cumulative/rate summary is:
`browser-e2e=31738 cumulative/224 per-hour; transport-integration=0 cumulative/0 per-hour; unit-property=0 cumulative/0 per-hour; coverage-guided-lower-level=0 cumulative/0 per-hour; backend-api=0 cumulative/0 per-hour; protocol-server=13111060 cumulative/0 per-hour; fuzz-assertion=0 cumulative/0 per-hour; other=0 cumulative/0 per-hour`. Some lower-level rows are approximate
...
`119` unique likely-real findings,
`1,931` bug-output rows, and
`1,818` unique output candidates.
...
`10` blockers:
`active=1; runnable=3; terminal=6`.
