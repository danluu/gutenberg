...
Snapshot generated: `2026-05-24T05:27:56Z`
...
The graph-refresh pipeline is current through `2026-05-24T05:24:06Z` for
monitor passes and through `2026-05-24T05:27:01Z` for current-run accounting.
...
`2026-05-24T05:24:06Z`.

The monitor has `4,348` passes from `2026-05-15T01:21:42Z` onward. Cumulative
coverage record observations are `293,263`, and the current-scan coverage-file
count is `1,153`. Current coverage/profile intake is populated again:
...
`2026-05-24T05:20:01Z`, CPU utilization is `24.73%`, iowait is `0.48%`, and
load averages are `20.27`, `20.84`, and `19.01` on `64` logical CPUs. Disk is
still the live constraint: at `2026-05-24T05:27:11Z`, root has `87.9GiB` free
and the data volume has `55.7GiB` free while `98.4%` used.
...
`novelty-ws-many-user-lifecycle`, sampled at `2026-05-24T05:26:46Z`.
...
remained disabled. The latest feedback-action then reports a backend/API
sentinel, a protocol/server sentinel, and one bounded lower-level replay were
started. The refreshed graph shows the current-root supervisor row as
`novelty-ws-many-user-lifecycle` and graph-active protocol-server only, so the
feedback rejects a graph-only large-post-current or broad-expansion read while
the backend/lower-level action evidence remains bounded or not yet
graph-counted.

The execution counter has `17,248,900` estimated individual executions. The
latest nonzero plotted bucket, `2026-05-24T05:15:00Z`, has `5` browser/e2e
executions and `1,075` protocol-server executions. The `05:00` bucket has
`14` browser/e2e and `25` protocol-server executions, the `04:45` bucket has
`9` browser/e2e and `25` protocol-server executions, and the `04:30` bucket
has `7` browser/e2e and `1,175` protocol-server executions.

The PR-focused critical-path data is live. The current controller snapshot is
`2026-05-24T05:25:37Z`. The state-count table has `35`
...
`2026-05-24T05:27:01Z`. It has `status_available=TRUE`,
`pending_until_first_pass=FALSE`, `current_run_metrics_trusted=TRUE`, and a
completed current-run full pass at `2026-05-24T05:24:06Z`. Latest completed
...
`55.7GiB` free and is still `98.4%` used, so output-size budgeting remains the
...
The latest enabled-group event count is `1,930`, and the newest enabled-group
...
`2026-05-24T05:26:46Z`: `novelty-ws-many-user-lifecycle`, profile
...
disabled with `coverage_ready=no`. The latest feedback-action then reports
that backend/API and protocol/server sentinels were started and one bounded
lower-level replay ran. The refreshed graph agrees only on protocol-server:
it shows a graph-active protocol-server sentinel, no graph-counted backend/API
...
in browser/e2e lanes, with protocol-server graph-active and backend/lower-level
work treated as capped or bounded evidence until the graph catches it.
...
execution volume through the `05:15` bucket.
...
Latest cumulative totals are approximately `4,764,984` browser/e2e,
`1,132,740` unit-property, `458,097` coverage-guided lower-level, and
`10,893,079` protocol-server executions. Transport-integration, backend-api,
fuzz-assertion, and other buckets are `0` in the reconstructed table.

The latest nonzero `2026-05-24T05:15:00Z` bucket has `5` browser/e2e
executions (`20`/hour) and `1,075` protocol-server executions (`4,300`/hour),
...
`05:00` bucket had `14` browser/e2e and `25` protocol-server executions, the
`04:45` bucket had `9` browser/e2e and `25` protocol-server executions, the
`04:30` bucket had `7` browser/e2e and `1,175` protocol-server executions,
and the `04:15` bucket had `11` browser/e2e and `1,350` protocol-server
executions.
...
findings over about `525.2` runner-hours, or `11.23` per 100 runner-hours.
...
startup failures. The many-user lifecycle profile has `27` records and `10`
...
largest feature categories by total count are history (`22,803`), invariant
(`13,781`), operation-ledger (`12,559`), block-depth (`5,229`), other
(`4,895`), block (`4,802`), action (`4,165`), action-pair (`4,071`), and
transport (`3,466`). Successful actions remain concentrated in a few paths:
`edit-title` (`703`), `append-paragraph` (`482`), and
`concurrent-paragraphs` (`316`).
...
`2026-05-24T05:25:37Z`. The state-count table has `35` counted items:
...
sample. CPU utilization is `24.73%`, iowait is `0.48%`, load is below the
`64`-core line, and the data volume remains very full at `98.4%` used with
`55.7GiB` free. Level-mix feedback rejects a broad or intentional
...
`novelty-http-existing-post-crdt-metadata` active dirs, then reported starting
backend/API and protocol sentinels plus a bounded lower-level replay. Treat the
feedback as rejecting a graph-only large-post-current or broad-expansion read;
...
partly supports the level-mix feedback, while still leaving the feedback's
active-dir read and claimed backend/lower-level activity only partially proven
in the graph. Protocol-server now has both a validated HTTP polling REST smoke
and recent graph-counted execution evidence; unit-property has a bounded
canary row; coverage-guided lower-level has only stale plotted residency plus
separate parser/serialization and replay evidence; transport-integration,
backend-api, and fuzz-assertion remain graph-inactive in the refreshed CSVs.
