...
Snapshot generated: `2026-05-23T15:35:36Z`
...
The graph-refresh pipeline is current through `2026-05-23T15:29:38Z` for
monitor passes, with the latest current-run accounting sample at
`2026-05-23T15:34:05Z`. The latest completed full duplicate/noise accounting
pass recorded in the active row is `2026-05-23T15:29:37Z`, so the sampled
full-pass lag is `4.5` minutes. The monitor has
`4,252` passes from `2026-05-15T01:21:42Z` onward, cumulative coverage record
observations are `286,512`, current-scan coverage files are `5,483`, and the
...
is `57.28%`, with `2.71%` iowait. Latest load averages are `42.98`,
`46.47`, and `52.92` on `64` logical CPUs, with `4` blocked tasks. The latest
disk sample has `88.8GiB` free on `/` and `158.7GiB`
free on `/media/volume/danluu-fuzz-data`; the data volume is `95.5%` used.
...
polling row from `2026-05-23T15:34:21Z`, and one stale
...
set sampled at `2026-05-23T15:29:25Z`, a fresh
...
`2026-05-23T15:34:21Z`. It still does not show
graph-counted backend/API, transport-integration, fuzz-assertion, or fresh
sustained parser lower-level capacity.

The execution counter has `17,147,722` estimated individual executions. The
latest plotted `2026-05-23T15:30:00Z` bucket is a partial bucket with
`23` browser/e2e executions, `3` unit-property executions, and `2,005`
...
`15:15` bucket had `138` browser/e2e and `2,730` protocol-server executions;
`15:00` had `278` browser/e2e and `5,245` protocol-server executions; `14:45`
had `232` browser/e2e and `5,040` protocol-server executions; `14:30` had
`212` browser/e2e and `5,040` protocol-server executions; `14:15` had `644`
...
at `2026-05-23T15:34:05Z`; the latest completed full duplicate/noise
accounting pass recorded in that row is `2026-05-23T15:29:37Z`, so the
sampled full-pass lag is `4.5` minutes. The row reports
...
product-evidence signatures, with trusted current-run accounting and `4.5`
...
stable at `88.8GiB` free and `42.3%` used; the data volume has `158.7GiB`
free and is `95.5%` used, so output-size budgeting still matters.
...
sampled at `2026-05-23T15:29:25Z`; `1` focused-shard lane from
...
`2026-05-23T15:34:21Z`. The lone `coverage-guided-lower-level` row is stale
...
`2026-05-23T15:34:21Z`. Protocol-server executions are `2,005` in the latest
partial `15:30` bucket, while that same bucket has `23` browser/e2e and `3`
unit-property executions. The prior `15:15` bucket has `2,730`
protocol-server executions and `138` browser/e2e executions. The `15:00`
...
Latest cumulative totals are approximately `122,869` browser/e2e, `5,837,147`
unit-property, `458,097` coverage-guided lower-level, and `10,729,609`
...
The latest `2026-05-23T15:30:00Z` bucket is a partial bucket with `23`
browser/e2e executions (`92`/hour), `3` unit-property executions
(`12`/hour), and `2,005` protocol-server executions (`8,020`/hour), with
zero coverage-guided-lower-level, backend-api, transport-integration, and
fuzz-assertion executions. The prior `15:15` bucket had `138` browser/e2e
...
findings over about `668.6` runner-hours, or `24.53` per 100 runner-hours.
...
lower-level assertion failures. Current unique candidate output has `1,708`
raw candidates. The by-level rate table attributes `1,700` browser/e2e
...
list-move refresh HTTP (`216` records, with `3` startup failures) and
collaboration UI signals (`686` records). Table stale snapshot HTTP now has
`390` successful records from `608` records, with `10` startup failures, and
...
revision persistence has `168` from `315`, multi-reload lifecycle has `468`
from `758`, and the adjacent large-post three-user HTTP profile has `179`
successful records from `1,588` records with `13` startup failures.
...
adjacent large-post three-user HTTP profile has `1,588` records seen with `179`
...
| Goal                                                        | Current | Target |
| ----------------------------------------------------------- | ------: | -----: |
| action ui-type-title next coverage tier                     |   1,431 |  2,000 |
| action ui-undo-redo-paragraph next coverage tier            |     565 |  1,000 |
| successful two-user documents next coverage tier            |   1,627 |  2,000 |
| CDP coverage records next coverage tier                     |   4,720 |  5,000 |
| action ui-type-paragraph next coverage tier                 |   1,797 |  2,000 |
...
`duplicateShareCurrent` `0.3333`, summary startup failures `0`, `6` current,
actionable, and product-evidence signatures, and `0.3333` top-duplicate share
in the `2026-05-23T15:34:05Z` sample, with the latest completed full pass
recorded at `2026-05-23T15:29:37Z` and `4.5` minutes of sampled
completed-pass lag.
...
still tight: latest CPU utilization is `57.28%`, iowait is `2.71%`,
one-minute load is `42.98` on `64` logical CPUs with `4` blocked tasks, and
the data volume is `95.5%` used. Optional browser admission should still
...
bucket evidence, including `2,005` executions in the latest partial `15:30`
bucket, `2,730` in `15:15`, `5,245` in `15:00`, `5,040` in `14:45`,
`5,040` in `14:30`, `5,425` in `14:15`, `5,223` in `14:00`, `5,425` in
...
refreshed graph is later, reports `14`/`24`
...
`23` browser/e2e, `3` unit-property, and `2,005` protocol-server executions.
The prior `15:15` bucket has `138` browser/e2e and `2,730` protocol-server
executions; `15:00` has `278` browser/e2e and `5,245` protocol-server executions; `14:45`
...
though the adjacent large-post three-user HTTP profile has `1,588` records and
