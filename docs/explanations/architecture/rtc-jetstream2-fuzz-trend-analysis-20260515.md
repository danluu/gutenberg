...
`20260519T075928Z`, updated the split and launched exactly one bounded
non-`1020002` `6000007` owner-replay/audit job. The graph event stream records
review cycle `20260519T082842Z` finishing at `2026-05-19T08:38:41Z`, followed
by feedback after cycle 446 starting with no collected finish row yet. The
strict owner gap remains open, so the graph still does not justify raw
reload/PR07D publication, PR16 filing, or broad final-stack fuzzing.
...
`currentRunDirSource=supervisor-active-run-dirs-startup`. Inline `triageYieldCurrent`
...
warnings, `currentRunDirSource=supervisor-active-run-dirs-startup`, `0` raw
signatures, `0` actionable signatures, `1` summary product-evidence record, `0`
...
The latest native-harness synthesis and action, `20260519T082431Z`, select and
validate the rich-text CRDT merge harness as the first ready isolated JS/V8
coverage-guided lower-level harness. The action reports a passing smoke with
...
lower-level lane is running. The latest protocol-server synthesis and action,
`20260519T082600Z`, keep `POST /wp-sync/v1/updates` through the HTTP polling
REST server as the first ready protocol/server target and report syntax/style
checks, preflight PHPUnit, and a bounded one-seed smoke that emitted
`fuzzLevel: "protocol-server"` events. The committed `protocol-server`
execution counter remains `0`. The latest fuzz-only assertion action,
...
The latest collected execution data has about `6,198,563` completed test
executions: `373,110` browser/e2e, `3,006` transport/integration, `5,374,624`
unit-property, and `447,823` coverage-guided-lower-level. The latest partial
15-minute bucket reports about `10,976` browser/e2e executions/hour and `9,472`
...
likely-real findings over about `2,739.0` runner-hours, or `28.88` per 100
...
Current unique bug-output candidate rates are: browser/e2e `6,276` candidates
over `2,739.0` runner-hours (`229.13` per 100 runner-hours),
transport/integration `106` over `59.5` runner-hours (`178.14` per 100
runner-hours), coverage-guided lower-level `2` over `20.9` runner-hours
(`9.55` per 100 runner-hours), and unit/property `6` over `59.1` runner-hours
...
are about `12676.5` for browser/e2e, `4537.5` for transport/integration,
`759.3` for coverage-guided lower-level, and `1222.5` for unit/property.
...
| --- | ---: | ---: | ---: | ---: |
| `full` | 1047 | 34 | 0 | 3.2% |
| `multi-reload-lifecycle` | 4355 | 163 | 0 | 3.7% |
| `revision-persistence` | 7508 | 289 | 0 | 3.8% |
| `parser-serialization` | 4485 | 283 | 0 | 6.3% |
| `real-user-editing` | 9749 | 691 | 0 | 7.1% |
| `common-blocks` | 5187 | 512 | 0 | 9.9% |
| `parser-transform` | 5859 | 657 | 0 | 11.2% |
| `long-session-large-doc` | 4952 | 596 | 0 | 12.0% |
| `structure` | 541 | 96 | 0 | 17.7% |
| `block-gauntlet` | 7589 | 1366 | 0 | 18.0% |
| `session-lifecycle` | 9971 | 2757 | 0 | 27.7% |
| `media-cross-entity` | 543 | 165 | 0 | 30.4% |
| `three-user-late-join` | 12320 | 3961 | 0 | 32.2% |
| `permissions-auth-locks` | 9544 | 3920 | 0 | 41.1% |
...
finished at `2026-05-19T08:38:41Z`. Feedback after cycle 446 then started with
no collected finish row yet. The most recent completed feedback
...
memory. The tradeoff is recent capacity pressure plus current novelty-state
no-product noise risk: `headroom=false`, and the copied current novelty state
reports `currentRunDirSource=supervisor-active-run-dirs-startup`, a single live
`novelty-ws-media-cross-entity` group, `0` raw signatures, no actionable
...
live health signal. Recent load exceeded the `64` logical CPU count in most,
but not all, of the latest 25 windows. The newest 1/5/15-minute load values are
`52.54`, `56.35`, and `62.63`, all below the logical CPU count. The newest
sample has `0` blocked tasks.
...
final-stack fuzzing. The latest completed feedback updated the split and
launched one bounded non-`1020002` strict `6000007` owner replay/audit, but the
active strict owner gap remains open.
