...
Snapshot generated: `2026-05-24T20:14:36Z`
...
The graph-refresh pipeline is current through `2026-05-24T20:13:00Z` for
completed monitor passes, with current-run accounting sampled at
`2026-05-24T20:13:12Z`. The active accounting row is
...
The monitor has `4,708` passes from `2026-05-15T01:21:42Z` onward. Cumulative
coverage record observations are `296,832`, and the current-scan coverage-file
count is `1,586`. Current coverage/profile intake is populated:
...
and `19:54:19Z` rows were also clear. The earlier `18:59:45Z`, `19:09:43Z`,
and `19:23:58Z` rows were non-clear at `1` over a one-signature denominator.
The current trusted row is clear; the earlier one-signature spikes should not
be read as a broad duplicate storm.
...
At `2026-05-24T20:10:05Z`, CPU utilization is `29.07%`, iowait is `2.17%`,
and load averages are `27.25`, `26.19`, and `24.68` on `64` logical CPUs. Root
has `88.1GiB` free and the data volume has `268.7GiB` free while `92.4%` used.
...
The execution counter has `17,425,471` estimated individual executions. The
latest plotted bucket, `2026-05-24T20:00:00Z`, is still the open bucket and has
`15` browser/e2e executions, `2` coverage-guided-lower-level executions, and
`5,605` protocol-server executions. The `19:45` bucket had `12` browser/e2e
executions and `6,480` protocol-server executions. The `19:15` bucket had `3`
browser/e2e executions, `2` coverage-guided-lower-level executions, and
`6,325` protocol-server executions. Recent protocol-server execution volume
still dominates the rate graph; the latest graph-counted lower-level rows are
still bounded rich-text CRDT smoke samples, now including the `20:00` bucket.

The PR-focused critical-path data is live. The current controller snapshot is
`2026-05-24T20:12:34Z`. The state-count table has `35` counted items:
...
`2026-05-24T20:13:12Z`, with its latest completed full pass at
`2026-05-24T20:13:00Z`. It has `status_available=TRUE`,
...
at `0`/`0`/`0`, and the latest `19:43:35Z`, `19:54:19Z`, and `20:13:12Z`
samples are also clear. Historical aggregate duplicate/noise is not used as
...
volume has `268.7GiB` free while still `92.4%` used. Latest CPU iowait is
`2.17%`; output-size and disk headroom remain the dominant resource constraints.
...
The latest enabled-group event count is `2,288`. The current enabled-group
...
The latest coverage-guided browser rows are at `2026-05-24T20:12:41Z`:
...
the graph-counted rich-text CRDT lower-level lane with a `20:10` smoke row and
`2` executions in the `20:00` bucket. The later level-mix feedback explicitly
...
open `20:00` bucket, with `5,965` in the `17:00` bucket, `6,480` in the
...
`6,325` in the `19:15` bucket, `5,940` in the `19:30` bucket, `6,480` in the
`19:45` bucket, and `5,605` in the open `20:00` bucket.
...
Latest cumulative totals are approximately `4,726,448` browser/e2e,
`1,132,740` unit-property, `458,621` coverage-guided lower-level, and
`11,107,662` protocol-server executions. Transport-integration, backend-api,
fuzz-assertion, and other buckets are `0` in the reconstructed table.

The latest `2026-05-24T20:00:00Z` bucket is still open: it has `15`
browser/e2e executions (`60`/hour), `2` coverage-guided-lower-level executions
(`8`/hour), and `5,605` protocol-server executions (`22,420`/hour), with zero
unit-property, backend-api, transport-integration, and fuzz-assertion
executions. The `19:15` bucket had `3` browser/e2e executions, `2`
...
protocol-server executions; the `19:45` bucket had `12` browser/e2e executions
and `6,480` protocol-server executions. Coverage-guided lower-level throughput
is visible only as small smoke rows, most recently the `20:00` rich-text CRDT
row, despite the rich-text CRDT lane being current in the mix table. The Cycle
22 feedback reports a bounded HTTP polling lower-level replay, but that replay
is not graph-counted in the refreshed execution table.
...
browser/e2e findings over about `111.5` runner-hours and `2`
transport-integration findings. Protocol-server has `0` over about `134.3`
...
lower-level assertion failures. Current unique candidate output has `109` raw
candidates: `90` browser/e2e candidates, `12` transport-integration
...
three-user HTTP lifecycle profile has `694` records, `56` successes, and `0`
...
documents at `66`/`100`, twelve-user documents at `6`/`10`, twelve-user late
...
The adjacent large-post three-user HTTP profile has `694` records seen, `56`
successful records, and a `0.0807` success rate. The generated goal table
also shows HTTP lifecycle scale at `694`/`10`, successful profile records at
...
largest feature categories by total count are history (`93,702`), invariant
(`57,388`), operation-ledger (`55,729`), block-depth (`21,892`), block
(`20,260`), action-pair (`19,933`), action (`17,786`), other (`16,647`),
transport (`10,258`), and collaborator (`10,120`). Successful actions remain
...
The resource picture is still disk-constrained. CPU utilization is `29.07%`,
iowait is `2.17%`, five-minute load is below the `64`-core line at `26.19`,
and the data volume is `92.4%` used with `268.7GiB` free. Level-mix persona
...
action, and recent execution evidence through the open `20:00` bucket.
Lower-level rich-text CRDT is present with a fresh `20:10` smoke row and `2`
executions in the `20:00` bucket, but the graph does not yet show sustained
