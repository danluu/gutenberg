...
Snapshot generated: `2026-05-24T15:00:05Z`
...
The graph-refresh pipeline is current through `2026-05-24T14:58:13Z` for
completed monitor passes, with current-run accounting sampled at
`2026-05-24T14:59:04Z`. The active accounting row is
`run-20260524T112329Z`; `current_run_metrics_trusted_last` is `TRUE`, so the
active output dir has completed a full pass.

The monitor has `4,542` passes from `2026-05-15T01:21:42Z` onward. Cumulative
coverage record observations are `296,114`, and the current-scan coverage-file
count is `2,490`. Current coverage/profile intake is populated:
...
`2026-05-24T14:51:04Z`, `2026-05-24T14:44:04Z`,
...
`2026-05-24T14:59:17Z`, root has `87.8GiB` free and the data volume has
`198.9GiB` free while `94.4%` used.
...
The execution counter has `17,294,695` estimated individual executions. The
latest plotted bucket, `2026-05-24T14:45:00Z`, has `19` browser/e2e executions
and `6,505` protocol-server executions. The `14:30` bucket had `19` browser/e2e
...
`2026-05-24T14:59:08Z`. The state-count table has `35` counted items:
...
`2026-05-24T14:59:04Z`. It has `status_available=TRUE`,
...
volume has `198.9GiB` free while still `94.4%` used. Latest CPU iowait is
...
The latest coverage-guided browser rows are at `2026-05-24T14:58:10Z`:
`novelty-http-provider-persisted-crdt-large-post` and
...
lower-level executions in the latest `14:45` bucket. It aligns with the
...
bucket, `6,300` in the `14:30` bucket, and `6,505` in the `14:45` bucket.
...
Latest cumulative totals are approximately `4,727,328` browser/e2e,
`1,132,740` unit-property, `458,615` coverage-guided lower-level, and
`10,976,012` protocol-server executions. Transport-integration, backend-api,
fuzz-assertion, and other buckets are `0` in the reconstructed table.

The latest `2026-05-24T14:45:00Z` bucket has `19` browser/e2e executions
(`76`/hour) and `6,505` protocol-server executions (`26,020`/hour), with zero
...
browser/e2e findings over about `124.8` runner-hours and `1`
...
three-user HTTP lifecycle profile has `635` records, `55` successes, and `0`
...
serialization at `21`/`50`, three-user late join at `24`/`25`,
...
The adjacent large-post three-user HTTP profile has `635` records seen, `55`
successful records, and a `0.087` success rate. The generated goal table
also shows HTTP lifecycle scale at `635`/`10`, successful profile records at
...
largest feature categories by total count are history (`82,191`), invariant
(`49,741`), operation-ledger (`48,782`), block-depth (`19,642`), block
(`18,107`), action-pair (`17,872`), action (`15,811`), other (`14,591`),
transport (`9,038`), and collaborator (`8,900`). Successful actions remain
...
`2026-05-24T14:59:04Z`; the prior `14:51`, `14:44`, `14:36`, `14:29`,
`14:21`, `14:12`, `14:06`, and `13:56` rows had the same `0.5` share over
two signatures, while the `13:48` and `13:41` rows were one-signature spikes.
The `13:32`, `13:25`, `13:17`, `13:09`, `13:00`, and `12:52` rows were clean
over zero signatures. The `12:45` and `12:37` rows were high over one
actionable product-evidence signature each, `12:29`, `12:15`, and `12:07`
were clean over zero signatures, the `11:57` and `10:50` rows were
one-signature spikes, and the `11:01` row was `0.2` over `5` signatures. The
recent spikes are control-plane health signals, not evidence of a broad product
duplicate storm.
...
the data volume is `94.4%` used with `198.9GiB` free. Level-mix persona output
