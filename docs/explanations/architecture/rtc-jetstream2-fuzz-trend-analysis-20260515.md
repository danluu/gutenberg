...
Snapshot generated: `2026-05-17T20:26:22Z`
...
- latest copied novelty state:
  current coverage output-dir state through `2026-05-17T20:24:35Z`
...
The coverage-guided loop is still expanding coverage. Across `2101` monitor
passes from `2026-05-15T01:21:42Z` through `2026-05-17T20:24:35Z`, coverage
files grew from `272` to `47518`, a delta of `47246`. The monitor's visible
...
current-output-dir sample has `duplicateShareCurrent=0.6667` and current
summary startup failures of `0`; the same pass has `1` quality issue, `0`
warnings, `427.9G` free memory, and a true monitor headroom flag. This report
...
duplicate share is `0.346` and is not the plotted live health signal.
...
headroom is true, but `duplicateShareCurrent=0.6667` and a quality issue is
...
latest substantive feedback-action remains `20260517T194638Z`; it deferred
filing/final-stack fuzzing, recorded Cycle296 PR07 replay as setup-only,
completed a non-Docker manifest/deferred harvest, and launched the bounded
Cycle298 PR07 readiness repair job.
...
`427.9G`, and headroom true. The health graph does not use historical aggregate
...
Recent sysstat samples through `2026-05-17T20:20:00Z` show bursty CPU and
...
`10`. The newest 1/5/15-minute load sample is `41.07`, `53.31`, and `56.96`, so
...
The latest native harness synthesis, `20260517T201437Z`, still keeps rich-text
CRDT merge as the first isolated lower-level target and labels it as V8/Node
coverage-guided, not AFL/libFuzzer. Its latest action file is empty, so this is
a promotion plan, not new live execution evidence. Protocol-server persona
output, `20260517T201728Z`, points to a real PHPUnit REST dispatch harness
around `/wp-sync/v1/updates` with case-counted event accounting. Its latest
action file is also empty. The refreshed graph still has `0` counted
`protocol-server` executions, so protocol-server remains harness-planning
evidence rather than collector-visible live trend activity. The latest
fuzz-only assertion apply file, `20260517T184037Z`, says browser/core-data/sync
assertion diagnostics were added and affected browser loops restarted. That is
...
The latest collected execution data has about `5,349,083` completed test
executions: `118,158` browser/e2e, `3,006` transport/integration, `4,799,584`
unit-property, and `428,335` coverage-guided-lower-level. The latest partial
15-minute bucket reports about `1184` browser/e2e test executions/hour, `4480`
...
likely-real findings over about `1,926.5` runner-hours, or `31.25` per 100
...
Current unique bug-output candidate rates are: browser/e2e `5312` candidates
over `1,926.5` runner-hours (`275.73` per 100 runner-hours),
transport/integration `106` over `59.5` runner-hours (`178.14` per 100
runner-hours), coverage-guided lower-level `2` over `19.4` runner-hours
(`10.32` per 100 runner-hours), and unit/property `4` over `24.6` runner-hours
(`16.26` per 100 runner-hours). `backend-api`, `protocol-server`, standalone
...
are about `4941.5` for browser/e2e, `4537.5` for transport/integration,
`820.3` for coverage-guided lower-level, and `2316.6` for unit/property.
...
| --- | ---: | ---: | ---: | ---: |
| `full` | 840 | 18 | 0 | 2.1% |
| `multi-reload-lifecycle` | 3316 | 114 | 0 | 3.4% |
| `revision-persistence` | 4609 | 171 | 0 | 3.7% |
| `parser-serialization` | 3360 | 177 | 0 | 5.3% |
| `real-user-editing` | 7230 | 578 | 8 | 8.0% |
| `parser-transform` | 4264 | 433 | 0 | 10.2% |
| `common-blocks` | 4162 | 447 | 0 | 10.7% |
| `structure` | 536 | 95 | 0 | 17.7% |
| `block-gauntlet` | 6288 | 1164 | 0 | 18.5% |
| `long-session-large-doc` | 2931 | 577 | 0 | 19.7% |
| `persistence-no-title` | 3304 | 871 | 1 | 26.4% |
| `session-lifecycle` | 8253 | 2483 | 0 | 30.1% |
...
| action ui-format-paragraph next 2000 tier | 1588 | 2000 |
...
The latest substantive feedback-action remains `20260517T194638Z`; it recorded
the Cycle296 PR07 replay as setup-only, completed a manifest/deferred harvest
with no branch/base/ref/head agreement failures, and launched the Cycle298 PR07
collaboration-readiness repair/replay job. The latest synthesis says that job
ended `disk-preflight-blocked`, so filing, final-stack fuzzing, broad fuzz, and
PR07D/PR17/PR18x work remain deferred.

The suggested-PR size charts are parsed from the status report's proposed PR
split history. The latest parsed snapshot, `2026-05-17T20:12:23Z`, has `14`
...
`1`, and warnings are `0`; current-output duplicate share is `0.6667`, while
...
root space is blocking replay. The latest substantive feedback-action launched
a bounded PR07 collaboration-readiness repair/replay job and completed
manifest/deferred harvest; the newest synthesis says that PR07 job ended
`disk-preflight-blocked`, so publication and final validation still wait for
PR07 collaboration-ready owner evidence.
...
the first lower-level target to promote and says the HTTP polling protocol
harness should be built around `/wp-sync/v1/updates`, but protocol-server plus
standalone assertion outputs are not collector-visible as live trend
executions. The level-mix persona loop also rejects trusting the supervisor lane
graph as browser materialization proof until deduped live PIDs meet the browser
floor. The next narrow checks are the duplicate current-output gate, producer
pause/rotation if product-evidence duplicates keep replenishing, browser
live-PID accounting, protocol counts becoming collector-visible, and PR07 root
space plus replay evidence before any filing or final-stack claim.
