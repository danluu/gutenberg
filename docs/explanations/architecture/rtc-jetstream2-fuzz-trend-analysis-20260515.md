...
Snapshot generated: `2026-05-18T06:46:49Z`
...
The coverage-guided loop is still expanding coverage. Across `2212` monitor
passes from `2026-05-15T01:21:42Z` through `2026-05-18T06:40:25Z`, coverage
files grew from `272` to `50648`, a delta of `50376`. The monitor's visible
...
ten `0.0000` samples and two `1.0000` recurrences, so the clean latest point
is not durable recovery evidence by itself. The latest pass has `0` quality
issues, `0` warnings, `408.0G` free memory, and a false monitor headroom flag.
This report
...
`408.0G`, and headroom false. The last twelve current-output duplicate-share
...
The copied novelty state currently lists `novelty-ws-three-user-late-join` as
the enabled group. Current-output duplicate share is `0.0000` and current
...
Recent sysstat samples through `2026-05-18T06:40:02Z` show bursty CPU. The
latest 25 CPU samples range from `61.0%` to `83.3%` utilization, with the latest
sample at `78.9%`. Over those same 25 samples, one-minute load exceeded the
...
1/5/15-minute load sample is `70.98`, `75.67`, and `74.85`; all three latest
...
The latest protocol-server synthesis, `20260518T063301Z`, keeps HTTP polling
REST as the first protocol/server target and requires root/lane `events.ndjson`
accounting. The committed graph still has `0` protocol-server cumulative
executions and no current protocol rate. The latest fuzz-only
...
The latest collected execution data has about `5,579,581` completed test
executions: `162,800` browser/e2e, `3,006` transport/integration, `4,965,952`
unit-property, and `447,823` coverage-guided-lower-level. The latest partial
15-minute bucket reports about `52` browser/e2e test executions/hour,
`256` unit-property executions/hour, and `0` coverage-guided-lower-level
...
likely-real findings over about `2,135.1` runner-hours, or `32.97` per 100
...
Current unique bug-output candidate rates are: browser/e2e `5,731` candidates
over `2,135.1` runner-hours (`268.42` per 100 runner-hours),
transport/integration `106` over `59.5` runner-hours (`178.14` per 100
runner-hours), coverage-guided lower-level `2` over `20.9` runner-hours
(`9.55` per 100 runner-hours), and unit/property `5` over `33.6` runner-hours
(`14.90` per 100 runner-hours). `backend-api`, `protocol-server`, standalone
...
The broader unique-output candidate view is led by three-user late join (`787`),
session lifecycle (`725`), real-user editing (`672`), revision persistence
(`490`), and permissions/auth/locks (`450`), with
...
are about `6494.1` for browser/e2e, `4537.5` for transport/integration,
`759.3` for coverage-guided lower-level, and `2139.4` for unit/property.
...
| --- | ---: | ---: | ---: | ---: |
| `full` | 840 | 18 | 0 | 2.1% |
| `multi-reload-lifecycle` | 3671 | 153 | 0 | 4.2% |
| `revision-persistence` | 5345 | 231 | 0 | 4.3% |
| `parser-serialization` | 3667 | 228 | 0 | 6.2% |
| `real-user-editing` | 7840 | 602 | 0 | 7.7% |
| `common-blocks` | 4400 | 474 | 0 | 10.8% |
| `parser-transform` | 4632 | 503 | 0 | 10.9% |
| `long-session-large-doc` | 3414 | 580 | 0 | 17.0% |
| `structure` | 536 | 95 | 0 | 17.7% |
| `block-gauntlet` | 6606 | 1241 | 0 | 18.8% |
| `session-lifecycle` | 8764 | 2634 | 0 | 30.1% |
| `persistence-no-title` | 3633 | 1119 | 0 | 30.8% |
...
| action ui-format-paragraph next 2000 tier | 1876 | 2000 |
...
split history. The latest parsed snapshot, `2026-05-18T06:35:34Z`, has `8`
...
quality issues `0`, warnings `0`, and `408.0G` free memory. The
latest 1/5/15-minute load windows are `70.98`, `75.67`, and `74.85`; all three
...
selected HTTP polling protocol/server target, and level-mix evidence that a
