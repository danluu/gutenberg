...
Snapshot generated: `2026-05-22T13:36:05Z`
...
The monitor data is current through `2026-05-22T13:34:30Z`, and the latest
current-run accounting row was sampled at `2026-05-22T13:35:03Z` for
`run-20260522T132644Z`. That active run has completed a trusted full pass:
`current_run_metrics_trusted` is `TRUE`, `pending_until_first_pass` is `FALSE`,
and `full_pass_pending` is `FALSE`. The monitor has `3,982` passes from
`2026-05-15T01:21:42Z` onward. Cumulative coverage record observations are
`278,301`; current-scan coverage files are `2,030`. The parsed coverage-goal
...
startup failures `0`. That share has only `2` current-run signatures, `2`
actionable signatures, and `2` product-evidence signatures, with a
current-run top duplicate share of `0.5`, so this is a small-denominator
control-plane/yield signal rather than evidence of a broad product duplicate
storm. Historical duplicate share is `0.2381` for context; it is not the
...
`394.4G` free memory; the latest disk sample has `90.8GiB` free on `/` and
`503.6GiB` free on `/media/volume/danluu-fuzz-data`. Latest CPU utilization is
`58.59%`, with `3.08%` iowait. Latest load averages are `52.93`, `48.12`, and
`47.91` on `64` logical CPUs, with `3` blocked tasks in the same sample.
...
The execution counter has `16,766,686` estimated individual executions. These
...
`2026-05-22T13:30:00Z` has `56` browser/e2e executions, `640`
unit-property executions, and `780` protocol-server executions, about
`224`/hour, `2,560`/hour, and `3,120`/hour. The preceding
`2026-05-22T13:15:00Z` bucket had `109` browser/e2e executions, `2,688`
unit-property executions, and `1,045` protocol-server executions, about
`436`/hour, `10,752`/hour, and `4,180`/hour. The latest nonzero
protocol-server bucket is now `2026-05-22T13:30:00Z`. The latest nonzero
...
`run-20260522T132644Z` was taken at `2026-05-22T13:35:03Z`; the latest
completed full pass was at `2026-05-22T13:34:30Z` with
...
The earlier first-pass-pending row at `2026-05-22T13:27:03Z` for the same run
was tracked separately as incomplete accounting and is now superseded by a
trusted full-pass row. The trusted rows at `2026-05-22T12:42:33Z` and
`2026-05-22T12:48:49Z` for `run-20260522T123220Z` carried
`duplicateShareCurrent` `1`, but that share had only `1` current-run signature
and `1` actionable signature, so it was a one-signature control-plane/yield
check rather than a broad duplicate/noise rate. The latest `13:35:03Z` row has
`duplicateShareCurrent` `0.5`, but the current-run/actionable denominator is
still only `2`, so the completeness graph should be read as a small-denominator
control-plane health signal rather than a measured product duplicate storm.
...
graph has since completed trusted accounting for `run-20260522T132644Z`; the
latest row still has only `2` current/actionable signatures. The right reading
is no broad measured product duplicate/noise storm, plus a small-denominator
current-output-dir signal that should be watched until the active run
accumulates a larger denominator.
...
around `85.8%` used and root is around `41.0%` used. Root pressure remains
...
The latest enabled-group event count is `1,711`. Current enabled groups are
...
rows include three coverage-guided rows: parser serialization, parser
transform, and media cross-entity. They also include one focused-shards large
HTTP lifecycle row, six gap-booster rows, and three strict HTTP expansion rows
for HTTP persistence, same-user stale draft, and large HTTP lifecycle. Those
rows cover persistence without title, large HTTP lifecycle, real-user editing,
three-user late join, revision, permissions/auth/locks, async/server, long-doc,
parser serialization, parser transform, media cross-entity, and same-user stale
draft coverage.
...
but the latest native-harness synthesis still selected parser serialization as
the first isolated Node/V8 coverage-guided lower-level harness and the latest
native-harness action file is empty. Parser serialization has `72` records and
`12` successful records. It is current browser/e2e residency, not current
graph-counted lower-level residency. The only current graph-counted
...
sentinel row and `780` executions in the `2026-05-22T13:30:00Z` bucket, so
...
legacy batch-count fields. The latest totals are approximately `79,012`
browser/e2e, `5,713,888` unit-property, `458,097` coverage-guided lower-level,
and `10,515,689` protocol-server executions. Transport-integration,
backend-api, fuzz-assertion, and other buckets are `0` in the current
reconstructed table.

The latest 15-minute bucket at `2026-05-22T13:30:00Z` is partial and has
`56` browser/e2e executions (`224`/hour), `640` unit-property executions
(`2,560`/hour), and `780` protocol-server executions (`3,120`/hour).
Coverage-guided lower-level, backend-api, transport-integration, and
fuzz-assertion are zero in that bucket. The preceding
`2026-05-22T13:15:00Z` bucket had `109` browser/e2e executions (`436`/hour),
`2,688` unit-property executions (`10,752`/hour), and `1,045`
protocol-server executions (`4,180`/hour). The latest nonzero protocol-server
bucket is now `2026-05-22T13:30:00Z`. The latest nonzero
coverage-guided lower-level bucket remains
...
findings over about `593.2` runner-hours, or `20.90` per 100 runner-hours.
...
parser serialization has `12`, full-profile rows have `16`, many-user
...
three-user HTTP has `42`, parser transform has `68`, media cross-entity and
three-user late join each have `96`, block-gauntlet has `105`, async/server
...
three-user HTTP profile has `473` records seen with `42` successful records.
...
| Goal                                                   | Current | Target |
| ------------------------------------------------------ | ------: | -----: |
| successful parser-serialization records                |      12 |     50 |
...
the seed `5200005` reducer queued/runnable, productive-analysis runnable,
...
`2026-05-22T13:35:03Z`; the latest completed full pass was at
`2026-05-22T13:34:30Z` with `full_pass_pending` `FALSE`,
...
`0.5` and summary startup failures `0`, with only `2` current-run signatures,
`2` actionable signatures, and `2` product-evidence signatures. That is a
trusted but small-denominator duplicate/noise row, not a broad product-duplicate
measurement.
...
`2` raw current signatures, and the refreshed graph still has only a
two-signature current/actionable denominator. The synthesis remains evidence of
a recently fixed producer-side control-plane risk; the live health issue now is
small-denominator current-output-dir accounting, not a measured broad
duplicate/noise rate.

The resource picture is usable in the latest sample. Latest CPU utilization is
`58.59%`, with `3.08%` iowait; load is `52.93`, `48.12`, and `47.91` on `64`
...
`72` records and `12` successes. It has no backend/API lane, no
...
native-harness synthesis selected parser serialization as the first isolated
lower-level harness, but the latest action file is empty, so it is not
graph-counted live lower-level residency. Protocol-server HTTP polling was
implemented and bounded-validated, with current graph telemetry and `780`
latest-bucket executions.

Browser/e2e remains the only level producing triaged likely-real findings in
the committed triage-output metric. The latest execution bucket is partial: the
`2026-05-22T13:30:00Z` bucket has `56` browser/e2e executions, `640`
unit-property executions, and `780` protocol-server executions. The preceding
`2026-05-22T13:15:00Z` bucket shows `109` browser/e2e executions, `2,688`
unit-property executions, and `1,045` protocol-server executions; the latest
nonzero protocol-server bucket is now `2026-05-22T13:30:00Z`. Lower-level
...
rather than more blocked or no-progress artifacts. Benchmark-canary is the only
active critical blocker, while the seed reducer is runnable, and the
