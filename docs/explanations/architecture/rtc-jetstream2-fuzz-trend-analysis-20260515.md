...
Snapshot generated: `2026-05-24T13:01:57Z`
...
The graph-refresh pipeline is current through `2026-05-24T12:59:58Z` for
completed monitor passes, with current-run accounting sampled at
`2026-05-24T13:00:56Z`. The active accounting row is
`run-20260524T112329Z`; `current_run_metrics_trusted_last` is `TRUE`, so the
active output dir has completed a full pass.

The monitor has `4,485` passes from `2026-05-15T01:21:42Z` onward. Cumulative
coverage record observations are `295,990`, and the current-scan coverage-file
count is `2,372`. Current coverage/profile intake is populated:
...
`2026-05-24T12:52:55Z` row was also clean over zero signatures. The
...
At `2026-05-24T13:00:08Z`, CPU utilization is `30.76%`, iowait is `0.95%`, and
load averages are `26.77`, `26.24`, and `23.93` on `64` logical CPUs. At
`2026-05-24T13:01:11Z`, root has `87.8GiB` free and the data volume has
`203.4GiB` free while `94.3%` used.
...
`novelty-ws-thirty-user-lifecycle`; the active enabled-group summary lists both
groups.
...
The execution counter has `17,243,712` estimated individual executions. The
latest plotted bucket, `2026-05-24T13:00:00Z`, has `1` browser/e2e execution
and `540` protocol-server executions. The `12:45` bucket had `30` browser/e2e
executions and `6,300` protocol-server executions, the `12:30` bucket had `12`
browser/e2e executions and `2,725` protocol-server executions, and the `11:15`
...
`2026-05-24T13:00:59Z`. The state-count table has `35` counted items:
...
`2026-05-24T13:00:56Z`. It has `status_available=TRUE`,
...
`current_run_top_duplicate_share=0`. The previous completed row at
`2026-05-24T12:52:55Z` was also clean over zero signatures. The
`2026-05-24T12:45:55Z` and `2026-05-24T12:37:55Z` rows were high over one
signature each, while `2026-05-24T12:29:01Z`, `12:15:25Z`, and `12:07:51Z`
were `0` over zero signatures. Earlier noisy rows at `2026-05-24T11:57:42Z` and
...
volume has `203.4GiB` free while still `94.3%` used. Latest CPU iowait is
...
The latest coverage-guided browser rows are at `2026-05-24T12:59:15Z`:
`novelty-http-large-post-lifecycle-completion` and
`novelty-ws-thirty-user-lifecycle`. The other graph-counted browser/e2e
...
completion row plus one WebSocket thirty-user lifecycle row.
...
explicit blockers/downscopes. The latest feedback-action implemented sticky
HTTP canary selection with one WebSocket reserve lane and reported backend/API
plus protocol/server single-lane restarts. The refreshed graph supports the
browser/e2e concentration and protocol-server restart, but it still shows one
HTTP coverage-guided row plus one WS coverage-guided row; it does not yet show
the synthesis-requested second forced HTTP row or a backend-api lane. Treat the
...
file is empty, so the latest non-empty action remains the validated rich-text
CRDT coverage-guided lower-level harness with root/lane events and
`fuzzLevel: "coverage-guided-lower-level"`; it did not start an unbounded loop.
The graph currently shows the rich-text CRDT lower-level lane and a small
`11:15` bucket execution count, so it aligns with the rich-text recommendation
...
through the latest `13:00` bucket, with `2,725` executions in the `12:30`
bucket, `6,300` in the `12:45` bucket, and `540` in the `13:00` bucket.
...
Latest cumulative totals are approximately `4,727,209` browser/e2e,
`1,132,740` unit-property, `458,611` coverage-guided lower-level, and
`10,925,152` protocol-server executions. Transport-integration, backend-api,
fuzz-assertion, and other buckets are `0` in the reconstructed table.

The latest `2026-05-24T13:00:00Z` bucket has `1` browser/e2e execution
(`4`/hour) and `540` protocol-server executions (`2,160`/hour), with zero
unit-property, coverage-guided-lower-level, backend-api,
transport-integration, and fuzz-assertion executions. The `12:45` bucket has
`30` browser/e2e executions and `6,300` protocol-server executions, the `12:30`
bucket has `12` browser/e2e executions and `2,725` protocol-server executions,
...
browser/e2e findings over about `121.4` runner-hours and `1`
...
three-user HTTP lifecycle profile has `544` records, `55` successes, and `0`
...
serialization at `21`/`50`, media cross-entity at `19`/`25`, thirty-user
...
completion row. The adjacent large-post three-user HTTP profile has `544`
records seen, `55` successful records, and a `0.10110` success rate. The
generated goal table also shows HTTP lifecycle scale at `544`/`10`, successful
...
largest feature categories by total count are history (`78,522`), invariant
(`48,083`), operation-ledger (`46,457`), block-depth (`18,901`), block
(`17,368`), action-pair (`17,083`), action (`15,140`), other (`14,146`),
transport (`8,794`), and collaborator (`8,656`). Successful actions remain
...
accounting: the latest trusted row is clean, and the prior `12:52` row was also
clean. The `12:45` and `12:37` rows were high over one actionable
...
The resource picture is still disk-constrained. CPU utilization is `30.76%`,
iowait is `0.95%`, five-minute load is below the `64`-core line at `26.24`, and
the data volume is `94.3%` used with `203.4GiB` free. Level-mix persona output
...
active coverage-guided root includes `novelty-http-large-post-lifecycle-completion`
plus `novelty-ws-thirty-user-lifecycle`, not the requested second forced HTTP
canary. Protocol-server HTTP polling has a current row and recent high
