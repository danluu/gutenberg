...
| `danluu/try/stale-rich-text-sibling-pr` | `0055b2bff6790af759d7f76ae5ed792bf4b0df8c` | Proposed Issue 1 fix branch after rebase onto current trunk | `WP_BASE_URL=http://localhost:8946 npm run test:e2e -- test/e2e/specs/editor/collaboration/collaboration-stale-rich-text-sibling.spec.ts --project=chromium --repeat-each=3`; `/Users/danluu/dev/fuzz/gutenberg-stale-rich-text-sibling-pr/artifacts/stale-rich-text-candidate-search/final_pr_branch_committed_repro_repeat3.log` | Failed 3/3; unit/lint/build passed, but the strengthened browser repro still observed visible truncated `fileName` values (`Re`, `Re`, `Rem`). |
