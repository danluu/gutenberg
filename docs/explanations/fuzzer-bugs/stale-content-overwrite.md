...
- These cover stale full block snapshots, remote top-level appends/deletes, remote rich-text edits on unchanged sibling blocks, deriving serialized `content` from merged blocks, save-time merging with the latest persisted CRDT record, full-record saves that carry unchanged stale saved fields, and the no-persisted-CRDT initial persistence path.
...
There is a subtler version of the same failure for internal/full-record saves. `saveEntityRecord` sends the edited record after merging `__unstablePrePersist` output back into the original payload. If the payload contains `content`, `title`, or `excerpt` that are unchanged relative to B's stale base, those fields can still overwrite newer server values. A guard that only runs for locally changed saved fields prevents some false positives, but it leaves this full-record stale overwrite open.
...
The later "only run the freshness check when the local saved field changed" version was also too cute. It protected first-time CRDT persistence, but it forgot that the REST payload can still contain stale unchanged full-body fields. A save guard has to reason about what is in the outgoing write, not just what the user appears to have edited.
...
From this angle, locally changed and merely present fields are different operations. For a locally changed field, the client is trying to write a new value and needs a merge against the latest state. For a field that is only present because the edited record is a full snapshot, the safe operation is "leave the server value alone." The fix must preserve that distinction.
...
The regression test that caught the first attempted fix was important: an old or externally created post may not have `_crdt_document` yet, and a meta/internal persistence save still must create it. The revised tests should therefore include both the user-content-loss repro and the "do not break initial CRDT persistence" path.

## Revised fix plan

The PR branch implements the revised plan:

1. Before persisting an existing collaborated post/page, fetch the latest server record whenever the outgoing payload contains `content`, `title`, or `excerpt`.
2. Split those fields into locally changed fields and full-record/pass-through fields that are only present in the payload.
3. For pass-through fields whose server value changed since this editor's saved base, overwrite the outgoing stale value with the latest server value. This turns an accidental full-snapshot overwrite into "leave the server field alone."
4. Apply the latest persisted CRDT document into the local sync document before creating the new persisted CRDT doc when there is a persisted document to merge, or when a local saved-field edit needs the latest record applied.
5. Use merged CRDT record data only for locally changed saved fields. Do not replace unchanged payload fields from a partially flushed or no-persisted-document local CRDT state.
6. If applying the latest record changes the local Y.Doc, serialize the document after that apply, including the no-persisted-CRDT path, so `_crdt_document` matches the current local document.
7. In block merging, reconcile stale full snapshots against the last local base so unchanged stale sibling blocks do not overwrite remote edits, and missing remote top-level blocks are not treated as local deletes.
8. When both `blocks` and `content` are present, derive `content` from the merged block tree rather than trusting the stale serialized full-string snapshot.
