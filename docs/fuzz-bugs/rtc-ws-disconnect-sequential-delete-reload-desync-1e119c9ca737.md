...
Pass 179 repairs that last evidence gap by copying only the generated `packages/icons/src/library` output from the known-fixes worktree into two disposable worktrees. With that bounded test-harness repair, the block-level regression runs cleanly: the tests-only worktree at `1289a0c796c` fails the reconnect-order test by resurrecting `Beta` and `Gamma`, while the fixed worktree at `ca859ff32a2` passes both sequential-delete stale-reload tests. A focused manager-test run in the same pair of worktrees also fails the three expected known-fixes-base invariants and passes on the fix. This pass does not add a natural Playwright repro, but it does independently confirm both lower-level routes with exact negative/positive test pairs.
...
The pass 179 code-path read makes the timing dependency concrete on the frozen base. `SyncManager.load()` observes `recordMap` and `stateMap` before `Promise.all(providerCreators.map(...))`, then calls `internal.applyPersistedCrdtDoc()` after provider creation. The local WebSocket provider returns only after `await provider.ready`, and `provider.ready` is resolved from `markSynced()` after snapshot or peer-update handling. That means a provider can apply remote state to the Y document during `load()` before stale persisted/REST hydration runs, without any malformed block injection or direct product-state mutation.
