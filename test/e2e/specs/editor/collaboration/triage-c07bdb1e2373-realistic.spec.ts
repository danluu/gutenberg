...
type AttemptResult = {
	afterSaveError?: string;
	afterSavePrimary?: unknown;
	afterSaveSecondary?: unknown;
	...
	attempt: number;
	persistedAfterHeading?: RestPostRecord;
...
async function runAttempt( {
	...
	try {
		...
		result.afterSaveError =
			error instanceof Error ? error.message : String( error );
		result.afterSavePrimary =
			await collaborationUtils.getNormalizedPostState( page, {
				includeCrdtDocument: true,
			} );
		result.afterSaveSecondary =
			await collaborationUtils.getNormalizedPostState( collaboratorPage, {
				includeCrdtDocument: true,
			} );
		result.persistedAfterSave = await getPersistedPost(
			requestUtils,
			post.id
		);
		result.reproduced = true;
		result.reproducedReason =
			'post-save checkpoint did not converge before the reload/heading step';
		return result;
	}
	...
	result.persistedAfterHeading = await getPersistedPost(
		requestUtils,
		post.id
	);
...
test( 'c07bdb1e2373 one-attempt save-reload-heading sequence', async ( {
	collaborationUtils,
	collaboratorUser,
	editor,
	page,
