...
type AttemptResult = {
	attempt: number;
	debugPrimaryState?: unknown;
	debugSecondaryState?: unknown;
	error?: string;
	postId?: number;
...
async function getDebugPostState( page: Page ): Promise< unknown > {
	return page.evaluate( () => {
		const describeBlock = ( block: any ) => ( {
			clientId: block?.clientId,
			name: block?.name,
			content: block?.attributes?.content,
			innerBlocks: ( block?.innerBlocks ?? [] ).map( describeBlock ),
		} );
		const postId = ( window as any ).wp.data
			.select( 'core/editor' )
			.getCurrentPostId();
		const core = ( window as any ).wp.data.select( 'core' );
		const blockEditor = ( window as any ).wp.data.select(
			'core/block-editor'
		);
		const editedRecord = core.getEditedEntityRecord(
			'postType',
			'post',
			postId
		);
		...
		return {
			blockEditorBlocks: blockEditor.getBlocks().map( describeBlock ),
			blockEditorOrder: blockEditor.getBlockOrder(),
			coreEditedBlocks: ( editedRecord?.blocks ?? [] ).map(
				describeBlock
			),
			editorEditedBlocks: (
				( window as any ).wp.data
					.select( 'core/editor' )
					.getEditedPostAttribute( 'blocks' ) ?? []
			).map( describeBlock ),
		};
	} );
}
...
async function runAttempt( {
	...
	try {
		...
		try {
			Object.assign( result, await getStates( collaborationUtils ) );
			const [ debugPrimaryState, debugSecondaryState ] =
				await Promise.all( [
					getDebugPostState( collaborationUtils.allPages[ 0 ] ),
					getDebugPostState( collaborationUtils.getPage( 0 ) ),
				] );
