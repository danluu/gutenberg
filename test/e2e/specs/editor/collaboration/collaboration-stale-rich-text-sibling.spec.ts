...
test.describe( 'Collaboration - Stale rich-text sibling snapshots', () => {
	test( 'keeps a remote file name edit while another user continuously edits the download button text', async ( {
		...
		const downloadButtonA = editor.canvas
			...
			.first();
		const downloadButtonB = editor2.canvas
			.locator(
				'[data-type="core/file"] [aria-label="Download button text"]'
			)
			.first();
		...
		const typingB = replaceText( page2, fileNameB, 'Remote file', {
			delay: 200,
		} );
		await expect( fileNameB ).toContainText( 'R', { timeout: 20_000 } );
		const typingA = typeChunks(
			page,
			Array.from( { length: 220 }, ( _, i ) => `chunk${ i } ` ),
			{ delay: 20 }
		);
		...
		await expect( fileNameB ).toHaveText( 'Remote file', {
			timeout: 20_000,
		} );
		await expect( downloadButtonA ).toContainText( 'Local button', {
			timeout: 20_000,
		} );
		await expect( fileNameA ).toHaveText( 'Remote file', {
			timeout: 20_000,
		} );
		await expect( downloadButtonB ).toContainText( 'Local button', {
			timeout: 20_000,
		} );
