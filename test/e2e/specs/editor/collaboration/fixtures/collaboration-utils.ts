...
interface StringStateDivergence {
	...
	entityNormalizedSnippet?: string;
	firstLength?: number;
	firstPresent?: boolean;
	firstSnippet?: string;
	kind:
		| 'string-html-entity-divergence'
		| 'string-path-divergence'
		| 'string-segment-divergence'
		| 'string-suffix-divergence'
		| 'string-value-divergence';
	missingSegment?: string;
	missingSegmentPageIndex?: number;
...
const HTML_ENTITY_NORMALIZATIONS: Record< string, string > = {
	amp: '&',
	apos: "'",
	copy: '\u00a9',
	gt: '>',
...
export default class CollaborationUtils {
	...
	private describeStringValueDivergence(
		...
		const firstMiddle = firstValue.slice(
			commonPrefixLength,
			firstValue.length - commonSuffixLength
		);
		const otherMiddle = otherValue.slice(
			commonPrefixLength,
			otherValue.length - commonSuffixLength
		);
		let missingSuffix: string | undefined;
		let missingSuffixPageIndex: number | undefined;
		let missingSegment: string | undefined;
		let missingSegmentPageIndex: number | undefined;
		...
		} else if ( firstMiddle && ! otherMiddle ) {
			missingSegment = firstMiddle.slice( 0, 80 );
			missingSegmentPageIndex = pageIndex;
		} else if ( otherMiddle && ! firstMiddle ) {
			missingSegment = otherMiddle.slice( 0, 80 );
			missingSegmentPageIndex = 0;
		}
		...
		const entityNormalizedFirstValue =
			this.normalizeHtmlEntitiesForComparison( firstValue );
		const entityNormalizedOtherValue =
			this.normalizeHtmlEntitiesForComparison( otherValue );
		const hasEntityOnlyDivergence =
			entityNormalizedFirstValue === entityNormalizedOtherValue;
		let kind: StringStateDivergence[ 'kind' ] = 'string-value-divergence';

		if ( hasEntityOnlyDivergence ) {
			kind = 'string-html-entity-divergence';
		} else if ( firstHasOtherPrefix || otherHasFirstPrefix ) {
			kind = 'string-suffix-divergence';
		} else if ( missingSegment ) {
			kind = 'string-segment-divergence';
		}
		...
		if ( hasEntityOnlyDivergence ) {
			divergence.entityNormalizedSnippet =
				this.getStringDivergenceSnippet(
					entityNormalizedFirstValue,
					Math.min(
						commonPrefixLength,
						entityNormalizedFirstValue.length
					)
				);
		}
	...
	private normalizeHtmlEntitiesForComparison( value: string ): string {
		return value.replace(
			...
			( match, entity: string ) => {
				...
				if ( normalizedEntity.startsWith( '#' ) ) {
					const isHex =
						normalizedEntity.charAt( 1 ).toLowerCase() === 'x';
