...
type AttemptResult = {
	...
	statesEqual: boolean;
	textCorruptionObserved: boolean;
};

const OUTPUT_DIR = process.env.RTC_810C_OUTPUT_DIR;
const MODIFIER_KEY = process.platform === 'darwin' ? 'Meta' : 'Control';

const INITIAL_CONTENT = [
	'<!-- wp:heading -->',
	'<h2 class="wp-block-heading">Seed 953009 multibyte heading</h2>',
	'<!-- /wp:heading -->',
	'<!-- wp:paragraph -->',
...
const test = base.extend< Fixtures >( {
	...
	collaboratorUser: async (
		...
		const collaboratorUser = {
			username: `rtc810c${ uniqueSuffix }`,
			email: `rtc810c+${ uniqueSuffix }@example.com`,
			firstName: 'RTC',
			lastName: 'Triage',
			password: 'password',
			roles: [ 'editor' ],
...
async function runScenario( {
	...
	const primaryText = `Seed 953009 realistic primary ${ scenario } ${ attempt }`;
	const secondaryText = `Seed 953009 realistic collaborator ${ scenario } ${ attempt }`;
	const anchorText =
	...
	const textCorruptionObserved = ! exactTextsPresentOnBoth;
...
test.describe( 'RTC triage 810c2d35f443 realistic paragraph-insert family search', () => {
	for ( const [ scenario, attempt ] of [
		[ 'enter-after-anchor', 0 ],
		[ 'shortcut-after-anchor', 0 ],
	] as const ) {
		test( `${ scenario } attempt ${ attempt }`, async ( {
			...
			const post = await requestUtils.createPost( {
				...
				status: 'draft',
				title: `RTC 810c family ${ scenario } ${ attempt }`,
			} );
