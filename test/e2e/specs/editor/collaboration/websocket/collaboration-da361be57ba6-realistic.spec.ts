...
function writeAttemptResult( result: AttemptResult ) {
	...
	fs.writeFileSync(
		path.join( OUTPUT_DIR, 'da361-realistic-attempt.json' ),
...
async function waitForSessionReady(
	...
	await collaborationUtils.waitForMutualDiscovery( { timeout: 25000 } );
...
async function insertTableAtEnd( editor: Editor, page: Page ) {
	...
	const values = [
		'initial row 1 A realistic da361',
		'initial row 1 B realistic da361',
		'initial row 2 A realistic da361',
		'initial row 2 B realistic da361',
	];
...
test.describe( 'RTC triage da361be57ba6 realistic delete after table insert', () => {
	test( 'does not restore a shared paragraph after a prior delete and table insert', async ( {
		collaborationUtils,
		collaboratorUser,
		editor,
		page,
		...
		const post = await requestUtils.createPost( {
			...
			status: 'draft',
			title: 'RTC da361 realistic table after delete',
		} );
