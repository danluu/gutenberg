...
const runs = [
	{
		runId: 'runtime_repeat_raw_cdp',
		repeatMode: 'raw CDP only',
		repeatCount: 0,
		dir: 'test/performance/artifacts/typing-delay-runtime-repeat-cdp-key-hold',
	},
	...[
		{
		...
		[ 1, 3, 7, 11, 17 ].map( ( repeatCount ) => ( {
			runId: `runtime_repeat_${ mode.delayMode }_${ repeatCount }`,
			repeatMode: mode.repeatMode,
			repeatCount,
			dir: `test/performance/artifacts/typing-delay-runtime-repeat-${ mode.delayMode }-${ repeatCount }`,
		} ) )
	),
];
...
function rowsForRun( run ) {
	...
	if ( ! jsonPath ) {
		throw new Error( `Missing runtime-repeat JSON for ${ run.runId }` );
	}
...
writeCsv(
	path.join( reportDataDir, 'typing-delay-runtime-repeat-samples.csv' ),
	sampleRows,
	[
		'run_id',
		'repeat_mode',
...
writeCsv(
	path.join( reportDataDir, 'typing-delay-runtime-repeat-summary.csv' ),
	summaryRows,
	[
		'run_id',
		'repeat_mode',
...
// eslint-disable-next-line no-console
console.log(
	`Wrote runtime-repeat CSVs to ${ path.relative( repoRoot, reportDataDir ) }`
);
