...
function getBlocksByClientIdDeepIfEveryBlockHasUniqueId(
	blocks: Block[]
): Map< string, Block > | null {
	const blocksByClientId = new Map< string, Block >();

	const walk = ( blockList: Block[] ): boolean => {
	...
	return walk( blocks ) ? blocksByClientId : null;
}

function replaceBlockByClientId(
	blocks: Block[],
	clientId: string,
	...
	blocks.forEach( ( block, index ) => {
		...
		if ( getBlockClientId( block ) === clientId ) {
			newBlock = replacementBlock;
		} else {
			const innerBlocks = block.innerBlocks ?? [];
			const newInnerBlocks = replaceBlockByClientId(
				innerBlocks,
				clientId,
				replacementBlock
			);

			if ( newInnerBlocks !== innerBlocks ) {
				newBlock = {
					...block,
		...
		replacedBlocks[ index ] = newBlock;
	} );

	return replacedBlocks ?? blocks;
}

function reconcileMovedStaleLocalBlocks(
	localBlocks: Block[],
	...
	const currentBlocksByClientId =
		getBlocksByClientIdDeepIfEveryBlockHasUniqueId( currentBlocks );
	const currentTopLevelClientIds =
		getClientIdsIfEveryBlockHasUniqueId( currentBlocks );

	if (
		! previousBlocksByClientId ||
		! currentBlocksByClientId ||
		! currentTopLevelClientIds
	) {
		return currentBlocks;
	}

	let reconciledCurrentBlocks = currentBlocks;
	const currentTopLevelClientIdSet = new Set( currentTopLevelClientIds );

	localBlocks.forEach( ( localBlock ) => {
		const clientId = getBlockClientId( localBlock );

		if ( ! clientId || currentTopLevelClientIdSet.has( clientId ) ) {
			return;
		}
		...
		reconciledCurrentBlocks = replaceBlockByClientId(
			reconciledCurrentBlocks,
			clientId,
			reconciledBlock
		);
	} );

	return reconciledCurrentBlocks;
}
...
function reconcileStaleLocalBlocks(
	...
	let currentBlocks = yblocks.toJSON() as Block[];
	...
	currentBlocks = reconcileMovedStaleLocalBlocks(
		localBlocksToSync,
		previousBlocks,
		currentBlocks
	);
