...
function reconcileStaleLocalBlockValues(
	...
	const localClientIds = getClientIdsIfEveryBlockHasUniqueId( localBlocks );
	...
	const blocksToSync = ( reconciledBlocks ?? localBlocks ).filter(
		( block ) => {
			const clientId = getBlockClientId( block );
...
function reconcileStaleLocalBlocks(
	...
	return reconcileStaleLocalBlockValues(
		localBlocksToSync,
		previousBlocks,
		currentBlocks
	);
 ...
 * @param baseBlocks      Optional pre-change block snapshot used for stale local reconciliation.
 */
				...
				switch ( incomingBlockProperty ) {
					...
					case 'innerBlocks': {
						...
						const incomingClientId =
							getBlockClientId( incomingYBlock );
						const baseBlock = incomingClientId
							? baseBlocksToSync?.find(
									( block ) =>
