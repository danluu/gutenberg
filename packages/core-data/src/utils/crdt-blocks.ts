...
export function mergeCrdtBlocks(
	...
	hydrateBlockArrayElementIdsFromYBlocks( yblocks, localBlocksToSync );
	previousBlocksByYArray.set( yblocks, localBlocksToSync );
}

function hydrateBlockArrayElementIdsFromYBlocks(
	yblocks: YBlocks,
	blocks: Block[]
): void {
	for (
		let index = 0;
		index < Math.min( yblocks.length, blocks.length );
		index++
	) {
		hydrateBlockArrayElementIdsFromYBlock(
			yblocks.get( index ),
			blocks[ index ]
		);
	}
...
function hydrateBlockArrayElementIdsFromYBlock(
	yblock: YBlock,
	block: Block
): void {
	const yAttributes = yblock.get( 'attributes' );
	if ( yAttributes instanceof Y.Map ) {
		hydrateArrayElementIdsFromYValue( yAttributes, block.attributes );
	}

	const yInnerBlocks = yblock.get( 'innerBlocks' );
	if (
		yInnerBlocks instanceof Y.Array &&
		Array.isArray( block.innerBlocks )
	) {
		hydrateBlockArrayElementIdsFromYBlocks(
			yInnerBlocks as YBlocks,
			block.innerBlocks
		);
	}
}

function hydrateArrayElementIdsFromYValue(
	yValue: unknown,
	plainValue: unknown
): void {
	if ( yValue instanceof Y.Array && Array.isArray( plainValue ) ) {
		const usedYIndexes = new Set< number >();

		for ( let index = 0; index < plainValue.length; index++ ) {
			const plainElement = plainValue[ index ];
			const yIndex = findYArrayElementIndexForHydration(
				yValue,
				plainElement,
				index,
				usedYIndexes
			...
			usedYIndexes.add( yIndex );

			const yElement = yValue.get( yIndex );
			const yElementId = getArrayElementId( yElement );

			if ( yElementId && isRecord( plainElement ) ) {
				defineArrayElementId( plainElement, yElementId );
			}

			hydrateArrayElementIdsFromYValue( yElement, plainElement );
		}
		return;
	}

	if ( yValue instanceof Y.Map && isRecord( plainValue ) ) {
		for ( const [ key, innerPlainValue ] of Object.entries( plainValue ) ) {
			hydrateArrayElementIdsFromYValue(
				yValue.get( key ),
				innerPlainValue
			);
		}
...
function findYArrayElementIndexForHydration(
	yArray: Y.Array< unknown >,
	plainElement: unknown,
	preferredIndex: number,
	usedYIndexes: Set< number >
): number {
	const plainElementId = getArrayElementId( plainElement );

	if ( plainElementId ) {
		for ( let index = 0; index < yArray.length; index++ ) {
			if (
				! usedYIndexes.has( index ) &&
				getArrayElementId( yArray.get( index ) ) === plainElementId
			) {
				return index;
			}
		}
	...
	if (
		...
		! usedYIndexes.has( preferredIndex ) &&
		areArrayElementsEqual( plainElement, yArray.get( preferredIndex ) )
	) {
	...
	for ( let index = 0; index < yArray.length; index++ ) {
		if (
			! usedYIndexes.has( index ) &&
			areArrayElementsEqual( plainElement, yArray.get( index ) )
		) {
			return index;
		}
	}
		...
		Object.entries( block ).forEach( ( [ key, value ] ) => {
			switch ( key ) {
				case 'attributes': {
					...
					const incomingAttributes = value as BlockAttributes;
				...
				default: {
					...
					if ( previousBlock ) {
						const previousBlockRecord =
							previousBlock as unknown as Record<
								string,
								unknown
							>;
						previousValue = previousBlockRecord[ key ];
					}

					if (
						previousBlock &&
						arePlainValuesEqual( value, previousValue )
					) {
						break;
					}
...
function mergeYArrayByElementIds(
	...
	if (
		! newValue.some( getArrayElementId ) ||
		! newValue.every( getArrayElementId )
	) {
		return false;
	}
...
function findYArrayElementIndex(
	...
	if ( previousElementId ) {
		for ( let i = 0; i < yArray.length; i++ ) {
			if ( getArrayElementId( yArray.get( i ) ) === previousElementId ) {
				return i;
			}
		}
...
function mergeYArrayLocalChanges(
	...
	// inserts/deletes/reorders unless the previous snapshot carries element IDs
	// that need to be preserved across a cloned incoming snapshot.
	if (
		isYArrayEqualToPlainArray( yArray, previousValue ) &&
	...
	for ( let offset = 0; offset < pairedCount; offset++ ) {
		...
		let newId = getArrayElementId( newElement );

		if ( previousId && ! newId && isRecord( newElement ) ) {
			defineArrayElementId( newElement, previousId );
			newId = previousId;
		}
	...
	for ( let i = previousMiddleEnd - 1; i >= left + pairedCount; i-- ) {
		const currentIndex = findYArrayElementIndex(
			yArray,
			previousValue[ i ],
			i,
	...
	if ( newMiddleEnd > left + pairedCount ) {
		const insertIndex = findYArrayLocalChangeInsertIndex(
			yArray,
			previousValue,
			left,
			previousMiddleEnd,
...
function mergeYArray(
	...
	for ( let i = 0; i < numOfUpdatesNeeded; i++ ) {
		...
		const previousElement = previousValue?.[ left + i ];
	...
	if ( numOfInsertionsNeeded > 0 ) {
		...
		for ( let i = 0; i < numOfInsertionsNeeded; i++ ) {
			itemsToInsert[ i ] = createYMapFromQuery(
				query,
				newValue[ insertAt + i ],
				[ ...cursorScope.attributePath, String( insertAt + i ) ].join(
					'/'
				),
				true
			);
 ...
 *                               The previous plain local value for the attribute.
 */
