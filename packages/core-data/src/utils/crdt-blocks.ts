...
function getUniqueClientIds( blocks: Block[] ): string[] | null {
	const ids: string[] = [];
...
function getBlocksByUniqueClientId(
	blocks: Block[]
): Map< string, Block > | null {
	const clientIds = getUniqueClientIds( blocks );
...
function reconcileStaleLocalBlockValues(
	...
	const previousBlocksByClientId =
		getBlocksByUniqueClientId( previousBlocks );
	const currentBlocksByClientId = getBlocksByUniqueClientId( currentBlocks );
...
function reconcileStaleLocalBlocks(
	...
	const localClientIds = getUniqueClientIds( localBlocksToSync );
	const previousClientIds = getUniqueClientIds( previousBlocks );
	const currentBlocks = yblocks.toJSON() as Block[];
	const currentClientIds = getUniqueClientIds( currentBlocks );
...
function resolveRichTextCursorPosition(
	...
	if ( typeof cursorPosition === 'number' ) {
		return richTextOffsetToHtmlIndex(
			updatedValue,
			createRichTextOffset( cursorPosition )
		);
	}
