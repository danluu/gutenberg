...
export function mergeCrdtBlocks(
	...
	const baseBlocksToSync = mergeOptions.baseBlocks
...
function mergeYArrayByElementIds(
	...
	const baseElementsById = new Map< string, unknown >();
	if ( Array.isArray( baseValue ) ) {
		baseValue.forEach( ( baseElement ) => {
			const baseId = getArrayElementId( baseElement );
			if ( baseId ) {
				baseElementsById.set( baseId, baseElement );
			}
		} );
	}
		...
		if ( currentIndex >= index ) {
			...
			if ( currentElement instanceof Y.Map && isRecord( newElement ) ) {
				mergeYMapValues(
					...
					query,
					cursorPosition,
					appendCursorScopeKey( cursorScope, index.toString() ),
					newId ? baseElementsById.get( newId ) : undefined
				);
			}
		} else {
			yArray.insert( index, [
