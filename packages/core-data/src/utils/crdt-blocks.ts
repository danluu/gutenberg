...
function hasSameRelativeOrder( baselineIds: string[], candidateIds: string[] ) {
	const candidateIdSet = new Set( candidateIds );
	const baselineSharedIds = baselineIds.filter( ( clientId ) =>
		candidateIdSet.has( clientId )
	);
	const baselineSharedIdSet = new Set( baselineSharedIds );
	const candidateSharedIds = candidateIds.filter( ( clientId ) =>
		baselineSharedIdSet.has( clientId )
	);

	return fastDeepEqual( baselineSharedIds, candidateSharedIds );
}
...
function reconcileStaleLocalBlock(
	...
	const reconciledInnerBlocks = reconcileStaleLocalBlockList(
...
function reconcileStaleLocalBlockList(
	...
	const previousLocalSharedClientIds = previousClientIds.filter(
		( clientId ) => localClientIdSet.has( clientId )
	);
	const localOrderChanged =
		previousLocalSharedClientIds.length < 2 ||
		! hasSameRelativeOrder( previousClientIds, localClientIds );

	if ( ! localOrderChanged ) {
		const localBlocksByClientId = new Map(
			reconciledLocalBlocks.map( ( block ) => [
		...
		const mergedBlocks = currentBlocks
			...
			.map( ( currentBlock ) => {
				const clientId = getBlockClientId( currentBlock );
				return clientId && localBlocksByClientId.has( clientId )
					? ( localBlocksByClientId.get( clientId ) as Block )
					: currentBlock;
			} );
		const mergedBlockClientIds = new Set(
			mergedBlocks
				.map( ( block ) => getBlockClientId( block ) )
				.filter( Boolean ) as string[]
		);

		reconciledLocalBlocks.forEach( ( localBlock ) => {
			const clientId = getBlockClientId( localBlock );

			if (
				clientId &&
				( previousClientIdSet.has( clientId ) ||
					mergedBlockClientIds.has( clientId ) )
			) {
				return;
			}
			...
			mergedBlocks.splice( insertIndex, 0, localBlock );
			if ( clientId ) {
				mergedBlockClientIds.add( clientId );
			}
		} );
	...
	// inserts/deletes are not inferred as local structural edits.
...
function reconcileStaleLocalBlocks(
	...
	return reconcileStaleLocalBlockList(
		localBlocksToSync,
		previousBlocks,
		currentBlocks
	);
...
export function recordCrdtBlocksLocalSnapshot(
	yblocks: YBlocks,
	blocks: Block[]
): void {
	previousBlocksByYArray.set( yblocks, makeBlocksSerializable( blocks ) );
}
