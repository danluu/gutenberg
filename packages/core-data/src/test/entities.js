...
describe( 'prePersistPostType', () => {
	...
	it( 'snapshots saved content into the CRDT before serializing the persisted document', async () => {
		...
		expect( syncManager.update.mock.invocationCallOrder[ 0 ] ).toBeLessThan(
			syncManager.createPersistedCRDTDoc.mock.invocationCallOrder[ 0 ]
		);
	...
	it( 'does not replace edited content from CRDT when the latest record has no persisted CRDT document', async () => {
		...
		const syncManager = {
			...
			createPersistedCRDTDoc: jest
				.fn()
				.mockResolvedValue( 'after-snapshot-doc' ),
			getCRDTRecordData: jest.fn( () => ( {
