...
function mergeSharedPrefixAppendedBlockContent( latestContent, localContent ) {
	if (
		typeof latestContent !== 'string' ||
		typeof localContent !== 'string'
	) {
	...
	if (
		...
		localBlocks.length < 2 ||
		( latestBlocks.length === localBlocks.length &&
			latestBlocks.every( ( _block, index ) =>
				areSerializedBlocksEqualAt( latestBlocks, localBlocks, index )
			) )
	) {
		return;
	}
	...
	while (
		sharedPrefixLength < latestBlocks.length &&
		sharedPrefixLength < localBlocks.length &&
	...
	return __unstableSerializeAndClean( [
		...localBlocks,
		...latestBlocks.slice( sharedPrefixLength ),
	] );
}

function mergeStaleContentWithLatest(
	baseContent,
	latestContent,
	localContent
) {
	...
	if ( mergedContent !== undefined && mergedContent !== localContent ) {
		return mergedContent;
	}

	return mergeSharedPrefixAppendedBlockContent( latestContent, localContent );
}

function getRecordForCRDTSerialization( edits, newEdits ) {
	const record = { ...edits, ...newEdits };
	const content = getRawPostValue( record.content );

	if ( typeof content === 'string' ) {
		record.blocks = parse( content );
	}
...
export const prePersistPostType = async (
	...
	if (
		...
		try {
			...
			if (
				...
				if (
					...
					for ( const key of locallyChangedSavedFields ) {
						...
						if ( hasCRDTValue ) {
							let crdtValue = getCRDTRawPostValue(
								crdtRecord,
								key
							);
							...
							if ( key === 'content' ) {
								...
								if (
									...
									mergedContent !== crdtValue
								) {
									crdtValue = mergedContent;
								}
							}
			...
			if (
				...
				const mergedContent = mergeStaleContentWithLatest(
	...
	if ( persistedRecord ) {
		serializedDoc = await (
			...
			record: getRecordForCRDTSerialization( edits, newEdits ),
		} );
