...
function seededRangeFromEnv(
	...
	const count = readIntFromEnv( countEnvName ) ?? defaultCount;
	const start = readIntFromEnv( startEnvName ) ?? defaultStart;

	if ( count < 0 ) {
		throw new Error( `Expected ${ countEnvName } to be non-negative.` );
	}

	return Array.from( { length: count }, ( _value, index ) => start + index );
}

const NESTED_SELECTION_SEEDS = seededRangeFromEnv(
	'GUTENBERG_RTC_SELECTION_FUZZ_SEED_COUNT',
	'GUTENBERG_RTC_SELECTION_FUZZ_SEED_START',
	8,
	1401
);
