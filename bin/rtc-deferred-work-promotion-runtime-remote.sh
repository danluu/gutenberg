...
CRITICAL_PATH_BASE=/media/volume/danluu-fuzz-data/rtc-critical-path-pr-executor-20260517
...
RELOAD_HYDRATION_EXACT_BLOCKER_CLASSIFICATION=${RTC_DEFERRED_WORK_RELOAD_HYDRATION_EXACT_BLOCKER_CLASSIFICATION:-"/media/volume/danluu-fuzz-data/rtc-critical-path-pr-executor-20260517/runs/reload-hydration/seed-7800014-repair-or-downscope/classification.tsv"}
RELOAD_HYDRATION_REPLAY_DOWNSCOPE_CLASSIFICATION=${RTC_DEFERRED_WORK_RELOAD_HYDRATION_REPLAY_DOWNSCOPE_CLASSIFICATION:-"/media/volume/danluu-fuzz-data/rtc-critical-path-pr-executor-20260517/runs/20260528T201206Z/continuations/reload-hydration-reproducer-reacquire/classification.tsv"}
RELOAD_HYDRATION_REPLAY_DOWNSCOPE_ARTIFACT=${RTC_DEFERRED_WORK_RELOAD_HYDRATION_REPLAY_DOWNSCOPE_ARTIFACT:-"/media/volume/danluu-fuzz-data/rtc-critical-path-pr-executor-20260517/runs/20260528T201206Z/continuations/reload-hydration-reproducer-reacquire/artifacts/exact-current-head-2/rtc-behavioral-coverage-7800014.json"}
PRE_SAVE_SEARCH_EXACT_BLOCKER_ID=${RTC_DEFERRED_WORK_PRE_SAVE_SEARCH_EXACT_BLOCKER_ID:-"pa-exact-pre-save-search-live-collapsede2b5502da052bb466094218380220a2c25812c2"}
...
CODEX_MODEL=${RTC_DEFERRED_WORK_CODEX_MODEL:-gpt-5.6-sol}
CODEX_REASONING_EFFORT=${RTC_DEFERRED_WORK_CODEX_REASONING_EFFORT:-max}
ACTIVE_SLEEP_PID=
...
acquire_singleton_lock() {
	...
	printf '%s\n' "$$" > "$pid_file"
	trap cleanup_deferred_loop EXIT TERM INT HUP
}

cleanup_deferred_loop() {
	local rc=$?
	trap - EXIT TERM INT HUP
	if [ -n "${ACTIVE_SLEEP_PID:-}" ] && kill -0 "$ACTIVE_SLEEP_PID" 2>/dev/null; then
		kill -TERM "$ACTIVE_SLEEP_PID" 2>/dev/null || true
		wait "$ACTIVE_SLEEP_PID" 2>/dev/null || true
	fi
	rm -f "$BASE/deferred-work-promotion-loop.pid"
	exit "$rc"
}
...
manifest_validation_summary() {
	local manifest=$1
	awk -F '\t' 'NR == 2 { print $8 }' "$manifest" 2>/dev/null || true
}

manifest_reason() {
	local manifest=$1
	awk -F '\t' 'NR == 2 { print $9 }' "$manifest" 2>/dev/null || true
}

manifest_has_full_build_evidence() {
	local manifest=$1 summary
	summary=$(manifest_validation_summary "$manifest")
	printf '%s' "$summary" | grep -Eiq '(npm run build|full build).*pass|pass.*(npm run build|full build)'
}

manifest_is_product_candidate() {
	local manifest=$1 reason
	reason=$(manifest_reason "$manifest")
	printf '%s' "$reason" | grep -Eiq 'product candidate|product fix|Product candidate|Product fix'
}

manifest_is_valid_hold_artifact() {
	local family=$1 manifest=$2 summary reason
	[ -n "$manifest" ] || return 1
	summary=$(manifest_validation_summary "$manifest")
	reason=$(manifest_reason "$manifest")
	case "$family" in
		reload-hydration)
			# Reload-hydration touches high-blast-radius RTC runtime code and is only
			# useful for promotion if the exact candidate can build browser assets.
			# Unit/lint-only manifests previously caused a bad single-flight hold.
			if manifest_is_product_candidate "$manifest" && ! manifest_has_full_build_evidence "$manifest"; then
				return 1
			fi
			if printf '%s %s' "$summary" "$reason" | grep -Eiq 'exact(-stack)? browser validation not run|exact browser replay not run'; then
				return 1
			fi
			;;
	esac
...
manifest_source_branch() {
	local manifest=$1
	awk -F '\t' 'NR == 2 { print $1 }' "$manifest" 2>/dev/null || true
}

manifest_source_head() {
	local manifest=$1
	awk -F '\t' 'NR == 2 { print $2 }' "$manifest" 2>/dev/null || true
}

manifest_adopted_or_published() {
	local manifest=$1 branch head
	branch=$(manifest_source_branch "$manifest")
	head=$(manifest_source_head "$manifest")
	[ -n "$branch" ] && [ -n "$head" ] && [ -s "$LOCAL_PUBLISH_MANIFEST" ] || return 1
	awk -F '\t' -v branch="$branch" -v head="$head" '
		NR > 1 && $5 == head && ( $6 == branch || $6 == "refs/heads/" branch ) {
			found = 1
		}
		END { exit found ? 0 : 1 }
...
family_manifest_adopted() {
	local family=$1 manifest
	manifest=$(latest_family_manifest "$family" || true)
	[ -n "$manifest" ] || return 1
	manifest_is_valid_hold_artifact "$family" "$manifest" || return 1
	manifest_adopted_or_published "$manifest"
}
...
family_manifest_hold() {
	...
	[ -n "$manifest" ] || return 1
	manifest_is_valid_hold_artifact "$family" "$manifest" || return 1
	manifest_adopted_or_published "$manifest" && return 1
	now=$(date -u +%s)
...
family_manifest_hold_summary() {
	local family=$1 manifest now mtime age validity adoption
	manifest=$(latest_family_manifest "$family" || true)
	if [ -z "$manifest" ]; then
		printf 'none'
		return
	...
	mtime=$(stat -c %Y "$manifest" 2>/dev/null || printf '0')
	age=$(( now - mtime ))
	if manifest_is_valid_hold_artifact "$family" "$manifest"; then
		validity=valid
	else
		validity=build-or-exact-validation-missing
	fi
	if manifest_adopted_or_published "$manifest"; then
		adoption=adopted
	else
		adoption=unadopted
	fi
	printf 'age_seconds=%s hold_seconds=%s validity=%s adoption=%s path=%s' "$age" "$ARTIFACT_HOLD_SECONDS" "$validity" "$adoption" "$manifest"
}

diagnostic_family_hold_reason() {
	local family=$1 report
	family_is_diagnostic "$family" || return 1
	report=$(latest_family_report "$family" || true)
	if [ -n "$report" ] && grep -Eiq 'documented downscope|diagnostic[/-]downscope|diagnostic[/-]downscoped|diagnostic-only decision|terminal diagnostic|keep this family (diagnostic-held/)?downscoped|should remain diagnostic-held/downscoped|product promotion remains downscoped|suppress product promotion|suppress generic relaunch|stop additional generic or diagnostic' "$report" 2>/dev/null; then
		printf 'downscoped:explicit-diagnostic-report=%s manifest=%s' "$report" "$(family_manifest_hold_summary "$family")"
		return 0
	fi
	if [ -n "$report" ] && grep -Eiq 'diagnostic-only|diagnostic only|no product-owned|no product owner|no product code changed|no product promotion|product promotion waits|product promotion remains downscoped|do not count this as product-ready' "$report" 2>/dev/null; then
		printf 'diagnostic-held:%s' "$(family_manifest_hold_summary "$family")"
		return 0
	fi
	return 1
}
...
family_hold_reason() {
	...
	if [ "$family" = "reload-hydration" ] && reload_hydration_replay_downscope_active; then
		printf 'downscoped:%s' "$(reload_hydration_replay_downscope_summary)"
		return
	fi
	if [ "$family" = "reload-hydration" ] && reload_hydration_exact_blocker_active; then
		printf 'exact-blocker-held:%s' "$(reload_hydration_exact_blocker_summary)"
		return
	fi
	if [ "$family" = "pre-save-search-live-collapse" ] && pre_save_search_exact_blocker_active; then
		printf 'exact-blocker-held:%s' "$(pre_save_search_exact_blocker_summary)"
		return
	fi
	if family_manifest_adopted "$family"; then
		printf 'manifest-adopted:%s' "$(family_manifest_hold_summary "$family")"
		return
	fi
	if diagnostic_family_hold_reason "$family"; then
		return
	fi
	...
	case "$reason" in
		none)
			...
			manifest-hold:*)
				...
				;;
			manifest-adopted:*)
				printf 'manifest-adopted'
				;;
			diagnostic-held:*)
				printf 'diagnostic-held'
				;;
		exact-blocker-held:*)
			printf 'exact-blocker-held'
			;;
...
reload_hydration_exact_blocker_active() {
	[ "${RTC_DEFERRED_WORK_RELOAD_HYDRATION_GENERIC_OK:-0}" = "1" ] && return 1
	[ -s "$RELOAD_HYDRATION_EXACT_BLOCKER_CLASSIFICATION" ] || return 1
	grep -Eq '^reload-hydration-seed7800014[[:space:]]+true_exact_product_blocker_needs_repair_or_downscope[[:space:]]' "$RELOAD_HYDRATION_EXACT_BLOCKER_CLASSIFICATION"
}

reload_hydration_exact_blocker_summary() {
	printf 'seed-7800014-final-ui-witness classification=%s' "$RELOAD_HYDRATION_EXACT_BLOCKER_CLASSIFICATION"
}
...
reload_hydration_replay_downscope_active() {
	...
	if [ -s "$RELOAD_HYDRATION_EXACT_BLOCKER_CLASSIFICATION" ]; then
		if grep -Eq '^reload-hydration-seed7800014[[:space:]]+replay_backed_downscope[[:space:]]' "$RELOAD_HYDRATION_EXACT_BLOCKER_CLASSIFICATION"; then
			return 0
		fi
...
reload_hydration_replay_downscope_summary() {
	printf 'replay-backed-downscope classification=%s artifact=%s' "$RELOAD_HYDRATION_REPLAY_DOWNSCOPE_CLASSIFICATION" "$RELOAD_HYDRATION_REPLAY_DOWNSCOPE_ARTIFACT"
}

pre_save_search_exact_blocker_active() {
	[ "${RTC_DEFERRED_WORK_PRE_SAVE_SEARCH_GENERIC_OK:-0}" = "1" ] && return 1
	[ -s "$CRITICAL_PATH_BASE/blockers.tsv" ] || return 1
	awk -F '\t' -v id="$PRE_SAVE_SEARCH_EXACT_BLOCKER_ID" '
		NR > 1 && $1 == id && $4 !~ /^(terminal|closed|resolved)$/ { found = 1 }
		END { exit found ? 0 : 1 }
	' "$CRITICAL_PATH_BASE/blockers.tsv"
}

pre_save_search_exact_blocker_summary() {
	printf '%s blockers=%s' "$PRE_SAVE_SEARCH_EXACT_BLOCKER_ID" "$CRITICAL_PATH_BASE/blockers.tsv"
}
...
write_deferred_queue() {
	...
	strict=$(cat "$STRICT_BASE/current-run-root.txt" 2>/dev/null || true)
	{
		...
		for family in $FAMILIES; do
			...
			case "$family" in
				reload-hydration)
					if [ "$state" = "downscoped" ]; then
						printf '%s\t%s\tdownscoped\treplay-backed downscope consumed for seed 7800014 final-ui-witness; suppress generic reload-hydration relaunch until newer same-head final-ui product evidence appears\t%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$family" "$coverage"
					elif [ "$state" = "exact-blocker-held" ]; then
						printf '%s\t%s\theld\texact seed 7800014 final-ui-witness blocker is unresolved; do not relaunch generic reload-hydration heads\t%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$family" "$coverage"
					elif [ "$state" = "manifest-adopted" ]; then
						printf '%s\t%s\tadopted\tvalidated manifest is already recorded in the local publish ledger; suppress generic reload-hydration relaunch and continue exact-stack/benchmark proof on the adopted head\t%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$family" "$coverage"
					elif [ "$state" = "single-flight-held" ]; then
					...
					;;
				pre-save-search-live-collapse)
					if [ "$state" = "exact-blocker-held" ]; then
						printf '%s\t%s\theld\texact blocker %s owns progress; no generic relaunch until focused replay, green adoption, or explicit downscope\t%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$family" "$PRE_SAVE_SEARCH_EXACT_BLOCKER_ID" "$coverage"
					elif [ "$state" = "downscoped" ]; then
						printf '%s\t%s\tdownscoped\texplicit diagnostic downscope report consumed; suppress product promotion and generic relaunch until repeatable core/search first-loss evidence appears\t%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$family" "$coverage"
					elif [ "$state" = "single-flight-held" ] || [ "$state" = "cooldown-held" ]; then
				...
				rich-text-suffix-corruption)
					if [ "$state" = "downscoped" ]; then
						printf '%s\t%s\tdownscoped\texplicit diagnostic downscope report consumed; suppress generic relaunch until a suffix/content ledger proves deterministic product-owned first loss\t%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$family" "$coverage"
					elif [ "$state" = "single-flight-held" ] || [ "$state" = "cooldown-held" ]; then
...
8a. For product candidates in high-blast-radius RTC runtime families such as reload-hydration, do not write a push manifest unless a clean full npm run build has passed on the exact candidate head, or the report explicitly marks the branch blocked with no push manifest. Do not dismiss TypeScript/build failures as unrelated baseline when the candidate touches the failing package or will be browser-replayed.
...
while true; do
	...
	if load_too_high; then
		...
		sleep_cycle "$CYCLE_SLEEP_SECONDS"
		continue
	fi
	...
	while IFS= read -r family; do
		...
		if [ "$family" = "reload-hydration" ] && reload_hydration_replay_downscope_active; then
			log "family downscoped family=$family summary=$(reload_hydration_replay_downscope_summary)"
			continue
		fi
		if [ "$family" = "reload-hydration" ] && reload_hydration_exact_blocker_active; then
			log "family exact blocker held family=$family summary=$(reload_hydration_exact_blocker_summary)"
			continue
		fi
			if [ "$family" = "pre-save-search-live-collapse" ] && pre_save_search_exact_blocker_active; then
				log "family exact blocker held family=$family summary=$(pre_save_search_exact_blocker_summary)"
				continue
			fi
			if family_manifest_adopted "$family"; then
				log "family manifest already adopted; suppressing relaunch family=$family summary=$(family_manifest_hold_summary "$family")"
				continue
			fi
		...
		diagnostic_hold_reason=$(diagnostic_family_hold_reason "$family" || true)
		if [ -n "$diagnostic_hold_reason" ]; then
			log "family diagnostic-held family=$family summary=$diagnostic_hold_reason"
			continue
		fi
		if family_manifest_hold "$family"; then
