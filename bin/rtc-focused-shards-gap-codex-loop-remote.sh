...
select_focused_run_root() {
	node - "$BASE/current-run-roots.txt" "$BASE/current-run-root.txt" <<'NODE'
const fs = require( 'fs' );
...
for ( const root of roots ) {
	...
	try {
		...
		lastUpdatedAt = Date.parse( state.lastUpdatedAt || '' ) || 0;
		activeRunDirs = ( state.groups || [] ).reduce(
			( sum, group ) => sum + ( group.activeRunDirs || [] ).length,
			0
		);
	} catch {}

	const recentActive =
		activeRunDirs > 0 && lastUpdatedAt > 0 && now - lastUpdatedAt < 15 * 60 * 1000;
	const score = ( recentActive ? 10 ** 15 : 0 ) + ( lastUpdatedAt || stat.mtimeMs );
...
if ( best ) {
	process.stdout.write( best.root );
}
NODE
}
...
3. If the focused shards are not producing useful coverage, propose the smallest useful adjustment to shard env/group policy or harness code.
...
6. Name focused syntax/lint checks for the proposal. Do not edit files or restart loops from this review job.
7. If a change must affect active fuzzing, describe the exact focused-shards restart scope; an isolated structural or critical controller must review and apply it.
