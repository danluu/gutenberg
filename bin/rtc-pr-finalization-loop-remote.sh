...
CODEX_MODEL=${RTC_PR_FINALIZATION_CODEX_MODEL:-gpt-5.6-sol}
CODEX_REASONING_EFFORT=${RTC_PR_FINALIZATION_CODEX_REASONING_EFFORT:-max}
...
prepare_finalization_worktree_deps() {
	local worktree=$1
	local pkg_modules pkg pkg_name
	if [ -e "$SRC/node_modules" ] && [ ! -e "$worktree/node_modules" ]; then
		if ! cp -al "$SRC/node_modules" "$worktree/node_modules" 2>/dev/null; then
			ln -s "$SRC/node_modules" "$worktree/node_modules" 2>/dev/null || \
				log "warning: unable to attach node_modules to finalization worktree $worktree"
		fi
	fi
	for pkg_modules in "$SRC"/packages/*/node_modules; do
		[ -e "$pkg_modules" ] || continue
		pkg=${pkg_modules%/node_modules}
		pkg_name=${pkg##*/}
		mkdir -p "$worktree/packages/$pkg_name"
		[ -e "$worktree/packages/$pkg_name/node_modules" ] || \
			cp -al "$pkg_modules" "$worktree/packages/$pkg_name/node_modules" 2>/dev/null || true
	done
	if [ ! -e "$worktree/test/e2e/node_modules" ] && [ -e "$SRC/node_modules/@wordpress/e2e-tests-playwright/node_modules" ]; then
		mkdir -p "$worktree/test/e2e"
		ln -s ../../node_modules/@wordpress/e2e-tests-playwright/node_modules "$worktree/test/e2e/node_modules" 2>/dev/null || \
			cp -al "$SRC/node_modules/@wordpress/e2e-tests-playwright/node_modules" "$worktree/test/e2e/node_modules" 2>/dev/null || true
	fi
	if [ -e "$SRC/vendor" ] && [ ! -e "$worktree/vendor" ]; then
		cp -al "$SRC/vendor" "$worktree/vendor" 2>/dev/null || true
	fi
}
...
launch_finalization_job() {
	local ts cycle_dir branch worktree prompt report codex_log runner session
	ts=$(date -u +%Y%m%dT%H%M%SZ)
	...
	prepare_finalization_worktree_deps "$worktree"
	...
	runner="$cycle_dir/finalization.run.sh"
...
export PATH="$CODEX_BIN_DIR:$TMUX_WRAP:$NODE_BIN:\$PATH"
set +e
"$CODEX_BIN_DIR/codex" -a never exec --skip-git-repo-check -m "$CODEX_MODEL" -c model_reasoning_effort="$CODEX_REASONING_EFFORT" -s danger-full-access < "$prompt" > "$report" 2> "$codex_log"
code=\$?
set -e
if [ ! -s "$report" ] && [ -s "$codex_log" ]; then
	{
		echo "# PR Finalization Stderr Transcript"
		echo
		echo "- reason: Codex produced no stdout report; preserving stderr so the finalization pass is inspectable."
		echo "- exit_code: \$code"
		echo "- worktree: $worktree"
		echo "- prompt: $prompt"
		echo "- stderr: $codex_log"
		echo
		echo '\`\`\`text'
		cat "$codex_log"
		echo
