...
tmux new-session -d -s "$ANALYSIS_SESSION" "bash -lc 'cd \"$HARNESS_SRC\"; export PATH=\"$CODEX_BIN_DIR:$TMUX_WRAP:$NODE_BIN:\$PATH\" CI=1; RTC_FUZZ_LIVE_ANALYSIS_REPO_ROOT=\"$HARNESS_SRC\" RTC_FUZZ_LIVE_ANALYSIS_CURRENT_OUTPUT_POINTER=\"$RUN/current-run-root.txt\" RTC_FUZZ_LIVE_ANALYSIS_INTERVAL_MS=120000 RTC_FUZZ_LIVE_ANALYSIS_MAX_PARALLEL=4 RTC_FUZZ_LIVE_ANALYSIS_MAX_ATTEMPTS=4 RTC_FUZZ_LIVE_ANALYSIS_CODEX_TIMEOUT_MS=2700000 RTC_FUZZ_LIVE_ANALYSIS_TMUX_PREFIX=\"$ANALYSIS_TMUX_PREFIX\" RTC_FUZZ_LIVE_ANALYSIS_ENABLE_DEEP=1 RTC_FUZZ_LIVE_DEEP_ANALYSIS_MAX_PARALLEL=2 node bin/rtc-browser-fuzz-live-analysis-monitor.mjs \"$RUN\" >> \"$ANALYSIS_LOG\" 2>&1'"
if [ "$RTC_FOCUSED_SHARDS_APPEND_CURRENT" = 1 ]; then
