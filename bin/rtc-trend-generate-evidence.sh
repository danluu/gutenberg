...
bug_output_effectiveness_path = artifact / "data" / "bug_output_effectiveness_by_level.csv"
...
bug_output_effectiveness_latest = []
if bug_output_effectiveness_path.exists():
	with bug_output_effectiveness_path.open(newline="") as f:
		rows = list(csv.DictReader(f))
	if rows:
		latest_bucket = max(row.get("bucket", "") for row in rows)
		for row in rows:
			if row.get("bucket", "") != latest_bucket:
				continue
			bug_output_effectiveness_latest.append(
				(
					row.get("fuzz_level", ""),
					row.get("cumulative_unique_bug_outputs", ""),
					row.get("cumulative_runner_hours", ""),
					row.get("unique_bug_outputs_per_100_runner_hours", ""),
					row.get("cumulative_confirmed_likely_real_outputs", ""),
					row.get("confirmed_likely_real_outputs_per_100_runner_hours", ""),
				)
			)
...
lines += ["", "## Latest Unique Bug-Output Candidate Effectiveness"]
if bug_output_effectiveness_latest:
	for level, unique_outputs, runner_hours, output_rate, confirmed_outputs, confirmed_rate in sorted(bug_output_effectiveness_latest):
		lines.append(
			f"- {level}: unique_bug_output_candidates={unique_outputs}, runner_hours={runner_hours}, unique_candidates_per_100_runner_hours={output_rate}, confirmed_likely_real_outputs={confirmed_outputs}, confirmed_likely_real_outputs_per_100_runner_hours={confirmed_rate}"
		)
else:
	lines.append("- no bug-output candidate effectiveness CSV rows available")
...
lines += [
	...
	"- Challenge whether the latest triaged likely-real outputs, unique bug-output candidates, and failed-attempts per runner-hour imply a mix shift. Do not treat failed attempts or untriaged unique candidates as confirmed bugs, and do not treat zero likely-real findings from an untriaged lower-level lane as proof the lane is useless.",
