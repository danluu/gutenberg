...
async function getNoAnalysisProductEvidenceDeepFamilyCounts(
	...
	await addCurrentOutputPeerNoAnalysisProductEvidenceDeepFamilyCounts(
		counts
	);
	return counts;
}
...
async function runScanCycle() {
	...
	if ( noAnalysis && ! isNoAnalysisLiveAdmissionActive( noAnalysis ) ) {
		state.lastLaunchSkippedForNoAnalysis = {
			at: new Date().toISOString(),
			reason: 'active producer no-analysis sentinel has no live-analysis admission; suppressed stale/noise jobs and exiting instead of keeping a warm deep-analysis loop',
			reasonKind: noAnalysis.reasonKind ?? null,
			family: noAnalysis.family ?? null,
			group: noAnalysis.group ?? null,
			hasProductEvidence:
	...
	if ( gateOnlyRefresh.status === 'failed' ) {
		state.lastLaunchSkippedForGateOnlyFailure = {
			at: new Date().toISOString(),
			reason: 'gate-only triage refresh failed; reconciled and suppressed stale/noise jobs but skipped new deep-analysis launches for this pass',
			gateOnlyRefresh,
		};
	} else {
