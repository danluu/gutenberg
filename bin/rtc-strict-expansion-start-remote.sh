...
STRICT_DURATION_HOURS=${RTC_STRICT_EXPANSION_DURATION_HOURS:-12}
...
E2E_UTILS_ARTIFACT_SOURCE=${RTC_STRICT_EXPANSION_E2E_UTILS_ARTIFACT_SOURCE:-/media/volume/danluu-fuzz-data/rtc-e2e-setup-20260514/gutenberg}
E2E_UTILS_PACKAGE_REL=packages/e2e-test-utils-playwright
required_e2e_utils_artifacts=(
	"$E2E_UTILS_PACKAGE_REL/build/index.js"
	"$E2E_UTILS_PACKAGE_REL/build/index.cjs"
	"$E2E_UTILS_PACKAGE_REL/build-types/index.d.ts"
)
...
has_required_e2e_utils_artifacts() {
	local repo=$1
	local artifact
	for artifact in "${required_e2e_utils_artifacts[@]}"; do
		[ -f "$repo/$artifact" ] || return 1
	done
}

e2e_utils_sources_match_artifact_source() {
	local repo=$1
	local label=$2
	local diff_log
	local source_pkg="$E2E_UTILS_ARTIFACT_SOURCE/$E2E_UTILS_PACKAGE_REL"
	local dest_pkg="$repo/$E2E_UTILS_PACKAGE_REL"

	if [ ! -d "$source_pkg" ] || [ ! -d "$dest_pkg" ]; then
		printf '[%s] strict expansion e2e-test-utils source compare unavailable profile=%s repo=%s dest_pkg=%s source_pkg=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$label" "$repo" "$dest_pkg" "$source_pkg" >&2
		return 1
	fi

	diff_log=$(mktemp "$BASE/e2e-utils-source-diff.XXXXXX")
	if diff -qr \
		--exclude=build \
		--exclude=build-types \
		...
		"$source_pkg" \
		"$dest_pkg" >"$diff_log"; then
		rm -f "$diff_log"
		return 0
	fi

	printf '[%s] strict expansion e2e-test-utils source differs from artifact source profile=%s repo=%s source=%s; refusing to hydrate shared artifacts\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$label" "$repo" "$E2E_UTILS_ARTIFACT_SOURCE" >&2
	sed -n '1,40p' "$diff_log" >&2
	rm -f "$diff_log"
	return 1
}

copy_e2e_utils_artifact_dir() {
	local rel=$1
	local repo=$2
	local source_path="$E2E_UTILS_ARTIFACT_SOURCE/$E2E_UTILS_PACKAGE_REL/$rel"
	local dest_path="$repo/$E2E_UTILS_PACKAGE_REL/$rel"

	if [ ! -d "$source_path" ]; then
		printf 'strict expansion e2e-test-utils artifact source missing path=%s\n' "$source_path" >&2
		return 1
	fi

	rm -rf "$dest_path"
	mkdir -p "${dest_path%/*}"
	cp -al "$source_path" "$dest_path" 2>/dev/null ||
		cp -a "$source_path" "$dest_path"
}

hydrate_ws_e2e_utils_artifacts() {
	local repo=$1
	local label=$2

	if has_required_e2e_utils_artifacts "$repo"; then
		return 0
	fi

	if ! e2e_utils_sources_match_artifact_source "$repo" "$label"; then
		return 1
	fi

	copy_e2e_utils_artifact_dir build "$repo" || return 1
	copy_e2e_utils_artifact_dir build-types "$repo" || return 1

	if ! has_required_e2e_utils_artifacts "$repo"; then
		printf 'strict expansion repo is missing e2e-test-utils build artifacts after hydration profile=%s repo=%s source=%s\n' "$label" "$repo" "$E2E_UTILS_ARTIFACT_SOURCE" >&2
		return 1
	fi

	printf '[%s] hydrated e2e-test-utils build artifacts profile=%s repo=%s source=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$label" "$repo" "$E2E_UTILS_ARTIFACT_SOURCE" >&2
}
...
strict_profile_enabled() {
	local profile=$1 enabled_names=${RTC_STRICT_EXPANSION_ENABLED_NAMES:-}
...
for idx in "${!profiles[@]}"; do
	p=${profiles[$idx]}
	strict_profile_enabled "$p" || continue
	if [[ $p == http-* ]]; then
		d="$REPOS_BASE/$p"
		...
		sync_rtc_browser_harness_from_http_source "$d" "$p"
		hydrate_ws_e2e_utils_artifacts "$d" "$p"
	...
	if [[ $p == ws-* ]] && ! has_required_e2e_utils_artifacts "$d"; then
		printf 'strict expansion WS repo is missing e2e-test-utils build artifacts after copy: %s\n' "$d" >&2
		exit 2
	fi
done
...
const enabledNames = new Set(
	( process.env.RTC_STRICT_EXPANSION_ENABLED_NAMES
		? process.env.RTC_STRICT_EXPANSION_ENABLED_NAMES.split( ',' )
		: [] )
		.map( ( name ) => name.trim() )
		.filter( Boolean )
);
...
const selectedSpecs = enabledNames.size
	? specs.filter( ( [ name ] ) => enabledNames.has( name ) )
	: specs;
if ( enabledNames.size && selectedSpecs.length === 0 ) {
	throw new Error(
		`RTC_STRICT_EXPANSION_ENABLED_NAMES matched no groups: ${ [
			...enabledNames,
		].join( ',' ) }`
	);
}
const groups = selectedSpecs.map((spec, i) => {
...

{
	...
	echo "WS package artifact audit: $E2E_UTILS_PACKAGE_REL build/index.js, build/index.cjs, and build-types/index.d.ts present in every WS repo."
} > "$RUN/strict-expansion.md"

tmux new-session -d -s rtc-fuzz-strict-expansion "bash -lc 'cd \"$HTTP_SRC\"; export PATH=\"$CODEX_BIN_DIR:$TMUX_WRAP:$NODE_BIN:\$PATH\" CI=1; while true; do RTC_FUZZ_SUPERVISOR_OUTPUT_DIR=\"$RUN\" RTC_FUZZ_SUPERVISOR_GROUPS_PATH=\"$RUN/supervisor-groups.json\" RTC_FUZZ_SUPERVISOR_DURATION_HOURS=\"$STRICT_DURATION_HOURS\" RTC_FUZZ_SUPERVISOR_POLL_MS=60000 RTC_FUZZ_TRIAGE_MAX_PARALLEL=1 RTC_FUZZ_INLINE_CODEX=0 RTC_FUZZ_SKIP_GLOBAL_POST_CLEANUP=1 RTC_FUZZ_LOW_DISK_MODE=1 RTC_FUZZ_PLAYWRIGHT_VIDEO=off node bin/rtc-browser-fuzz-supervisor.mjs >> \"$BASE/logs/supervisor.log\" 2>&1; code=\$?; printf \"SUPERVISOR_EXIT:%s %s\\n\" \"\$code\" \"\$(date -u +%Y-%m-%dT%H:%M:%SZ)\" >> \"$BASE/logs/supervisor.log\"; sleep 30; done'"
tmux new-session -d -s rtc-fuzz-strict-expansion-watchdog "bash -lc 'cd \"$HTTP_SRC\"; export PATH=\"$CODEX_BIN_DIR:$TMUX_WRAP:$NODE_BIN:\$PATH\" CI=1; while true; do RTC_FUZZ_WATCHDOG_REPO_ROOT=\"$HTTP_SRC\" RTC_FUZZ_WATCHDOG_OUTPUT_DIR=\"$RUN\" RTC_FUZZ_WATCHDOG_GROUPS_PATH=\"$RUN/supervisor-groups.json\" RTC_FUZZ_WATCHDOG_SESSION=rtc-fuzz-strict-expansion RTC_FUZZ_WATCHDOG_DURATION_HOURS=\"$STRICT_DURATION_HOURS\" RTC_FUZZ_WATCHDOG_POLL_MS=60000 node bin/rtc-browser-fuzz-watchdog.mjs >> \"$BASE/logs/watchdog.log\" 2>&1; code=\$?; printf \"WATCHDOG_EXIT:%s %s\\n\" \"\$code\" \"\$(date -u +%Y-%m-%dT%H:%M:%SZ)\" >> \"$BASE/logs/watchdog.log\"; sleep 30; done'"
tmux new-session -d -s rtc-fuzz-strict-expansion-analysis "bash -lc 'cd \"$HTTP_SRC\"; export PATH=\"$CODEX_BIN_DIR:$TMUX_WRAP:$NODE_BIN:\$PATH\" CI=1; RTC_FUZZ_LIVE_ANALYSIS_REPO_ROOT=\"$HTTP_SRC\" RTC_FUZZ_LIVE_ANALYSIS_INTERVAL_MS=120000 RTC_FUZZ_LIVE_ANALYSIS_MAX_PARALLEL=1 RTC_FUZZ_LIVE_ANALYSIS_MAX_ATTEMPTS=4 RTC_FUZZ_LIVE_ANALYSIS_CODEX_TIMEOUT_MS=2700000 node bin/rtc-browser-fuzz-live-analysis-monitor.mjs \"$RUN\" >> \"$BASE/logs/analysis.log\" 2>&1'"
