...
const PRODUCT_FAILURE_BASE =
	process.env.RTC_FUZZ_ANALYSIS_PRODUCT_FAILURE_BASE ?? null;
...
const INCLUDE_EXTERNAL_IMPORTS =
	process.env.RTC_FUZZ_ANALYSIS_INCLUDE_EXTERNAL_IMPORTS === '1';
...
function getGateOnlyTriageEnv() {
	...
	if ( SUPERVISOR_STATE_PATH ) {
		env.RTC_FUZZ_TRIAGE_SUPERVISOR_STATE_PATH = SUPERVISOR_STATE_PATH;
	}
...
async function readActiveSupervisorRunDirSet() {
	...
	for ( const group of supervisorState.groups ?? [] ) {
		...
		if (
			...
			Number( group.productFailureRecords ?? 0 ) > 0
		) {
			groupDirs.push( ...( group.productFailureRunDirs ?? [] ) );
		}
...
async function getCurrentOutputPeerRunDirs() {
	...
	if ( currentOutputRoot ) {
		for ( const dir of await findRunDirsWithTriageState(
			currentOutputRoot
		) ) {
...
function getSourceGroupName() {
	const baseName = path.basename( RUN_DIR );
	const match = baseName.match( /^(.*)-gen-[0-9]+-[0-9]{8}T[0-9]{6}Z$/ );
	return match?.[ 1 ] ?? baseName;
}

async function persistLikelyRealProductFailure( job, signature ) {
	if ( ! PRODUCT_FAILURE_BASE || ! job?.resultPath ) {
		return null;
	}
	...
	const sourceRoot = path.dirname( RUN_DIR );
	const candidateHead = (
		process.env.RTC_FUZZ_ANALYSIS_CANDIDATE_HEAD ??
		( await fs.readFile(
			path.join( sourceRoot, 'source-head.txt' ),
			'utf8'
		) )
	).trim();
	if ( ! /^[0-9a-f]{40}$/i.test( candidateHead ) ) {
		throw new Error(
			`Cannot persist actionable product failure without a full candidate head: ${ candidateHead }`
		);
	}

	const destination = path.join(
		PRODUCT_FAILURE_BASE,
		candidateHead,
		`${ signature.hash }.json`
	);
	...
	await writeJson( destination, {
		...
		userHitLikelihoodScore: result.userHitLikelihoodScore ?? null,
		distinctBugType: result.distinctBugType ?? 'unknown',
		summary: result.summary ?? 'Likely-real RTC browser fuzz failure',
		recommendedTriageAction:
			result.recommendedTriageAction ?? 'prioritize_deep_triage',
		sourceRoot,
		sourceRunDir: RUN_DIR,
		analysisPath: job.analysisPath ?? null,
		handoffPath: job.handoffPath ?? null,
...
function isCurrentActionableProductEvidenceSourceSignature( signature ) {
	...
	if ( NON_ACTIONABLE_SOURCE_STATUSES.has( signature.status ?? 'unknown' ) ) {
		return false;
	}
	if (
	...
	if (
		isFuzzHelperRestEndpointConstructionSignature( signature ) ||
		isRtcWsRuntimeConfigPortBleedSignature( signature )
	) {
		return false;
	}
...
async function suppressJobsWithInactiveSource(
	...
	for ( const job of Object.values( state.jobs ?? {} ) ) {
			...
			if ( job.status === 'source-suppressed' ) {
				...
				if ( sourceSignature ) {
					job.semanticFamilyKey =
						getSemanticFamilyKey( sourceSignature );
					job.sourceSemanticFamilyKey =
						getSourceSemanticFamilyKey( sourceSignature );
					job.launchFamilyKey =
						getAnalysisLaunchFamilyKey( sourceSignature );
		...
		job.pid = null;
		job.status = producerFamilyCapKey
			? 'family-capped'
			: 'source-suppressed';
		job.completedAt = new Date().toISOString();
...
function shouldRestoreProducerFamilyCappedProductEvidenceJob(
	...
	return (
		[ 'family-capped', 'source-suppressed' ].includes( job?.status ) &&
		noAnalysis?.preserveProductEvidence === true &&
		isCurrentActionableProductEvidenceSourceSignature( sourceSignature ) &&
...
function restoreProducerFamilyCappedProductEvidenceJob(
	...
	job.restoreReason =
		'live-admitted product-evidence representative had no durable first-level analysis result';
	job.sourceStatus = sourceSignature?.status ?? null;
	job.sourceCount = sourceSignature?.count ?? 0;
...
async function launchQueuedAnalysisJobs(
	sourceState,
	state,
	noAnalysis = null
) {
	...
	restoreNoAnalysisProductEvidenceFamilyCappedJobs(
		sourceState,
		state,
		noAnalysis
	);
	...
	for ( const signature of signatures ) {
		...
		if ( noAnalysisProductEvidenceFamilyKey ) {
			noAnalysisProductEvidenceFamilyCounts ??=
				await getNoAnalysisProductEvidenceFamilyLaunchCounts(
					sourceState,
					state,
					noAnalysis
				);
		...
		const familyLaunchCount = getAdjustedFamilyLaunchCountForSignature(
			familyLaunchCounts,
			state,
			signature,
			familyKey
...
async function getNoAnalysisProductEvidenceFamilyLaunchCounts(
	sourceState,
	state,
	noAnalysis
) {
	...
	addNoAnalysisProductEvidenceFamilyCountsFromJobs(
		counts,
		state.jobs,
		signatureByHash,
		noAnalysis
	);
	await addCurrentOutputPeerNoAnalysisProductEvidenceFamilyCounts( counts );
	return counts;
}

async function addCurrentOutputPeerNoAnalysisProductEvidenceFamilyCounts(
	counts
) {
	const currentOutput = await readCurrentOutputRoot();
	const currentOutputRoot = currentOutput?.root ?? null;
	for ( const peerRunDir of await getCurrentOutputPeerRunDirs() ) {
		...
		if (
			...
			! isNoAnalysisProductEvidenceRepresentativeCapActive(
				peerNoAnalysis
			)
		) {
			continue;
		...
		const [ peerSourceState, peerAnalysisState ] = await Promise.all( [
			readJson(
				path.join( peerRunDir, '.triage-watcher/state.json' ),
				null
			),
			readJson(
				path.join(
...
function addNoAnalysisProductEvidenceFamilyCountsFromJobs(
	counts,
	jobs,
	signatureByHash,
	noAnalysis
...
function recordNoAnalysisProductEvidenceFamilyCappedJob(
	...
	recordFamilyCappedJob( state, signature, familyKey );
	const job = state.jobs[ signature.hash ];
	job.reason =
		'product-preserving no-analysis sentinel already has a representative for this semantic family';
...
function recordActiveOnlyFamilyThrottleSkip( state, signature, familyKey ) {
	...
	state.lastActiveOnlyFamilyThrottleSkips.push( {
		...
		detailedFamilyKey: signature.familyKey ?? null,
		reason: 'ambiguous product-evidence timeout/unknown already has an active or completed first-level representative',
	} );
	state.lastActiveOnlyFamilyThrottleSkips =
...
function recordFamilyCapOnlySkip( state, signature, familyKey ) {
	...
	state.lastFamilyCapOnlySkips.push( {
		...
		semanticFamilyKey: getSemanticFamilyKey( signature ),
		detailedFamilyKey: signature.familyKey ?? null,
		reason: 'family-cap-only housekeeping pass left uncapped product-evidence signature visible for normal analysis',
	} );
...
function recordFamilyCappedJob( state, signature, familyKey ) {
	...
	state.jobs[ signature.hash ] = {
		...
		reason: 'semantic family already has an active/completed first-level analysis',
...
function getAdjustedFamilyLaunchCountForSignature(
	counts,
	state,
	signature,
	familyKey
) {
	let count = counts.get( familyKey ) ?? 0;
	const existing = state.jobs?.[ signature.hash ];
	if (
		existing &&
		! TERMINAL_JOB_STATUSES.has( existing.status ) &&
		shouldCountJobForFamilyCap( existing, signature ) &&
		getFamilyLaunchCountKeys( existing, signature ).includes( familyKey )
	) {
		count -= 1;
	}
...
function restoreNoAnalysisProductEvidenceFamilyCappedJobs(
	sourceState,
	state,
	noAnalysis
) {
	...
	for ( const job of Object.values( state.jobs ?? {} ) ) {
		...
		const signature = sourceState.signatures?.[ job.hash ];
		if (
			! isCurrentActionableProductEvidenceSourceSignature( signature )
		) {
			continue;
		...
		job.completedAt = null;
		job.sourceSuppressionReason = null;
		job.noAnalysisProductEvidenceFamilyCap = false;
		job.restoredAt = new Date().toISOString();
		job.restoreReason =
			'product-evidence no-analysis family cap must be recomputed with only safe representatives';
		job.sourceStatus = signature.status ?? null;
		job.sourceCount = signature.count ?? 0;
		restored += 1;
	}
	if ( restored > 0 ) {
		state.lastNoAnalysisProductEvidenceFamilyCapRestored = {
			at: new Date().toISOString(),
			restored,
			reason: 'restored product-evidence family-capped jobs so unsafe representatives cannot hide siblings',
		};
	}
	return restored;
}
...
function getFamilyLaunchCountKeys(
	...
	if ( sourceFamilyKey ) {
		if ( currentOutputPeer ) {
			...
			! isActiveOnlyAnalysisLaunchFamily( signature, sourceFamilyKey ) ||
			shouldApplyCurrentOutputFamilyCap( signature, sourceFamilyKey, job )
		) {
			keys.add( sourceFamilyKey );
		}
	}
...
async function launchCodexAnalysisJob( sourceState, state, signature ) {
	...
	child.on( 'exit', async ( code, signal ) => {
		...
		if ( code === 0 && fsSync.existsSync( resultPath ) ) {
			...
			try {
				latestJob.productFailureLedgerPath =
					await persistLikelyRealProductFailure(
						latestJob,
						signature
					);
				latestJob.productFailureLedgerError = null;
			} catch ( error ) {
				latestJob.productFailureLedgerError = String(
					error?.stack ?? error
				);
			}
...
function compactSummaryRecord( record, lineIndex ) {
	const coverageRecords = getCompactCoverageRecords(
	...
	return sanitizeForPrompt( {
		...
		coverageRecords,
		attempts: compactAttempts( record.attempts ?? [] ),
		coverage: {
...
function compactAttempts( attempts ) {
	return attempts.slice( 0, 6 ).map( ( attempt ) => ( {
		...
		coverageRecords: getCompactCoverageRecords(
			attempt.behavioralCoverage ?? []
		),
	} ) );
}

function getCompactCoverageRecords( records ) {
	return records
		.filter( ( record ) => ! record?.parseError )
		.slice( 0, 6 )
		.map( ( record, index ) => {
			const historyEvents = record.historyEvents ?? [];
			const invariantEvents = record.invariantEvents ?? [];
			const operationEvents = record.operationEvents ?? [];
			const lastHistoryEvent = historyEvents.at( -1 );
			return {
				index,
				status: record.status,
				errorExcerpt: excerptFailureOutput( record.error ),
				actionProfile: record.actionProfile,
				transport: record.transport,
				initialContentProfile: record.initialContentProfile,
				...
				revisionRestore: record.revisionRestore,
				saveCheckpointSteps: record.saveCheckpointSteps,
				...
				failedInvariants: invariantEvents
					.filter(
						( event ) => event.status && event.status !== 'ok'
					)
					.slice( -8 ),
				notableOperationEvents: operationEvents
					.filter( ( event ) =>
...
function sanitizeForPrompt( value, depth = 0 ) {
	...
	if ( Array.isArray( value ) ) {
		return value
			.slice( 0, 24 )
			.map( ( entry ) => sanitizeForPrompt( entry, depth + 1 ) );
	}
...
function buildCodexPrompt( sourceState, signature, paths ) {
	...
	return [
		...
		'Priority classification rule: base-passes/fixed-stack-fails multi-user convergence, saved-content union, or list/block ordering after refresh is likely product/candidate correctness unless evidence proves harness-only.',
		...
		'Do not dump whole summary/event streams or large Playwright error-context files. Never run `cat`, `sed`, `head`, or `tail` directly on `summary.ndjson`, `events.ndjson`, `events.jsonl`, or large files under the fuzz artifacts tree; use the compact context file or compact extractor instead. Do not bypass the guard wrappers with absolute paths such as `/bin/cat`, `/usr/bin/sed`, or `/usr/bin/rg`.',
		'Do not run `rg`, `grep`, `sed`, `cat`, `head`, or `tail` directly on `command.log`. If command-log evidence is still needed after reading compact context, use `node bin/rtc-browser-fuzz-command-log-probe.mjs <command.log> --pattern <regex> --max-matches 40` so only a bounded current-run tail is inspected.',
		...
		'The compact context includes per-attempt coverageRecords. If an artifact path names a Playwright retry, for example chromium-retry1, compare against the matching attempt/retry coverage and command-log section before assigning a bucket.',
...
async function runScanCycle() {
	...
	if ( noAnalysis && ! isNoAnalysisLiveAdmissionActive( noAnalysis ) ) {
		state.lastLaunchSkippedForNoAnalysis = {
			at: new Date().toISOString(),
			reason: 'active producer no-analysis sentinel has no live-analysis admission; suppressed stale/noise jobs and exiting instead of keeping a warm analysis loop',
			reasonKind: noAnalysis.reasonKind ?? null,
			family: noAnalysis.family ?? null,
			group: noAnalysis.group ?? null,
			hasProductEvidence:
		...
		process.stdout.write(
			...
			}; exiting without launching analysis\n`
		);
		return;
	}
	if ( gateOnlyRefresh.status === 'failed' ) {
		state.lastLaunchSkippedForGateOnlyFailure = {
			at: new Date().toISOString(),
			reason: 'gate-only triage refresh failed; reconciled and suppressed stale/noise jobs but skipped new analysis launches for this pass',
			gateOnlyRefresh,
		};
	} else {
