...
const ENABLE_HUMAN_SMOKE_PREFLIGHT =
	( process.env.RTC_FUZZ_HUMAN_SMOKE_PREFLIGHT ?? '1' ) !== '0';
...
function getPlaywrightArtifactArgs() {
	...
	if ( PLAYWRIGHT_TRACE ) {
		args.push( '--trace', PLAYWRIGHT_TRACE );
	}
...
async function ensurePackageCjsEntrypoints() {
	for ( const entrypoint of REQUIRED_PACKAGE_CJS_ENTRYPOINTS ) {
		const sourcePath = path.join( REPO_ROOT, entrypoint.source );
		const targetPath = path.join( REPO_ROOT, entrypoint.target );
		...
		if ( ! ( await fileExists( sourcePath ) ) ) {
			throw new Error(
				`Gutenberg build artifacts are missing for ${ entrypoint.packageName }: ${ entrypoint.source }`
			);
		}
		...
		await log(
			`generated missing CommonJS entrypoint ${ entrypoint.target } for ${ entrypoint.packageName }.`
		);
	}
}
...
async function runFullPreflight( label ) {
	...
	if ( ENABLE_HUMAN_SMOKE_PREFLIGHT ) {
		await ensureFileExists( path.join( REPO_ROOT, HUMAN_SMOKE_SPEC_PATH ) );
	}
	...
	const listResult = await runCombinedCommand( {
		command: 'npm',
		args: [
			'run',
			'test:e2e',
	...
	if ( ! listResult.ok || ! ENABLE_HUMAN_SMOKE_PREFLIGHT ) {
		return {
			...listResult,
			preflightKind: 'test-list',
		};
	}

	const smokeArtifactsDir = path.join(
		OUTPUT_DIR,
		`${ label }-human-smoke-artifacts`
	);
	const smokeLogPath = path.join( OUTPUT_DIR, `${ label }-human-smoke.log` );
	const smokeResult = await runCombinedCommand( {
		command: 'npm',
		args: [
			'run',
			'test:e2e',
			'--',
			HUMAN_SMOKE_SPEC_PATH,
			'--project=chromium',
			...getPlaywrightArtifactArgs(),
		],
		env: getSharedEnv(
			getBrowserFuzzEnv( {
				WP_ARTIFACTS_PATH: smokeArtifactsDir,
				...
				GUTENBERG_RTC_LANE_LABEL: `${ LANE_LABEL }-human-smoke`,
			} )
		),
	...
	return {
		...smokeResult,
		artifactsDir: smokeArtifactsDir,
		preflightKind: 'human-product-smoke',
	};
}
...
async function stopForPreflightFailure( { seed = null, stage, result } ) {
	...
	if (
		...
		await stopForInfraFailure( { seed, stage, result } );
		return 'infra';
	}

	const behavioralCoverage = [
		{
			status: 'failed',
			...
			transport:
				process.env.GUTENBERG_RTC_TEST_WS_PROVIDER === '1'
					? 'ws'
					: 'http',
			actionProfile: ACTION_PROFILE || 'human-product-smoke',
			collaboratorMode: COLLABORATOR_MODE || 'human-product-smoke',
			userCount: 1,
			actions: [ { label: 'human-product-smoke' } ],
			invariantEvents: [
				{
					name: 'human-product-smoke-oracle',
					phase: 'rendered-editor',
					status: 'fail',
					error: failureSnippet,
				},
			],
			error: failureSnippet,
			evidencePath: result.logPath,
		},
	];
	const record = {
		kind: 'real-bug',
		...
		failureSnippet,
		logPath: result.logPath,
		artifactsDir: result.artifactsDir ?? null,
		behavioralCoverage,
	...
	await updateState( {
		...
		stopReason:
			seed === null
				...
				: `seed-${ seed }-${ stage }-product-failure`,
	} );
	await log(
		`Stopping after actionable human product smoke failure${
			seed === null ? '' : ` for seed ${ seed }`
		}. See ${ result.logPath }.`
	);
	return 'product';
...
function hasOracleCoverageEvidence( coverage ) {
	if ( ! coverage || typeof coverage !== 'object' ) {
		return false;
	}
	if ( ( coverage.userCount ?? 0 ) > 0 ) {
		return true;
	}
	...
	return [
		...( coverage.historyEvents ?? [] ).map( ( event ) => event.phase ),
		...( coverage.invariantEvents ?? [] ).flatMap( ( event ) => [
			event.phase,
			event.name,
		] ),
	]
...
function getPreActionBootstrapStall( attempt, failureSnippet ) {
	...
	if ( hasOracleCoverageEvidence( coverage ) ) {
		return null;
	}
...
function pickReplayEnv( env ) {
	const names = [
		...
		'GUTENBERG_RTC_BROWSER_ENABLE_REVISION_RESTORE_PROBE',
		'GUTENBERG_RTC_BROWSER_SKIP_GLOBAL_POST_CLEANUP',
		'GUTENBERG_RTC_BROWSER_BASIC_UI_ORACLE',
		'GUTENBERG_RTC_BROWSER_POST_NEW_UI_ORACLE',
		'GUTENBERG_RTC_BROWSER_REQUIRE_SAVE_CLEAN',
		...
		'RTC_FUZZ_HUMAN_SMOKE_PREFLIGHT',
	];
	...
	const manifest = {
		...
		behavioralCoverage: firstCoverage
			? {
					...
					invariantEvents: firstCoverage.invariantEvents ?? [],
...
async function main() {
	...
	await ensurePackageCjsEntrypoints();
	...
	if ( statePath ) {
		try {
			await updateState( {
				currentSeed: null,
				stopReason:
					state.stopReason ?? `runner-error: ${ error.message }`,
			} );
		} catch {}
	}
