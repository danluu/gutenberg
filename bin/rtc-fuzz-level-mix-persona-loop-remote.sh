...
CODEX_BIN="${CODEX_BIN:-/home/exouser/.npm-global/bin/codex}"
MODEL="${RTC_FUZZ_LEVEL_MIX_MODEL:-gpt-5.6-sol}"
...
add_cg_lower_level_roots() {
  local roots_file="$1"
  local cg_base cg_index known_bases
  cg_index=0

  add_cg_lower_level_base() {
    local cg_base="$1"
    local cg_root cg_smoke_root cg_smoke_mtime cg_now
    [ -d "$cg_base" ] || return 0
    cg_root="$(current_root "$cg_base")"
    add_root "$roots_file" "coverage-guided-lower-current-$cg_index" "$cg_root"
    cg_smoke_root="$(ls -td "$cg_base"/runs/coverage-guided-lower-level-smoke-* 2>/dev/null | head -1 || true)"
    if [ -n "$cg_smoke_root" ] && [ "$cg_smoke_root" != "$cg_root" ]; then
      cg_smoke_mtime="$(stat -c %Y "$cg_smoke_root/events.ndjson" 2>/dev/null || stat -c %Y "$cg_smoke_root/status.tsv" 2>/dev/null || printf '0\n')"
      cg_now="$(date +%s)"
      if [[ "$cg_smoke_mtime" =~ ^[0-9]+$ ]] && [ "$(( cg_now - cg_smoke_mtime ))" -le 21600 ]; then
        add_root "$roots_file" "coverage-guided-lower-bounded-$cg_index" "$cg_smoke_root"
      fi
    fi
    cg_index=$(( cg_index + 1 ))
  }
  ...
  known_bases="
$CG_LOWER_LEVEL_BASE
/media/volume/danluu-fuzz-data/rtc-coverage-guided-lower-level-block-parser-serialization-20260519
...
/media/volume/danluu-fuzz-data/rtc-coverage-guided-lower-level-http-awareness-selection-propagation-20260528
/media/volume/danluu-fuzz-data/rtc-coverage-guided-lower-level-client-overlay-projection-20260528
/media/volume/danluu-fuzz-data/rtc-coverage-guided-lower-level-rich-text-crdt-20260518
/media/volume/danluu-fuzz-data/rtc-coverage-guided-lower-level-rich-text-multiblock-20260518
"

  while IFS= read -r cg_base || [ -n "$cg_base" ]; do
    [ -n "$cg_base" ] || continue
    add_cg_lower_level_base "$cg_base"
  done <<EOF_CG_KNOWN_BASES
$known_bases
EOF_CG_KNOWN_BASES

  for cg_base in /media/volume/danluu-fuzz-data/rtc-coverage-guided-lower-level-*-202605*; do
    [ -d "$cg_base" ] || continue
    if printf '%s\n' "$known_bases" | grep -Fxq "$cg_base"; then
      continue
    fi
    if [ -f "$cg_base/current-run-root.txt" ] || ls "$cg_base"/runs/coverage-guided-lower-level-smoke-* >/dev/null 2>&1; then
      add_cg_lower_level_base "$cg_base"
    fi
  done
}
...
lower_level_current_root() {
  local marker="$LOWER_LEVEL_BASE/current-run-root.txt"
  if [ -f "$marker" ]; then
    sed -n '1p' "$marker"
  fi
}
...
root_has_recent_audited_output() {
  local root="${1:-}"
  local max_age="${2:-21600}"
  local now mtime candidate
  [ -n "$root" ] && [ -d "$root" ] || return 1
  now="$(date +%s)"
  for candidate in "$root/status.tsv" "$root/events.ndjson"; do
    [ -s "$candidate" ] || continue
    mtime="$(stat -c %Y "$candidate" 2>/dev/null || printf '0\n')"
    if [[ "$mtime" =~ ^[0-9]+$ ]] && [ "$(( now - mtime ))" -le "$max_age" ]; then
      return 0
    fi
  done
  return 1
...
add_root_if_recent_bounded_output() {
  local roots_file="$1"
  local label="$2"
  local root="${3:-}"
  local max_age="${4:-21600}"
  if root_has_recent_audited_output "$root" "$max_age"; then
    add_root "$roots_file" "$label" "$root"
  fi
}
...
write_root_inventory() {
  local roots_file="$1"
  local coverage_root focused_root strict_root gap_root lower_root lower_bounded_root
  : > "$roots_file"
  ...
  lower_root="$(lower_level_current_root)"
  lower_bounded_root="$(sed -n '1p' "$LOWER_LEVEL_BASE/last-bounded-run-root.txt" 2>/dev/null || true)"
  ...
  if [ -n "$lower_bounded_root" ] && [ "$lower_bounded_root" != "$lower_root" ]; then
    add_root "$roots_file" "unit-property-bounded-latest" "$lower_bounded_root"
  fi
  add_cg_lower_level_roots "$roots_file"
  ...
  add_root_if_recent_bounded_output "$roots_file" "protocol-bounded-current" "$(protocol_current_root)"
  add_root_if_recent_bounded_output "$roots_file" "backend-api-bounded-current" "$(backend_api_current_root)"
...
def level_has_live_tmux(level):
    if level == "unit-property":
        return any(
            session == "rtc-lower-level-fuzz-loop"
            or session.startswith("rtc-lower-level-fuzz-")
            or session.startswith("rtc-unit-property")
            for session in tmux_session_set
        )
    if level == "coverage-guided-lower-level":
        return any(
            session.startswith("rtc-coverage-guided-lower-level")
            or session.startswith("rtc-cg-parser-action-")
            or session.startswith("rtc-native-action-")
            for session in tmux_session_set
        )
    if level == "backend-api":
        return "rtc-backend-api-fuzz" in tmux_session_set
    if level == "protocol-server":
        return "rtc-protocol-server-fuzz" in tmux_session_set
    return False

def group_has_live_tmux(level, name):
    group_name = str(name or "")
    if level == "coverage-guided-lower-level" and group_name:
        return group_name in tmux_session_set or f"rtc-{group_name}" in tmux_session_set
    return level_has_live_tmux(level)
...
def root_has_fresh_audited_output(root, max_age=7200):
    return any(
        fresh_file(os.path.join(root, candidate), max_age=max_age)
        for candidate in ("status.tsv", "events.ndjson")
    )
...
counts = Counter()
bounded_evidence_counts = Counter()
active_rows = []
held_browser_rows = []
browser_summaries = []
invariant_failures = []
for label, root in roots:
    ...
    for group in parsed if isinstance(parsed, list) else []:
        ...
        if level == "fuzz-assertion":
            ...
            continue
        if level == "coverage-guided-lower-level":
            if group_has_live_tmux(level, name) and root_has_fresh_audited_output(root):
                counts[level] += lanes
                active_rows.append((level, name, lanes, transport, profile, "live-tmux", "", label, root))
            elif root_has_fresh_audited_output(root):
                bounded_evidence_counts[level] += lanes
                active_rows.append((level, name, 0, transport, profile, "fresh-bounded-evidence", "", label, root))
            else:
                active_rows.append((level, name, 0, transport, profile, "stale-completed-output", "", label, root))
            continue
        if level in {"unit-property", "backend-api", "protocol-server"}:
            if group_has_live_tmux(level, name) and root_has_fresh_audited_output(root):
                counts[level] += lanes
                active_rows.append((level, name, lanes, transport, profile, "live-tmux", "", label, root))
            elif root_has_fresh_audited_output(root):
                ...
                active_rows.append((level, name, 0, transport, profile, "stale-or-configured-no-live-lane", "", label, root))
            continue
        counts[level] += lanes
...
levels_to_print = sorted(set(counts) | set(bounded_evidence_counts))
for level in levels_to_print:
    bounded = bounded_evidence_counts[level]
    if bounded:
        print(f"- {level}: {counts[level]} live lane(s); {bounded} fresh bounded evidence lane(s)")
    else:
        print(f"- {level}: {counts[level]} live lane(s)")
required_levels = ("unit-property", "coverage-guided-lower-level", "backend-api", "protocol-server", "fuzz-assertion")
missing_levels = [
    level
    for level in required_levels
    if counts[level] == 0 and bounded_evidence_counts[level] == 0
]
if missing_levels:
    print(f"- gap: 0 live or fresh bounded evidence lanes for {', '.join(missing_levels)}")
...
def load_group_metadata(root):
    ...
    for group in groups if isinstance(groups, list) else []:
        ...
        metadata[name] = {
            ...
            "maxCorpusFiles": group.get("maxCorpusFiles"),
            "maxAttempts": group.get("maxAttempts"),
            "disableMutation": group.get("disableMutation"),
        }
    return metadata
            ...
            with open(path, errors="ignore") as handle:
                for line in handle:
                    ...
                    if event.get("kind") == "run-start":
                        meta = {
                            **meta,
                            ...
                            "maxCorpusFiles": event.get("maxCorpusFiles", meta.get("maxCorpusFiles")),
                            "maxAttempts": event.get("maxAttempts", meta.get("maxAttempts")),
                            "disableMutation": event.get("disableMutation", meta.get("disableMutation")),
                        }
                        continue
                    ...
                    records[(level, event_group)].append({
                        ...
                        "maxAttempts": event.get("maxAttempts", meta.get("maxAttempts")),
                        "disableMutation": event.get("disableMutation", meta.get("disableMutation")),
                        "knownBatchFailureKeyBeforeAttempt": event.get("knownBatchFailureKeyBeforeAttempt"),
                        "failureArtifactSuppressed": event.get("failureArtifactSuppressed"),
                        "failureArtifactSuppressionReason": event.get("failureArtifactSuppressionReason"),
                        "newCoverageKeys": event.get("newCoverageKeys"),
                        "newFeatureKeys": event.get("newFeatureKeys"),
                        "newProductFailureKey": event.get("newProductFailureKey"),
                        "productYield": event.get("productYield"),
                        "productYieldReason": event.get("productYieldReason"),
                        "actionProfile": event.get("actionProfile") or event.get("profile") or "",
                        "fixedUnitCaseCount": event.get("fixedUnitCaseCount"),
                        "generatedUnitCaseCount": event.get("generatedUnitCaseCount"),
                        "usefulExecutionCount": event.get("usefulExecutionCount"),
                        "label": label,
                        "root": root,
                    })
        except OSError:
...
for (level, group), rows in sorted(records.items()):
    ...
    bounded_failure_isolation = (
        level == "coverage-guided-lower-level"
        and isolation_attempts > 0
        and all(
            0 < int(row.get("maxAttempts") or 0) <= 1
            for row in rows
            if row.get("failureIsolationAttempted")
        )
    )
    exact_seed_confirmation = (
        level == "coverage-guided-lower-level"
        and len(rows) <= 2
        and all(str(row.get("disableMutation")).lower() == "true" for row in rows)
        and all(int(row.get("executions") or 0) <= 1 for row in rows)
        and all(int(row.get("batchSize") or 0) <= 1 for row in rows)
        and all(int(row.get("maxCorpusFiles") or 0) <= 1 for row in rows)
    )
    bounded_static_unit_replay = (
        level == "unit-property"
        and len(rows) <= 2
        and all(int(row.get("fixedUnitCaseCount") or 0) > 0 for row in rows)
        and all(int(row.get("generatedUnitCaseCount") or 0) == 0 for row in rows)
        and total_exec <= 16
    )
    bounded_smoke_confirmation = (
        level == "coverage-guided-lower-level"
        and len(rows) <= 2
        and total_exec <= 16
        and any("coverage-guided-lower-level-smoke-" in str(row.get("root") or "") for row in rows)
    )
    overhead_note = ""
    if exact_seed_confirmation:
        overhead_note = "bounded exact-seed confirmation; throughput not steady-state"
    elif bounded_failure_isolation:
        overhead_note = f"bounded failure-isolation confirmation; throughput not steady-state; attempts={isolation_attempts} checked={isolation_checks}"
    elif bounded_static_unit_replay:
        overhead_note = "bounded static unit-property canary replay; throughput not steady-state"
    elif bounded_smoke_confirmation:
        overhead_note = "bounded lower-level smoke confirmation; throughput not steady-state"
    held_duplicate_confirmation = (
        level == "coverage-guided-lower-level"
        and "table-query-array" in group
        and "direct-isolation" in str(latest.get("actionProfile") or latest.get("strategy") or "")
        and as_int(latest.get("maxAttempts")) <= 1
        and truthy(latest.get("disableMutation"))
        and truthy(latest.get("knownBatchFailureKeyBeforeAttempt"))
        and not truthy(latest.get("newProductFailureKey"))
        and not truthy(latest.get("productYield"))
        and as_int(latest.get("newCoverageKeys")) == 0
    )
    if held_duplicate_confirmation:
        overhead_note = "held duplicate canonical failure confirmation; throughput not steady-state"
    if avg_duration and avg_overhead / avg_duration > 0.5:
        if exact_seed_confirmation or bounded_failure_isolation or bounded_static_unit_replay or bounded_smoke_confirmation:
            pass
        elif isolation_attempts:
    ...
    if exact_seed_confirmation or bounded_failure_isolation or bounded_static_unit_replay or bounded_smoke_confirmation:
        status = "ok"
    elif held_duplicate_confirmation:
        status = "blocked: held duplicate confirmation; next code change is a persistent/direct table-query oracle or a non-duplicate target"
    elif sleep not in (None, "", 0, "0"):
...
for level in all_levels:
    ...
    latest_age = now - latest_mtime.get(level, 0) if latest_mtime.get(level, 0) else None
    status = "ok"
    if level == "fuzz-assertion" and tmux_levels[level] and (latest_age is None or latest_age > 1800):
        status = "stale-audited-output session-live; count as zero/held"
    elif level in {"backend-api", "protocol-server", "fuzz-assertion"} and not tmux_levels[level] and not event_levels[level]:
        ...
        elif level == "coverage-guided-lower-level" and latest_age is not None and latest_age <= 1800:
            status = "ok: fresh bounded lower-level output; no persistent tmux session expected after max-attempts stop"
        elif level == "unit-property" and latest_age is not None and latest_age <= 1800:
            bounded_complete = False
            try:
                with open(os.path.join(root, "status.tsv"), encoding="utf-8", errors="replace") as status_file:
                    bounded_complete = any("\tcompleted\t" in line and "max_generated_executions=" in line for line in status_file)
            except OSError:
                bounded_complete = False
            if bounded_complete:
                status = "ok: fresh bounded unit/property output; no persistent tmux session expected after max-generated stop"
            else:
                status = "check-stale-events: events exist but no matching tmux session"
        elif level in {"backend-api", "protocol-server"} and latest_age is not None and latest_age <= 21600:
            status = f"ok: fresh bounded {level} sentinel output; no persistent tmux session expected after seed budget stop"
        else:
            status = "check-stale-events: events exist but no matching tmux session"
...
optional_browser_pressure_gated = autoscaler_reason in {"pressure", "high_pressure", "severe_pressure", "disk_pressure", "disk_high_pressure", "disk_severe_pressure"}
...
pressure_gated_exact_sessions = {
    "rtc-backend-api-fuzz",
    "rtc-protocol-server-fuzz",
}
blocked_exact_session_start_commands = {
    "rtc-backend-api-fuzz": "RTC_BACKEND_API_FUZZ_LANES=1 RTC_BACKEND_API_FUZZ_SEED_COUNT=64 RTC_BACKEND_API_FUZZ_CASE_COUNT=4 RTC_BACKEND_API_FUZZ_UPDATE_CURRENT_RUN_ROOT=1 bin/rtc-backend-api-fuzz-start-remote.sh",
    "rtc-protocol-server-fuzz": "RTC_PROTOCOL_SERVER_FUZZ_LANES=1 RTC_PROTOCOL_SERVER_FUZZ_SEED_COUNT=64 RTC_PROTOCOL_SERVER_FUZZ_CASE_COUNT=8 RTC_PROTOCOL_SERVER_FUZZ_UPDATE_CURRENT_RUN_ROOT=1 bin/rtc-protocol-server-fuzz-start-remote.sh",
}
blocked_exact_session_current_root_markers = {
    "rtc-backend-api-fuzz": "/media/volume/danluu-fuzz-data/rtc-backend-api-fuzz-20260518/current-run-root.txt",
    "rtc-protocol-server-fuzz": "/media/volume/danluu-fuzz-data/rtc-native-assert-protocol-20260516/protocol/current-run-root.txt",
}
...
def latest_status_age_seconds(root):
    if not root or not os.path.isdir(root):
        return None
    latest = None
    for relative in ("status.tsv", "events.ndjson"):
...
def optional_browser_zero_capacity_detail(name):
    roots = optional_browser_roots_for_session(name)
    if not roots:
        return "no current optional browser root recorded"
    details = []
    for root in roots[:5]:
        state_path = os.path.join(root, "supervisor-state.json")
        try:
            ...
            details.append(f"{os.path.basename(root)}: no readable supervisor-state.json ({exc})")
            continue
        groups = state.get("groups") if isinstance(state, dict) else state
        if not isinstance(groups, list):
            details.append(f"{os.path.basename(root)}: no supervisor groups")
            continue
        updated = parse_ts((state or {}).get("lastUpdatedAt") or (state or {}).get("updatedAt") or (state or {}).get("startedAt")) if isinstance(state, dict) else None
        if updated is None:
            try:
                ...
                updated = None
        age = int(max(0, time.time() - updated)) if updated is not None else "unknown"
        counts = Counter(str(group.get("status") or "unknown") for group in groups if isinstance(group, dict))
        active_dirs = 0
        live_dirs = 0
        for group in groups:
            if not isinstance(group, dict):
            ...
            for active_dir in group.get("activeRunDirs") or []:
                ...
                if active_dir_has_live_lane(active_dir) and active_dir_has_fresh_artifact(active_dir):
                    live_dirs += 1
        details.append(f"{os.path.basename(root)}: age={age}s statuses={dict(counts)} active_dirs={active_dirs} live_fresh_dirs={live_dirs}")
    return "; ".join(details)
...
if any(name in critical_exact_sessions and name not in session_set for name in expected):
    time.sleep(2)
    refreshed_sessions = read_tmux_sessions()
...
for name in expected:
    ...
    if name in pressure_gated_exact_sessions and optional_browser_pressure_gated:
        ...
        continue
    if name in blocked_exact_session_start_commands and not prefix_matches:
        marker = blocked_exact_session_current_root_markers.get(name, "")
        current_root = read_first_line(marker) if marker else ""
        latest_age = latest_status_age_seconds(current_root)
        if latest_age is not None and latest_age <= 21600:
            print(
                f"| exact-session:{name} | ok | exact tmux session absent after bounded sentinel completed; fresh audited current_root={current_root} latest_status_age_seconds={latest_age}; relaunch only when the bounded sentinel ages out or PR-evidence policy requires a live lane |"
            )
            continue
        print(
            f"| exact-session:{name} | blocked-or-zero | exact tmux session missing; no active audited lane is configured for deadline PR-evidence mode. current_root={current_root or 'missing'} latest_status_age_seconds={latest_age}; next bounded start command: {blocked_exact_session_start_commands[name]} |"
        )
        continue
    if name in optional_browser_sessions:
        ...
        if not prefix_matches:
            detail = optional_browser_zero_capacity_detail(name)
            pressure_detail = f" autoscaler reason={autoscaler_reason};" if optional_browser_pressure_gated else ""
            print(
                f"| exact-session:{name} | check | exact optional browser session absent;{pressure_detail} no PID-backed fresh active run dirs, so this contributes zero useful capacity until restarted. detail={detail}. next remediation: after pressure clears, run the matching focused/strict/gap start script instead of relying on a watchdog or prefix session |"
            )
            continue
    if prefix_matches:
        print(
...
current_run_dir_reconciliation = None
if isinstance(novelty_state, dict):
    reconciliation = novelty_state.get("currentRunDirReconciliation")
    if isinstance(reconciliation, dict):
        current_run_dir_reconciliation = reconciliation
...
if active_run_dirs > 0 and novelty_current_run_dirs == 0:
    ...
    if supervisor_dir_set and novelty_dir_set and supervisor_dir_set != novelty_dir_set:
        policy_matches_current = bool(
            current_run_dir_reconciliation
            and current_run_dir_reconciliation.get("policyMatchesCurrentRunDirs")
            and int(current_run_dir_reconciliation.get("currentRunDirs") or -1) == novelty_current_run_dirs
        )
        supervisor_extra = (
            current_run_dir_reconciliation.get("supervisorExtraOverPolicyPaths") or []
            if isinstance(current_run_dir_reconciliation, dict)
            else []
        )
        policy_missing = (
            current_run_dir_reconciliation.get("policyMissingFromSupervisorPaths") or []
            if isinstance(current_run_dir_reconciliation, dict)
            else []
        )
        if policy_matches_current:
            print(f"| coverage-novelty-supervisor-active-dirs | ok | supervisor published activeRunDirs={len(supervisor_dir_set)} and novelty policy currentRunDirs={novelty_current_run_dirs} ({len(novelty_dir_set)} existing); filtered_by_policy={len(supervisor_extra)} policy_missing={len(policy_missing)} sample_filtered={'; '.join(map(os.path.basename, supervisor_extra[:5])) or 'none'} |")
        else:
...
events = deque(sorted(events, key=lambda item: item[0])[-25:], maxlen=25)
...
def is_canonical_rtc_failure_key(value):
    key = normalize_semantic_key(value)
    return key.startswith("RTC_") and "ZERO_COVERAGE_CANARY" not in key

def truthy(value):
    return str(value).lower() in {"1", "true", "yes"}

def reproducer_has_retained_input(dirpath, reproducer):
    index = reproducer.get("index")
    seed_paths = []
    if isinstance(index, int):
        seed_paths.extend([
            os.path.join(dirpath, f"reproducer-input-{index:04d}.bin"),
            os.path.join(dirpath, f"input-{index:04d}.bin"),
        ])
    seed_path = reproducer.get("inputPath")
    if isinstance(seed_path, str):
        seed_paths.append(seed_path)
    return bool(seed_paths) and any(os.path.exists(path) for path in seed_paths)

def creditable_lower_level_reproducer(dirpath, reproducer):
    if not isinstance(reproducer, dict):
        return ""
    if not (
        truthy(reproducer.get("newProductFailureKey"))
        or truthy(reproducer.get("productYield"))
        or reproducer.get("productFailureKey")
    ):
        return ""
    if str(reproducer.get("exitCode")) not in ("1",):
        return ""
    if str(reproducer.get("failureKind") or "") != "oracle-failure":
        return ""
    semantic = (
        reproducer.get("productFailureKey")
        or reproducer.get("isolatedFailureCanonicalKey")
        or reproducer.get("failureCanonicalKey")
        or reproducer.get("canonicalFailureKey")
        or reproducer.get("semanticFailureKey")
        or reproducer.get("failureSummary")
    )
    semantic = normalize_semantic_key(semantic)
    if not is_canonical_rtc_failure_key(semantic):
        return ""
    if not reproducer_has_retained_input(dirpath, reproducer):
        return ""
    return semantic

def creditable_lower_level_event_failure(event):
    semantic = normalize_semantic_key(
        event.get("failureCanonicalKey")
        or event.get("canonicalFailureKey")
        or event.get("semanticFailureKey")
        or event.get("failureSummary")
    )
    isolated = normalize_semantic_key(event.get("isolatedFailureCanonicalKey"))
    if not is_canonical_rtc_failure_key(semantic):
        return ""
    if not (
        event.get("newProductFailureKey")
        or (
            event.get("productYield")
            and isolated
            and isolated == semantic
        )
    ):
        return ""
    return semantic
    ...
    for dirpath, _, files in os.walk(root):
        if "metadata.json" in files:
            ...
            if metadata_path not in seen_metadata_paths:
                ...
                try:
                    metadata = json.load(open(metadata_path, errors="ignore"))
                except Exception:
                    metadata = {}
                reproducers = ((metadata.get("minimization") or {}).get("reproducers") or [])
                if isinstance(reproducers, list):
                    for reproducer in reproducers:
                        if level == "coverage-guided-lower-level":
                            semantic = creditable_lower_level_reproducer(dirpath, reproducer)
                        else:
                            if not isinstance(reproducer, dict):
                                continue
                            semantic = (
                                reproducer.get("failureCanonicalKey")
                                or reproducer.get("canonicalFailureKey")
                                or reproducer.get("semanticFailureKey")
                                or reproducer.get("failureSummary")
                            )
                        ...
                        if not semantic:
                            continue
                        if level != "coverage-guided-lower-level" and not reproducer_has_retained_input(dirpath, reproducer):
                            continue
                ...
                for line in events:
                    ...
                    if is_failure:
                        ...
                        if semantic:
                            stats[level]["semantic_keys"][semantic] += 1
                            credited = (
                                creditable_lower_level_event_failure(event)
                                if level == "coverage-guided-lower-level"
                                else semantic
                            )
                            if credited:
                                stats[level]["bug_keys"][credited] += 1
                                stats[level]["candidate_rows"] += 1
        except OSError:
            pass
...
def creditable_lower_level_event_failure(event):
    ...
    isolated = normalize(event.get("isolatedFailureCanonicalKey"))
    if not is_canonical_rtc_failure_key(semantic):
        return ""
    if not (
        event.get("newProductFailureKey")
...
def root_has_fresh_audited_output(root, max_age=7200):
    for candidate in ("status.tsv", "events.ndjson"):
        try:
            if time.time() - os.path.getmtime(os.path.join(root, candidate)) <= max_age:
                return True
        except OSError:
            continue
    return False

def load_resource_autoscaler_status():
    status = {}
    path = "/media/volume/danluu-fuzz-data/rtc-resource-autoscaler-20260516/resource-autoscaler-status.md"
    try:
...
def parse_budget_value(raw, key):
    match = re.search(rf"\b{re.escape(key)}=([0-9]+)", str(raw or ""))
    if not match:
        return None
    try:
...
fresh_bounded_evidence = Counter()
paused_browser = 0
for label, root in roots:
    ...
    for group in groups:
        ...
        if level == "browser-e2e":
            ...
            if status == "running" and live_pids:
                ...
                paused_browser += lanes
        elif level in {"unit-property", "coverage-guided-lower-level", "backend-api", "protocol-server", "fuzz-assertion"}:
            if group_has_live_tmux(level, group.get("name")) and root_has_fresh_audited_output(root):
                active_lanes[level] += lanes
            elif root_has_fresh_audited_output(root):
                fresh_bounded_evidence[level] += lanes
        else:
            active_lanes[level] += lanes
...
for label, root in roots:
    ...
    seen_metadata_paths = set()
    for dirpath, _, files in os.walk(root):
        if level in bug_outputs and "metadata.json" in files:
            metadata_path = os.path.join(dirpath, "metadata.json")
            if metadata_path not in seen_metadata_paths:
                seen_metadata_paths.add(metadata_path)
                try:
                ...
                if isinstance(reproducers, list):
                    for reproducer in reproducers:
                        if level == "coverage-guided-lower-level":
                            ...
                            semantic = normalize(semantic)
                        if not semantic or is_infra_or_harness_output(semantic):
                            continue
                        if level != "coverage-guided-lower-level" and not reproducer_has_retained_input(dirpath, reproducer):
                            continue
                        bug_outputs[level][semantic] += 1
        if "events.ndjson" not in files:
            continue
        try:
            for line in open(os.path.join(dirpath, "events.ndjson"), errors="ignore"):
                ...
                if is_failure and level in bug_outputs:
                    ...
                    if level == "coverage-guided-lower-level":
                        semantic = creditable_lower_level_event_failure(event)
                    else:
                        semantic = normalize(semantic)
                    if semantic and not is_infra_or_harness_output(semantic):
                        bug_outputs[level][semantic] += 1
        except OSError:
            pass
...
resource_status = load_resource_autoscaler_status()
resource_reason = resource_status.get("reason", "")
budget_target = parse_budget_value(resource_status.get("current_budget"), "target")
budget_max = parse_budget_value(resource_status.get("current_budget"), "max")
disk_pressure_reasons = {
    "disk_pressure",
    "disk_high_pressure",
    "disk_severe_pressure",
    "pressure",
    "high_pressure",
    "severe_pressure",
}
deadline_cap_reasons = {
    "deadline_benchmark_canary_cap",
}
e2e_disk_pressure_blocked = resource_reason in disk_pressure_reasons
e2e_deadline_downscoped = (
    budget_target is not None
    and budget_target > 0
    and budget_target < e2e_floor
    and budget_max is not None
    and (
        resource_reason in deadline_cap_reasons
        or budget_max <= max(budget_target, 2)
    )
)
effective_e2e_floor = budget_target if e2e_deadline_downscoped else e2e_floor
...
if e2e_deadline_downscoped:
    print(
        f"- e2e effective active lane floor: {effective_e2e_floor} "
        f"(resource autoscaler current_budget={resource_status.get('current_budget')} reason={resource_reason})"
    )
...
if fresh_bounded_evidence:
    evidence_display = ", ".join(
        f"{level}={fresh_bounded_evidence[level]}"
        for level in sorted(fresh_bounded_evidence)
        if fresh_bounded_evidence[level]
    )
    print(f"- fresh bounded evidence lanes (not live active lanes): {evidence_display}")
print()
print("| level | active lanes | executions | failed rows | unique bug/assertion outputs | status |")
print("| --- | ---: | ---: | ---: | ---: | --- |")

e2e_status = "ok"
if active_lanes["browser-e2e"] < effective_e2e_floor:
    if e2e_disk_pressure_blocked:
        e2e_status = (
            f"blocked: browser/e2e active lanes are below floor, but resource autoscaler reason={resource_reason}; "
            "compact/expire redundant artifacts or clear disk pressure before browser backfill"
        )
    elif e2e_deadline_downscoped:
        e2e_status = (
            f"ACTION-NEEDED: browser/e2e active lanes are below effective deadline floor {effective_e2e_floor}; "
            "repair materialization or write the exact startup/noise blocker"
        )
    else:
        e2e_status = "ACTION-NEEDED: browser/e2e active lanes are below floor; backfill e2e or write the exact load/noise blocker"
elif e2e_deadline_downscoped and active_lanes["browser-e2e"] < e2e_floor:
    e2e_status = (
        f"ok: deadline/low-disk downscope uses effective floor {effective_e2e_floor}; "
        "do not chase nominal browser floor until disk pressure and PR evidence gates clear"
    )
...
for level in ("unit-property", "coverage-guided-lower-level", "backend-api", "protocol-server", "fuzz-assertion"):
    ...
    if level == "unit-property" and execution_counts[level] >= 20000 and unique == 0:
        ...
        if fresh_bounded_evidence[level]:
            status = f"ok: {fresh_bounded_evidence[level]} fresh bounded evidence lane(s); no live lane counted"
        else:
...
if active_lanes["browser-e2e"] < effective_e2e_floor and e2e_disk_pressure_blocked:
    print(
        f"- blocker: browser/e2e is below floor, but resource autoscaler reason={resource_reason}; "
        "next remediation is disk compaction/artifact expiry, then backfill non-paused high-yield e2e groups."
    )
elif active_lanes["browser-e2e"] < effective_e2e_floor and paused_browser:
...
run_synthesis() {
  ...
  cat > "$prompt" <<EOF
Task: synthesize the fuzz-level mix persona reports.
