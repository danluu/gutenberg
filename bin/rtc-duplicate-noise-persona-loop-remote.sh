...
OPERATOR_ALERT="${RTC_OPERATOR_ALERT_PATH:-/media/volume/danluu-fuzz-data/rtc-operator-alerts/rtc-correctness-regression-alert-20260519.md}" # RTC correctness alert injected 2026-05-19
...
DUP_NOISE_SHARE_THRESHOLD="${RTC_DUP_NOISE_SHARE_THRESHOLD:-0.50}"
DUP_NOISE_MIN_CURRENT_SIGNATURES="${RTC_DUP_NOISE_MIN_CURRENT_SIGNATURES:-3}"
...
duplicate_noise_gate_status() {
  local coverage_root="$1"
  if [ -z "$coverage_root" ] || [ ! -f "$coverage_root/novelty-state.json" ]; then
    echo "wait reason=no-novelty-state coverage_root=${coverage_root:-none}"
    return 1
  fi

  node - "$coverage_root/novelty-state.json" "$DUP_NOISE_SHARE_THRESHOLD" "$DUP_NOISE_MIN_CURRENT_SIGNATURES" <<'NODE'
const fs = require( 'fs' );

const statePath = process.argv[ 2 ];
const shareThreshold = Number( process.argv[ 3 ] );
const minCurrentSignatures = Number.parseInt( process.argv[ 4 ], 10 );

let state;
try {
	state = JSON.parse( fs.readFileSync( statePath, 'utf8' ) );
} catch ( error ) {
	console.log( `wait reason=state-read-failed path=${ statePath } error=${ error.message }` );
	process.exit( 1 );
}

const current = state.triageYieldCurrent || state.triageYield || null;
if ( ! current || ! state.lastCompletedFullPassAt ) {
	console.log( 'wait reason=current-full-pass-pending' );
	process.exit( 1 );
}

const top = current.topSemanticFamilies?.[ 0 ] || current.rawTopSemanticFamilies?.[ 0 ] || {};
const topFamily = top.family || 'none';
const actionable = current.actionableSignatureCount ?? current.signatureCount ?? 0;
const raw = current.rawSignatureCount ?? 0;
const share = Math.max(
	current.topDuplicateFamilyShare ?? 0,
	current.rawTopDuplicateFamilyShare ?? 0
);
const startupEvidence = Math.max(
	current.bootstrapStalls ?? 0,
	current.bootstrapStallSignatures ?? 0,
	current.suppressedStrictStartupRecords ?? 0,
	current.suppressedStrictStartupIdentities ?? 0,
	current.suppressedStrictStartupVirtualSignatures ?? 0,
	current.summaryStrictStartupRecords ?? 0,
	current.summaryStrictStartupFailures ?? 0
);
const startupTop = topFamily === 'pre_action_bootstrap_stall';
const duplicateGate = actionable >= minCurrentSignatures && share >= shareThreshold;
const startupGate = startupTop && startupEvidence > 0;
const actionNeeded = duplicateGate || startupGate;

console.log(
	[
		actionNeeded ? 'action' : 'wait',
		`family=${ topFamily }`,
		`share=${ share }`,
		`actionable=${ actionable }`,
		`raw=${ raw }`,
		`likelyReal=${ current.likelyRealVisible ?? 0 }`,
		`productEvidence=${ current.productEvidenceSignatures ?? current.rawProductEvidenceSignatures ?? 0 }`,
		`startupEvidence=${ startupEvidence }`,
		`minCurrent=${ minCurrentSignatures }`,
		`threshold=${ shareThreshold }`,
	].join( ' ' )
);
process.exit( actionNeeded ? 0 : 1 );
NODE
}

should_run_cycle() {
  local coverage_root gate
  coverage_root="$(latest_coverage_root)"
  gate="$(duplicate_noise_gate_status "$coverage_root" 2>/dev/null || true)"
  if [ -z "$gate" ]; then
    gate="wait reason=gate-evaluation-failed coverage_root=${coverage_root:-none}"
  fi
  log "duplicate/noise gate: $gate"
  case "$gate" in
    action\ *) return 0 ;;
    *) return 1 ;;
  esac
...
write_duplicate_noise_gate() {
  ...
  echo
  printf 'Gate evaluation: '
  duplicate_noise_gate_status "$coverage_root" 2>/dev/null || true
  echo
...
write_context() {
  ...
  coverage_root="$(latest_coverage_root)"
  {
    ...
    echo
    echo "## Paths"
    echo "## Operator Correctness Alert"
    echo
    if [ -f "$OPERATOR_ALERT" ]; then
      sed -n '1,220p' "$OPERATOR_ALERT"
    else
      echo "missing $OPERATOR_ALERT"
    fi
    echo
...
main() {
  acquire_process_singleton
  log "loop started model=$MODEL reasoning=$REASONING max_parallel=$MAX_PARALLEL interval=${INTERVAL_SECONDS}s action_every=${ACTION_EVERY_CYCLES} codex_timeout=${CODEX_TIMEOUT_SECONDS}s stale_loop_lock=${STALE_LOOP_LOCK_SECONDS}s duplicate_threshold=${DUP_NOISE_SHARE_THRESHOLD} min_current_signatures=${DUP_NOISE_MIN_CURRENT_SIGNATURES}"
  while true; do
        ...
        if [ "$ACTION_EVERY_CYCLES" -gt 0 ] && [ $(( cycle_index % ACTION_EVERY_CYCLES )) -eq 0 ]; then
          ...
          if [ -n "$latest" ]; then
            if should_run_cycle; then
              ...
              log "skipping duplicate/noise feedback action after cycle $cycle_index because gate cleared"
            fi
          fi
        fi
      else
        log "skipping duplicate/noise review cycle because gate is not action-needed"
      fi
      rm -rf "$BASE/loop.lock"
    else
      local lock_pid lock_started lock_age_seconds
