...
5. Update docs/explanations/architecture/rtc-jetstream2-fuzz-trend-analysis-20260515.md so the status and interpretation reflect the refreshed graph data and the standard persona-loop feedback. Keep it concise and concrete. If the feedback rejects a graph interpretation, say that in the report.
   ...
   Preserve the fuzz output effectiveness section when data/bug_effectiveness_by_level.csv, data/bug_effectiveness_by_profile.csv, data/bug_output_effectiveness_by_level.csv, data/bug_output_effectiveness_by_profile.csv, plots/bug-effectiveness-likely-real-by-level.png, plots/unique-bug-output-cumulative-by-level.png, plots/unique-bug-output-rate-by-level.png, plots/unique-bug-output-cumulative-by-profile-within-level.png, plots/unique-bug-output-rate-by-profile-within-level.png, plots/failure-candidate-effectiveness-by-level.png, plots/bug-effectiveness-by-profile-within-level.png, and plots/failure-candidate-effectiveness-by-profile-within-level.png are present. Explain that the likely-real graphs are triage-output metrics only: they count non-duplicate .triage-watcher result.json rows classified likely_real per 100 runner-hours. Explain that the unique bug-output candidate graphs are broader and dedupe non-infra likely_real/uncertain triage rows, untriaged raw browser/transport failure signatures, and lower-level assertion failures by canonical output key. Do not call untriaged candidates confirmed bugs.
6. Run:
