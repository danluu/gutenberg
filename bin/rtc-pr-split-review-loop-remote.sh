...
MODEL="${RTC_PR_SPLIT_REVIEW_MODEL:-gpt-5.6-sol}"
REASONING="${RTC_PR_SPLIT_REVIEW_REASONING:-xhigh}"
ACTION_REASONING="${RTC_PR_SPLIT_REVIEW_ACTION_REASONING:-max}"
...
EOF
  ...
  (
    cd "$FIX_REPO" || exit 1
    timeout --kill-after=60s "$CODEX_TIMEOUT_SECONDS" "$CODEX_BIN" -a never exec --skip-git-repo-check -m "$MODEL" -c "model_reasoning_effort=$ACTION_REASONING" -s danger-full-access < "$prompt" > "$out" 2> "$err"
    echo "$?" > "$rc_file"
  )
