...
MATERIALIZATION_STARTUP_GRACE_SECONDS=${RTC_RESOURCE_AUTOSCALER_MATERIALIZATION_STARTUP_GRACE_SECONDS:-900}
	...
	awk -F '\t' '
		...
		}
		{
			...
			primary = $(h["primary"]);
				...
				successes = $(h["current_run_successful_group_records"]) + 0;
				open_primary_blocker = primary == "yes" && successes == 0;
				if ($(h["forced"]) == "yes" && $(h["promotion_blocked"]) == "yes" && status_only != "yes" && coverage_state != "absent-from-supervisor-state" && (scheduled == "yes" || live_forced == "yes" || active == "yes" || retained == "yes" || open_primary_blocker)) {
					count++;
				}
			}
		END { print count + 0; }
...
benchmark_canary_slot_floor() {
	local latest floor_path floor floor_file_floor status_floor feedback_floor max_floor required_extra_slots required_status_floor
	latest=$(latest_run)
	floor_path="$latest/benchmark-canary-coverage-floor.tsv"
	floor_file_floor=$(awk -F '\t' '$1 == "slot_floor" { print $2; found = 1 } END { if (! found) print 0 }' "$floor_path" 2>/dev/null || printf '0')
	[[ "$floor_file_floor" =~ ^[0-9]+$ ]] || floor_file_floor=0
	floor=$floor_file_floor
	status_floor=$(benchmark_canary_status_floor 2>/dev/null || true)
	if [[ "$status_floor" =~ ^[0-9]+$ ]] && [ "$status_floor" -gt "$floor" ]; then
		floor=$status_floor
	fi
	[[ "$floor" =~ ^[0-9]+$ ]] || floor=0
	feedback_floor=0
	if [ -s "/media/volume/danluu-fuzz-data/rtc-benchmark-canary-feedback-20260520/current-feedback.tsv" ]; then
	...
	if [[ "$status_floor" =~ ^[0-9]+$ ]] && [ "$status_floor" -gt 0 ]; then
		required_extra_slots=${RTC_FUZZ_NOVELTY_OPEN_BENCHMARK_CANARY_REQUIRED_EXTRA_SLOTS:-9}
		[[ "$required_extra_slots" =~ ^[0-9]+$ ]] || required_extra_slots=9
		required_status_floor=$((status_floor + required_extra_slots))
		if [ "$required_status_floor" -gt "$floor" ]; then
			floor=$required_status_floor
		fi
	fi
	max_floor=${RTC_FUZZ_NOVELTY_OPEN_BENCHMARK_CANARY_REQUIRED_MAX_GROUPS:-24}
	[[ "$max_floor" =~ ^[0-9]+$ ]] || max_floor=24
	if [ "$max_floor" -gt 0 ] && [ "$floor" -gt "$max_floor" ]; then
		floor=$max_floor
	fi
	printf '%s\n' "$floor"
}

benchmark_canary_reserve_floor() {
	local latest floor_path status_floor floor_file_reserve floor_file_blockers floor
	latest=$(latest_run)
	floor_path="$latest/benchmark-canary-coverage-floor.tsv"
	status_floor=$(benchmark_canary_status_floor 2>/dev/null || true)
	if [[ "$status_floor" =~ ^[0-9]+$ ]] && [ "$status_floor" -gt 0 ]; then
	...
	floor_file_reserve=$(awk -F '\t' '$1 == "benchmark_canary_bootstrap_reserve_slots" { print $2; found = 1 } END { if (! found) print 0 }' "$floor_path" 2>/dev/null || printf '0')
	floor_file_blockers=$(awk -F '\t' '$1 == "promotion_blocked_group_count" { print $2; found = 1 } END { if (! found) print 0 }' "$floor_path" 2>/dev/null || printf '0')
	[[ "$floor_file_reserve" =~ ^[0-9]+$ ]] || floor_file_reserve=0
	[[ "$floor_file_blockers" =~ ^[0-9]+$ ]] || floor_file_blockers=0
	floor=$floor_file_reserve
	[ "$floor_file_blockers" -gt "$floor" ] && floor=$floor_file_blockers
	printf '%s\n' "$floor"
}
...
materialization_needs_remediation() {
	...
	local latest_root root_mtime root_age now
	latest_root=$(latest_run)
	if [ -n "$latest_root" ] && [ -d "$latest_root" ]; then
		root_mtime=$(stat -c %Y "$latest_root/source-manifest.tsv" 2>/dev/null || stat -c %Y "$latest_root" 2>/dev/null || printf '0')
		now=$(epoch)
		root_age=$(( now - root_mtime ))
		if [ "$root_age" -ge 0 ] && [ "$root_age" -lt "$MATERIALIZATION_STARTUP_GRACE_SECONDS" ]; then
			return 1
		fi
	fi
...
coverage_root_in_startup_grace() {
	local latest_root started now age
	latest_root=$(latest_run)
	[ -n "$latest_root" ] && [ -d "$latest_root" ] || return 1
	started=$(stat -c %Y "$latest_root/source-manifest.tsv" 2>/dev/null || stat -c %Y "$latest_root" 2>/dev/null || printf '0')
	now=$(epoch)
	age=$(( now - started ))
	[ "$age" -ge 0 ] && [ "$age" -lt "$MATERIALIZATION_STARTUP_GRACE_SECONDS" ]
}
...
write_budget_env() {
	...
	if [ "${target:-0}" -le 0 ] && [ "${max:-0}" -le 0 ]; then
		...
		if [ "$reserve_floor" -gt "$benchmark_canary_bootstrap_reserve_slots" ]; then
			benchmark_canary_bootstrap_reserve_slots=$reserve_floor
		fi
...
const positiveInteger = ( value ) => {
	...
	const runMonitorSlotFloor = Math.max(
		0,
		...lines
		.map( ( line ) =>
			line.match(
				/^export RTC_FUZZ_NOVELTY_BENCHMARK_CANARY_SLOT_FLOOR='([^']*)'/
			)
		)
		.filter( Boolean )
			.map( ( match ) => positiveInteger( match[ 1 ] ) )
	);
	const runMonitorBootstrapReserveSlots = Math.max(
		0,
		...lines
			.map( ( line ) =>
				line.match(
					/^export RTC_FUZZ_NOVELTY_BENCHMARK_CANARY_BOOTSTRAP_RESERVE_SLOTS='([^']*)'/
				)
			)
			.filter( Boolean )
			.map( ( match ) => positiveInteger( match[ 1 ] ) )
	);
	const coverageFloorPath = file.replace(
		/\/run-monitor[.]sh$/,
		'/benchmark-canary-coverage-floor.tsv'
	);
	const coverageFloorValues = new Map();
	try {
		const floorText = fs.readFileSync( coverageFloorPath, 'utf8' );
		for ( const line of floorText.split( /\n/ ) ) {
			const [ key, value ] = line.split( /\t/ );
			if ( key ) {
				coverageFloorValues.set( key, value );
			}
		}
	} catch ( error ) {}
	const coverageFileSlotFloor = positiveInteger(
		coverageFloorValues.get( 'slot_floor' )
	);
	const coveragePromotionBlockedCount = positiveInteger(
		coverageFloorValues.get( 'promotion_blocked_group_count' )
	);
	const coverageBootstrapReserveSlots = positiveInteger(
		coverageFloorValues.get( 'benchmark_canary_bootstrap_reserve_slots' )
	);
...
let targetNumber = Math.max( 0, Number.parseInt( target, 10 ) || 0 );
let maxNumber = Math.max( 0, Number.parseInt( max, 10 ) || 0 );
	const benchmarkCanarySlotFloorNumber = Math.max(
		runMonitorSlotFloor,
		coverageFileSlotFloor
	);
	const benchmarkCanaryReservedSlotNumber = Math.max(
		runMonitorBootstrapReserveSlots,
...
const exportLines = [
	[ 'RTC_FUZZ_NOVELTY_TARGET_ENABLED_GROUPS', targetValue ],
	[ 'RTC_FUZZ_NOVELTY_MAX_ENABLED_GROUPS', maxValue ],
	[ 'RTC_FUZZ_NOVELTY_COVERAGE_GUIDED_TARGET_ENABLED_GROUPS', targetValue ],
	[ 'RTC_FUZZ_NOVELTY_COVERAGE_GUIDED_MAX_ENABLED_GROUPS', maxValue ],
	[ 'RTC_FUZZ_NOVELTY_COVERAGE_QUALITY_MAX_ENABLED_GROUPS', maxValue ],
	[
		'RTC_FUZZ_NOVELTY_BENCHMARK_CANARY_BOOTSTRAP_RESERVE_SLOTS',
...
restart_coverage() {
	...
	RTC_COVERAGE_FORCE_RESTART="$force_restart" \
...
while true; do
	...
	if [ "${desired_target:-0}" -gt "${target:-0}" ] && coverage_root_in_startup_grace; then
		desired_target=${target:-0}
		desired_max=${max:-$desired_target}
		reason=startup_budget_hold
	fi
