...
DEADLINE_PRODUCTIVE_TARGET_GROUPS=${RTC_RESOURCE_AUTOSCALER_DEADLINE_PRODUCTIVE_TARGET_GROUPS:-10}
DEADLINE_PRODUCTIVE_MAX_GROUPS=${RTC_RESOURCE_AUTOSCALER_DEADLINE_PRODUCTIVE_MAX_GROUPS:-10}
...
benchmark_canary_slot_floor() {
	local latest floor_path floor status_floor feedback_floor max_floor required_extra_slots required_status_floor
	latest=$(latest_run)
...
apply_benchmark_canary_finalization_ceiling() {
	...
	cap=${RTC_FUZZ_NOVELTY_BENCHMARK_CANARY_FINALIZATION_MAX_GROUPS:-10}
	[[ "$cap" =~ ^[0-9]+$ ]] || cap=10
...
if ( ! Array.isArray( groups ) ) {
	...
	const stickyGroups = groups.filter(
		...
		const effectiveDesiredMax =
			desiredMax > 0 ? Math.max( desiredMax, policyGroups.length ) : 0;
...
while true; do
	...
	# Disk pressure is a hard safety cap. Re-apply it after floor, repair, and
	# ramp adjustments so coverage-breadth logic cannot restart into ENOSPC.
	read -r desired_target desired_max reason <<<"$(apply_disk_budget "$desired_target" "$desired_max" "$reason" "$disk_avail" "$root_disk_avail")"
