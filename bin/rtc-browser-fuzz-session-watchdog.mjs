...
async function hasNamedSession( sessionName ) {
...
async function supervisorSessionHealth( outputDir, outputAgeMs ) {
	if ( ! outputDir || ! IS_COVERAGE_GUIDED_NOVELTY_WATCHDOG ) {
		return {
			ok: true,
			checked: false,
	...
	const supervisorSession =
		typeof state?.supervisorSession === 'string'
			? state.supervisorSession.trim()
			: '';
	if ( ! supervisorSession ) {
		return {
			ok: true,
			checked: true,
			reason: 'missing-supervisor-session-field',
			statePath,
		};
	}

	if ( await hasNamedSession( supervisorSession ) ) {
		return {
			ok: true,
			checked: true,
			reason: 'supervisor-session-present',
			statePath,
			supervisorSession,
		};
	}
	...
	return {
		ok: false,
		checked: true,
		reason: 'supervisor-session-missing',
		statePath,
		supervisorSession,
		outputAgeMs,
	};
...
async function currentOutputAgeMs( outputDir ) {
	if ( ! outputDir ) {
		return Infinity;
	}
	return Date.now() - ( ( await fileMtimeMs( outputDir ) ) ?? Date.now() );
}
...
async function restart( reason, freshness ) {
	...
	const lastRestartAtIso = new Date( lastRestartAt ).toISOString();
	...
	await writeWatchdogState( {
		...
		reason,
		freshness,
		lastRestartAt: lastRestartAtIso,
	} );
	if ( CLEANUP_COMMAND ) {
...
async function monitorOnce() {
	...
	const outputAgeMs = await currentOutputAgeMs( outputDir );
	...
	const supervisor = await supervisorSessionHealth( outputDir, outputAgeMs );
	if ( ! supervisor.ok ) {
		...
		await restart( 'missing-supervisor-session', {
			...freshness,
			binding,
			supervisor,
			outputAgeMs,
