...
REPO=${RTC_COVERAGE_GUIDED_CLEANUP_REPO:-/media/volume/danluu-fuzz-data/rtc-fuzz-validation-20260515/repo}
BASE=/media/volume/danluu-fuzz-data/rtc-coverage-guided-20260515
DOCKER_TIMEOUT_SECONDS=${RTC_COVERAGE_CLEANUP_DOCKER_TIMEOUT_SECONDS:-20}
...
if command -v docker >/dev/null 2>&1; then
	mapfile -t container_ids < <(timeout "$DOCKER_TIMEOUT_SECONDS" docker ps -aq --filter 'name=wp-env-novelty-' 2>/dev/null || true)
	if [ "${#container_ids[@]}" -gt 0 ]; then
		printf '[%s] docker rm -f wp-env-novelty containers count=%s ids=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "${#container_ids[@]}" "${container_ids[*]}" >> "$log"
		timeout "$DOCKER_TIMEOUT_SECONDS" docker rm -f "${container_ids[@]}" >> "$log" 2>&1 || true
	fi

	mapfile -t network_names < <(timeout "$DOCKER_TIMEOUT_SECONDS" docker network ls --format '{{.Name}}' 2>/dev/null | grep -E '^wp-env-novelty-' || true)
	if [ "${#network_names[@]}" -gt 0 ]; then
		...
		timeout "$DOCKER_TIMEOUT_SECONDS" docker network rm "${network_names[@]}" >> "$log" 2>&1 || true
	fi

	mapfile -t volume_names < <(timeout "$DOCKER_TIMEOUT_SECONDS" docker volume ls -q 2>/dev/null | grep -E '^wp-env-novelty-' || true)
	if [ "${#volume_names[@]}" -gt 0 ]; then
		...
		timeout "$DOCKER_TIMEOUT_SECONDS" docker volume rm -f "${volume_names[@]}" >> "$log" 2>&1 || true
	fi
fi
