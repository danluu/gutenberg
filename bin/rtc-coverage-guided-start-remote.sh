...
if [ ! -f "$NOVELTY_POLICY_CHECK" ] ||
	...
	printf '[%s] refusing coverage start: novelty monitor does not satisfy the hard scheduling policy source=%s checker=%s\n' \
...
BENCHMARK_CANARY_PROMOTION_BLOCKED_GROUP_COUNT=$(
	...
	if [ "$BENCHMARK_CANARY_PRIMARY_GROUP_COUNT" -gt 0 ] &&
		...
		NOVELTY_TARGET_ENABLED_GROUPS=6
		NOVELTY_MAX_ENABLED_GROUPS=6
		NOVELTY_COVERAGE_GUIDED_TARGET_ENABLED_GROUPS=6
		NOVELTY_COVERAGE_GUIDED_MAX_ENABLED_GROUPS=6
		NOVELTY_COVERAGE_QUALITY_MAX_ENABLED_GROUPS=6
		...
		NOVELTY_DEADLINE_COVERAGE_GAP_MIN_RESERVED_GROUPS=1
	fi
...
export RTC_FUZZ_NOVELTY_COVERAGE_CODEX='0'
...
export RTC_FUZZ_TRIAGE_MAX_PARALLEL='1'
