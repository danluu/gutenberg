...
while true; do
	...
	cp "$BASE/context.md" "$run/context.md"
	{
		...
		echo
		echo "## Bounded probe contract"
		echo "- Do not run broad grep -R, find, or unbounded rg over /tmp, $SRC, $BASE, or historical run trees."
		echo "- Prefer this context, exact source files, current run artifacts, and source paths from git -C $SRC ls-files."
		echo "- For source discovery, use explicit paths plus rg -m 80 and head -n 120; do not search /tmp unless an exact current-run artifact path is named."
		echo "- For historical artifacts, use the recent run index below and then open exact report paths only."
		echo
		echo "## Recent native run index"
		find "$BASE/native-runs" -maxdepth 1 -mindepth 1 -type d -printf '%f\n' 2>/dev/null | sort | tail -n 8
		echo
...
Probe constraints:
- Do not run broad grep -R, find, or unbounded rg over /tmp, $SRC, $BASE, or historical run trees.
- Use $run/context.md, exact source files, current run artifacts, or paths discovered from git -C $SRC ls-files.
- Bound searches with explicit paths plus rg -m 80 or head -n 120.
...
Probe constraints:
- Do not run broad grep -R, find, or unbounded rg over /tmp, $SRC, $BASE, or historical run trees.
- Use $run/context.md, exact files named by reports, current run artifacts, or paths discovered from git -C $SRC ls-files.
- Bound searches with explicit paths plus rg -m 80 or head -n 120.
...
- $run/context.md
- $run/synthesis.md
- recent prior native loop reports listed in $run/context.md Recent native run index; open exact report paths only
...
Probe constraints:
- Do not run broad grep -R, find, or unbounded rg over /tmp, $SRC, $BASE, or historical run trees.
- Use $run/context.md, $run/synthesis.md, exact source files, current run artifacts, and the Recent native run index.
- Bound searches with explicit paths plus rg -m 80 or head -n 120.
...
while true; do
	...
	cp "$BASE/context.md" "$run/context.md"
	{
		...
		echo "- For historical artifacts, use the recent run index below and then open exact report paths only."
		echo
		echo "## Recent protocol run index"
		find "$BASE/protocol/runs" -maxdepth 1 -mindepth 1 -type d -printf '%f\n' 2>/dev/null | sort | tail -n 8
		echo
...
- $run/context.md
- $run/synthesis.md
- recent prior protocol loop reports listed in $run/context.md Recent protocol run index; open exact report paths only
...
Probe constraints:
- Do not run broad grep -R, find, or unbounded rg over /tmp, $SRC, $BASE, or historical run trees.
- Use $run/context.md, $run/synthesis.md, exact source files, current run artifacts, and the Recent protocol run index.
- Bound searches with explicit paths plus rg -m 80 or head -n 120.
