...
function isSafeGeneratedWpEnvChildPath( wpEnvHome, candidatePath ) {
	if ( ! wpEnvHome || ! isPathInsideDirectory( wpEnvHome, candidatePath ) ) {
		return false;
	}
	const relativeParts = path
		.relative( path.resolve( wpEnvHome ), path.resolve( candidatePath ) )
		.split( path.sep )
		.filter( Boolean );
	return relativeParts.length >= 2 && relativeParts[ 0 ].startsWith( 'wp-env-' );
}
...
async function findCloneConflictForGroup( groupState, groupConfig ) {
	const wpEnvHome = groupConfig?.env?.WP_ENV_HOME;
	if ( ! wpEnvHome ) {
		return null;
	}
	const logNames = [
		`${ groupState.name }-wp-env-start.log`,
		`${ groupState.name }-wp-env-start-retry.log`,
		`${ groupState.name }-wp-env-start-after-clone-conflict-retry.log`,
		`${ groupState.name }-wp-env-start-after-compose-repair.log`,
		`${ groupState.name }-wp-env-start-after-compose-repair-clone-conflict-retry.log`,
	];
	for ( const logName of logNames ) {
		const logPath = path.join( OUTPUT_DIR, logName );
		const conflictPath = getWpEnvCloneDestinationConflictPath(
			await readTextFile( logPath )
		);
		if (
			conflictPath &&
			isSafeGeneratedWpEnvChildPath( wpEnvHome, conflictPath )
		) {
			return {
				conflictPath,
				installPath: path.dirname( conflictPath ),
				logPath,
				wpEnvHome,
			};
		}
...
async function maybeRepairWpEnvCloneConflictBlocker( supervisorState ) {
	if ( ! supervisorState?.groups?.length ) {
		return false;
	}
	const groupConfigs = await readJsonFile( GROUPS_PATH );
	const groupConfigByName = new Map(
		( Array.isArray( groupConfigs ) ? groupConfigs : [] ).map( ( group ) => [
			group.name,
			group,
		] )
	);
	for ( const groupState of supervisorState.groups ) {
		if (
			...
			! /wp-env start failed|destination path .* already exists/i.test(
				groupState.lastReason ?? ''
			)
		) {
			continue;
		}
		const conflict = await findCloneConflictForGroup(
			groupState,
			groupConfigByName.get( groupState.name )
		);
		if ( ! conflict || ! ( await fileExists( conflict.conflictPath ) ) ) {
			continue;
		}
		const composePath = path.join( conflict.installPath, 'docker-compose.yml' );
		if ( await fileExists( composePath ) ) {
			const composeResult = await runCommand( 'docker', [
				'compose',
				'-f',
				composePath,
				'-p',
				path.basename( conflict.installPath ),
				'down',
				'--remove-orphans',
			] );
			await event( {
				kind: 'wp-env-clone-conflict-compose-down',
				group: groupState.name,
				installPath: conflict.installPath,
				ok: composeResult.ok,
				code: composeResult.code,
				output: composeResult.stderr || composeResult.stdout,
			} );
		}
		await fs.rm( conflict.conflictPath, { recursive: true, force: true } );
		await log(
			`removed stale generated wp-env checkout for ${ groupState.name }: ${ conflict.conflictPath }; restarting supervisor.`
		);
		await event( {
			kind: 'wp-env-clone-conflict-repaired',
			group: groupState.name,
			conflictPath: conflict.conflictPath,
			installPath: conflict.installPath,
			logPath: conflict.logPath,
		} );
		await restartSupervisor( 'wp-env-clone-conflict-repaired' );
		return true;
	}
	return false;
}
...
async function monitorOnce() {
	...
	if ( await maybeRepairWpEnvCloneConflictBlocker( supervisorState ) ) {
		await writeWatchdogState( {
			status: 'restarted-wp-env-clone-conflict',
			stateAgeMs,
