...
STRUCTURAL_BASE=${RTC_PRODUCTIVE_ANALYSIS_STRUCTURAL_BASE:-/media/volume/danluu-fuzz-data/rtc-structural-watchdog-20260518}
...
MAX_ACTIVE_LANES=${RTC_PRODUCTIVE_ANALYSIS_MAX_ACTIVE_LANES:-1}
MIN_LANE_INTERVAL_SECONDS=${RTC_PRODUCTIVE_ANALYSIS_MIN_LANE_INTERVAL_SECONDS:-900}
MODEL=${RTC_PRODUCTIVE_ANALYSIS_MODEL:-gpt-5.6-sol}
...
LANES=(
	"pr-blocker-router"
	"benchmark-product-failure-router"
...
productive_loop_pids() {
	local script_name
	script_name=$(basename "$0")
	ps -eo pid=,args= | awk -v script="$script_name" '
		index($0, script) && $0 ~ / (run|run-locked)( |$)/ { print $1 }
	'
}

stop_orphaned_productive_loop() {
	local pid pids=()
	mapfile -t pids < <(productive_loop_pids)
	for pid in "${pids[@]}"; do
		[ -n "$pid" ] && [ "$pid" != "$$" ] || continue
		log "stopping orphaned productive-analysis controller pid=$pid"
...
atomic_tmp_for() {
	...
	base=$(basename "$dest")
	mkdir -p "$dir"
	mktemp "$dir/.${base}.XXXXXX.tmp"
}

atomic_write() {
	local dest=$1 tmp
	tmp=$(atomic_tmp_for "$dest")
	cat > "$tmp"
	mv -f "$tmp" "$dest"
}
...
write_context() {
	local run_dir=$1 context=$run_dir/context.md coverage_root tmp
	coverage_root=$(latest_coverage_root)
	tmp=$(atomic_tmp_for "$context")
	{
		...
		if [ -n "$coverage_root" ]; then
			...
			bounded_file "Benchmark Canary Coverage Status" "$coverage_root/benchmark-canary-coverage-status.tsv" 180
			echo
			echo "## Benchmark Canary Product Failure Rows"
			if [ -s "$coverage_root/benchmark-canary-coverage-status.tsv" ]; then
				awk -F '\t' '
					...
					END {
						if (!found) {
							print "no current product-failure rows"
						}
					}
				' "$coverage_root/benchmark-canary-coverage-status.tsv"
			else
				echo "missing or empty: $coverage_root/benchmark-canary-coverage-status.tsv"
			fi
		else
			echo
...
Benchmark canary rows with product_evidence_records > 0, retained_product_evidence=yes,
or coverage_state containing product-failure are direct product-repair blockers.
For those rows, emit a P0 or high critical-path action with action_kind=product-repair,
family_or_pr=benchmark-canary-product-failure or benchmark-canary/<group>, and
evidence_path pointing to benchmark-canary-coverage-status.tsv. Do not convert
product-failure rows into generic coverage-retarget actions.
...
- benchmark-product-failure-router: turn current benchmark canary product-failure
  rows, especially rtc-reference-oracle, into focused repair/reduction actions
...
start_lane() {
	...
	[ -z "$active" ] || return 0
	session="rtc-productive-lane-$slug-$(date -u +%Y%m%dT%H%M%SZ)-$$-$RANDOM"
...
EOF
	...
	if has_session "$session"; then
		log "lane session already exists; skipping lane=$lane session=$session"
		return 0
	fi
	if ! tmux new-session -d -s "$session" "bash '$runner'"; then
		log "lane launch failed lane=$lane session=$session"
		return 0
	fi
}

merge_lane_actions() {
	local tmp critical_tmp feedback_tmp now
	now=$(date -u +%Y-%m-%dT%H:%M:%SZ)
	tmp=$(atomic_tmp_for "$ACTIONS")
	critical_tmp=$(atomic_tmp_for "$CRITICAL_FEEDBACK_TSV")
	feedback_tmp=$(atomic_tmp_for "$CRITICAL_FEEDBACK_MD")
	{
		printf 'generated_at\taction_id\ttarget_loop\tpriority\taction_kind\tfamily_or_pr\tevidence_path\tnext_action\tcontrol_path\n'
	...
	mv -f "$critical_tmp" "$CRITICAL_FEEDBACK_TSV"
	{
	...
	mv -f "$feedback_tmp" "$CRITICAL_FEEDBACK_MD"
}

write_report_and_status() {
	local tmp status_tmp now active
	now=$(date -u +%Y-%m-%dT%H:%M:%SZ)
	active=$(active_lane_count)
	tmp=$(atomic_tmp_for "$REPORT")
	status_tmp=$(atomic_tmp_for "$STATUS")
	{
		...
		echo
		echo "It runs targeted analysis lanes that look for work most likely to move the RTC project forward: PR blocker routing, benchmark product-failure routing, benchmark-to-fuzzer closure, deferred-family reduction, and lower-level fuzzing yield retargeting."
		echo
